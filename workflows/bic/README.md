...
- INSPECT routes bounded SOURCE evidence plus a decontextualised DONOR vocabulary packet; no donor verse text and no pre-existing TARGET Scripture are routed.
...
Use `./bic --help` for operator syntax. Use `--source`, `--donor`, and `--target` for BIC resource bindings.
