...
from .vrs import VerseRef, VersificationSchema, load_project_vrs
from .references import ScriptureScope
...
_SCOPE_REFERENCE_RE = re.compile(
    r"(?P<book>[1-4]?[A-Z0-9]{2,3})(?:\s+(?P<chapter>\d+)(?::(?P<start>\d+)(?:-(?P<end>\d+))?)?)?"
)
...
def _issue_intersects_scope(issue: dict[str, str], scope: "ScriptureScope") -> bool:
    ...
    candidates = [str(issue.get("reference", "")), str(issue.get("message", ""))]
    parsed_any = False
        ...
        for match in _SCOPE_REFERENCE_RE.finditer(value.upper()):
            ...
            if start_text is None:
                if scope.start_chapter is None:
                    return True
                if scope.start_verse is None:
                    return chapter == scope.start_chapter
                return (scope.start_chapter or 0) <= chapter <= (scope.end_chapter or scope.start_chapter or 0)
            start = int(start_text)
            end = int(match.group("end") or start)
            if any(scope.contains(VerseRef(scope.book, chapter, verse)) for verse in range(start, end + 1)):
                return True
    # Book-level parser, encoding, duplicate-ID, and missing-file failures remain blocking.
...
def compile_project_scope(
    ...
    initialisation state, but do not prevent a bounded task from starting.
    """
