...
def marker_sequence(text: str) -> tuple[str, ...]:
    """Return the exact normalised USFM marker sequence, including closers."""
    return tuple(
...
def protected_marker_sequence(text: str) -> tuple[str, ...]:
    """Return identity-critical markers while allowing approved layout normalisation."""
    protected: list[str] = []
    for match in _MARKER_RE.finditer(text):
...
def _scope_contains_reference(parent: ScriptureScope, value: str) -> bool:
    """Return whether a submitted reference is authorised by the immutable task scope."""
    ...
    if child.start_verse is None:
        points.extend([(child.start_chapter, 1), (child.start_chapter, 9999)])
    else:
        points.extend(
            [
    ...
    return all(parent.contains(VerseRef(parent.book, chapter, verse)) for chapter, verse in points)




def _atomic_reference_labels(value: str) -> set[str]:
    """Expand one submitted verse/range reference into atomic labels for stage-bound validation."""
    ...
    if end_chapter == scope.start_chapter:
        ...
        return labels
    # Cross-chapter ranges cannot be expanded without a VRS maximum; reject them in stage-bound outputs.
    raise ValidationError(f"Stage-bound finding/reference may not cross chapters: {value}")

def _normalised_stage(operation: str, qa_stage: str | None = None) -> str:
    """Return the exact stage required by one SAW operation or composite QA subtask."""
    if operation == "qa":
        return qa_stage or "TRANSLATION_AND_MEANING_QA"
...
def validate_saw_findings(
    ...
    *,
    ...
    expected_identity = {
        ...
        "operation": operation,
        "scope": scope_value,
        "stage": _normalised_stage(operation, qa_stage),
    }
    ...
    if operation in {"focused", "ol"}:
        ...
        raise ValidationError("SAW QA findings must not introduce a focus question")
    ...
    if not isinstance(raw_receipts, list) or not raw_receipts:
        raise ValidationError(
            ...
            code="QA_REVIEW_EVIDENCE_MISSING",
    ...
    for index, raw_receipt in enumerate(raw_receipts, start=1):
        ...
        if required_checks - checks:
            raise ValidationError(
                ...
                code="QA_REVIEW_EVIDENCE_INCOMPLETE",
                affected_scope=scope_value,
            )
    ...
    for index, raw in enumerate(raw_adjudications, start=1):
        ...
        finding_id = str(row.get("finding_id", "")).strip() or None
    ...
    if operation == "qa" and _normalised_stage(operation, qa_stage) == "TRANSLATION_AND_MEANING_QA":
        ...
        for index, request in enumerate(raw_requests, start=1):
            ...
            reference = _require_string(request.get("target_reference"), f"ol_review_requests[{index}].target_reference", maximum=160)
            ...
            deferred_finding_id = _require_string(
                request.get("deferred_finding_id"),
                f"ol_review_requests[{index}].deferred_finding_id",
                maximum=64,
            ).upper()
            if not _FINDING_ID_RE.fullmatch(deferred_finding_id):
                raise ValidationError(f"ol_review_requests[{index}].deferred_finding_id has invalid syntax")
        ...
        raise ValidationError("OL review requests may be emitted only by the SAW QA meaning stage")
    ...
    for index, raw_resolution in enumerate(raw_resolutions, start=1):
        ...
        target_reference = _require_string(resolution.get("target_reference"), f"ol_resolutions[{index}].target_reference", maximum=160)
        ...
        finding_id = (
            str(raw_finding_id).strip().upper()
    ...
    if expected_request_set:
        if seen_resolution_ids != expected_request_set:
            raise ValidationError("SAW QA OL stage does not reconcile the exact requested OL issue inventory")
    elif ol_resolutions:
        raise ValidationError("ol_resolutions are valid only for a requested SAW QA OL stage")
    ...
    for index, raw in enumerate(findings, start=1):
        ...
        finding_id = _require_string(row.get("finding_id"), f"findings[{index}].finding_id", maximum=64).upper()
        if not _FINDING_ID_RE.fullmatch(finding_id):
            raise ValidationError(f"findings[{index}].finding_id has invalid syntax")
        ...
        stage_name = _normalised_stage(operation, qa_stage)
        if operation == "qa" and stage_name in {"STRUCTURAL_ADJUDICATION", "SELECTIVE_OL_ADJUDICATION"}:
            if not _atomic_reference_labels(reference).issubset(set(expected_references)):
                raise ValidationError(
                    f"findings[{index}].target_reference is outside the exact composite-QA stage references"
                )
        ...
        if (operation == "ol" or (operation == "qa" and _normalised_stage(operation, qa_stage) == "SELECTIVE_OL_ADJUDICATION")) and not ol_evidence:
            raise ValidationError(
    ...
    stage_name = _normalised_stage(operation, qa_stage)
    if operation == "qa" and stage_name == "TRANSLATION_AND_MEANING_QA":
    ...
    if operation == "qa" and stage_name == "SELECTIVE_OL_ADJUDICATION":
    ...
    if operation == "qa" and stage_name == "STRUCTURAL_ADJUDICATION":
...
def render_action_report(document: Mapping[str, Any]) -> str:
    """Render one deterministic human-facing SAW action report."""
    lines = [
        "# SAW Action Report",
        "",
        ...
        f"- Coverage: `{document['coverage']['status']}` ({len(document['coverage']['reviewed_references'])} coordinates)",
    ]
    if document.get("focus"):
        lines.append(f"- Focus: {document['focus']}")
    if document.get("check_type"):
        lines.append(f"- Check type: `{document['check_type']}`")
    if document.get("answer"):
        lines.extend(["", "## Answer", "", str(document["answer"])])
    lines.extend(["", "## Actionable findings", ""])
    findings = list(document.get("findings", []))
    if not findings:
        lines.append("No actionable findings were submitted.")
    for row in findings:
        lines.extend(
            [
                f"### {row['finding_id']} — {row['target_reference']}",
                "",
                ...
                f"**Issue:** {row['issue']}",
                "",
                f"**Required action:** {row['required_action'] or 'No action required.'}",
                "",
                f"**Evidence IDs:** {', '.join(row['evidence_ids'])}",
                "",
            ]
        )
        if row.get("grammar_rule_ids"):
            lines.extend([f"**Grammar rule IDs:** {', '.join(row['grammar_rule_ids'])}", ""])
        if row.get("original_language_evidence"):
            lines.extend([f"**Original-language evidence:** {row['original_language_evidence']}", ""])
    return "\n".join(lines).rstrip() + "\n"


def render_operator_note_text(document: Mapping[str, Any]) -> str:
    """Render plain copy/paste-ready issue blocks for manual Paratext Notes entry."""
    findings = list(document.get("findings", []))
    if not findings:
        return "NO ACTIONABLE ISSUES\n"
    blocks: list[str] = []
    for row in findings:
        evidence = ", ".join(row.get("evidence_ids", [])) or "none"
        recommendation = row.get("required_action") or "Review with the Team."
        blocks.append(
            "\n".join(
                [
                    f"REFERENCE: {row['target_reference']}",
                    f"TYPE: {row['category']}",
                    f"ACTION LEVEL: {row['action_level']}",
                    f"CONFIDENCE: {row['confidence']}",
                    "ISSUE:",
