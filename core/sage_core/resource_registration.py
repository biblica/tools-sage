...
def compatible_language_options(settings_path: Path, role: str) -> tuple[tuple[str, str | None], ...]:
    ...
    RC7.04 uses this only while assigning a Project to a Job role. Language-profile variants
...
def register_external_scripture_resource(
    ...
    *,
    ...
    ``role`` and ``profile_variant`` are accepted only for source compatibility with earlier RC7
    callers. RC7.04 deliberately ignores them: SOURCE/DONOR/TARGET/WIP/REFERENCE and grammar
...
def register_catalogued_scripture_project(
    ...
    *,
    ...
    """Add one preparsed Paratext catalogue row to SAGE with no Job role assignment."""
    ...
    language_code = str(catalogue_row.get("language_iso") or "").strip().casefold()
    if not project_id or not project_path.is_dir():
        raise ValidationError("Catalogue row does not resolve to a valid Project folder", code="PARATEXT_CATALOG_ENTRY_INVALID")
    ...
    base_file = reported_base if reported_base.casefold() in config.base_vrs_files else config.canonical_versification
    paratext_meta = {
