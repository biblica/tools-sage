...
def _remove(path: Path, removed: list[str], root: Path) -> None:
    """Remove one prevalidated generated path and record its workspace-relative name."""
...
def _reset_inspect_memory(
    ...
    approved = eligible_memory_records(remaining_proposals + _read_list(memory_root / "lexicon-records.json"))
