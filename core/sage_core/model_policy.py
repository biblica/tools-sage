...
def model_policy_path(root: Path) -> Path:
    ...
    return root.resolve() / "meta" / "model-policy.yml"
...
def catalog_cache_path(root: Path, provider: str = "codex") -> Path:
    ...
    return root.resolve() / "state" / "model-catalog" / f"{provider}.json"
...
def ensure_reasoning_effort_supported(policy: dict[str, Any], effort: str | None) -> str | None:
    """Normalise one explicit effort and reject anything above SAGE's XHigh ceiling."""
    if effort is None:
        return None
