"""Section-led bounded work-unit planning with labelled context overlap."""
...
def _serialized_stats_json(value: Any) -> _SerializedStats:
    """Serialise JSON deterministically before measuring its context cost."""
...
class _PacketSizer:
    ...
    def __init__(
        ...
        """Initialise the instance with the supplied governed state."""
    ...
    def _list_stats(values: Iterable[_SerializedStats], count: int) -> _SerializedStats:
        """Measure and sum a sequence of serialised evidence items."""
    ...
    def measure(
        ...
        *,
        ...
        """Measure serialised packet content against configured context limits."""
...
@dataclass(frozen=True)
class WorkUnit:
    """One primary evidence unit with optional labelled context-only records."""
...
def build_evidence_packet(unit: WorkUnit, shared: dict[str, Any] | None = None) -> dict[str, Any]:
    """Build the exact labelled packet represented by a planned work unit."""
    return _packet(
        unit.unit_id,
        unit.primary,
        unit.context_before,
...
def _select_end(
    ...
    """Choose the strongest valid split boundary before the context limit is exceeded."""
    if furthest == len(records) - 1:
        return furthest
    threshold = min(
        policy.minimum_target_tokens,
        int(policy.target_estimated_tokens * 0.60),
    )
    ...
    for end in range(start, furthest + 1):
        ...
        if measurement.estimated_tokens < threshold and end < furthest:
            continue
        distance = abs(measurement.estimated_tokens - policy.target_estimated_tokens)
        candidates.append(
            (next_record.boundary_score, -distance, end, measurement.estimated_tokens)
        )
    if not candidates:
        return furthest
    ...
    while start < len(ordered):
        ...
        if furthest < start:
            ...
            raise EvidenceLimitError(
                f"Single verse record {ordered[start].reference} exceeds hard limits: "
...
def manifest(
    ...
    *,
    ...
    """Serialise a complete work-unit plan for audit and later task routing."""
