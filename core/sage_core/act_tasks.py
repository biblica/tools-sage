...
from .bounded_target import merge_bounded_usfm, record_target_commit
...
def _relative(root: Path, path: Path) -> str:
    """Return a workspace-relative path and reject any path outside the SAGE root."""
    try:
        return path.resolve().relative_to(root.resolve()).as_posix()
    except ValueError as exc:
        raise ValidationError(f"Task path is outside the SAGE workspace: {path}") from exc
...
def _vrs_provenance_reference(
    ...
    *,
    ...
    """Label one authorised VRS source without exposing it as a model-readable task path."""
    try:
    ...
    try:
        ...
        if custom:
            raise ValidationError(
                f"Project custom VRS is outside its authorised Project root: {path}",
...
def load_skill_registry(root: Path) -> dict[tuple[str, str], SkillBinding]:
    ...
    if not isinstance(items, list) or not items:
        raise ConfigurationError("meta/skills.yml must contain a non-empty skills list")
    result: dict[tuple[str, str], SkillBinding] = {}
    for index, raw_item in enumerate(items):
        item = require_mapping(raw_item, f"skills[{index}]")
        skill_id = require_string(item.get("id"), f"skills[{index}].id")
        workflow = require_string(item.get("workflow"), f"skills[{index}].workflow").lower()
        operation = require_string(item.get("operation"), f"skills[{index}].operation").lower()
        file_value = require_string(item.get("file"), f"skills[{index}].file")
        ...
        binding = SkillBinding(
            ...
            source_system=require_string(
                item.get("source_system"), f"skills[{index}].source_system"
            ),
            source_version=require_string(
                item.get("source_version"), f"skills[{index}].source_version"
            ),
            original_file=(root / require_string(
                item.get("original_file"), f"skills[{index}].original_file"
            )).resolve(),
            original_sha256=require_string(
                item.get("original_sha256"), f"skills[{index}].original_sha256"
            ).lower(),
            adapted_sha256=require_string(
                item.get("adapted_sha256"), f"skills[{index}].adapted_sha256"
            ).lower(),
            qualification_status=require_string(
                item.get("qualification_status"), f"skills[{index}].qualification_status"
            ).upper(),
        )
        try:
...
def _load_bic_protected_rewrite_contract(config: EcosystemConfig) -> dict[str, Any]:
    ...
    pin_path = config.root / "meta" / "bic-protected-rewrite-contract.yml"
    raw = load_yaml(pin_path)
...
def _load_bic_protected_verb_selection_contract(config: EcosystemConfig) -> dict[str, Any]:
    ...
    pin_path = config.root / "meta" / "bic-protected-verb-selection-contract.yml"
    raw = load_yaml(pin_path)
...
def _assert_project_scope(project: ProjectSpec, scope: ScriptureScope, label: str) -> None:
    ...
    if scope.book not in books:
        raise ValidationError(
            f"{label} project {project.project_id} does not authorise book {scope.book}"
        )
...
def _assert_initialized_and_ready(
    ...
    """Require fresh initialisation and exact-scope resource readiness."""
    ...
    # root ecosystem here would discard the Job initialisation performed by the menu.
    receipt_config = config
    state_path = ecosystem_state_path(config.workspace_data_root)
    ...
    if not state and isinstance(runtime_context, Mapping) and runtime_context.get("kind") == "JOB":
        # Direct API/CLI callers historically initialise the root configuration before
        # SAGE derives the owning Job. Preserve that route when no Job receipt exists;
        # an explicitly initialised Job receipt always takes precedence.
        base_config = load_ecosystem(config.root / "ecosystem.yml")
        base_state = read_state(ecosystem_state_path(base_config.workspace_data_root))
        if base_state:
            receipt_config = base_config
            state = base_state
    if not state:
        raise InputRequiredError(
            "Run `./sage workspace initialize` before creating analytical ACT tasks",
            code="WORKSPACE_INITIALIZATION_INPUT_REQUIRED",
            received="NOT_RUN",
            suggestions=[
                {
                    "value": "sage workspace initialize",
                    "label": "Run guided workspace initialisation",
                    "score": 1.0,
                    "confidence": "AUTHORITATIVE",
                }
            ],
            next_action="Run `./sage workspace initialize` and retry the exact scope.",
            affected_scope=scope.label(),
        )
    if state.get("state") not in {"READY", "READY_WITH_ACTIONS", "READY_WITH_LIMITATIONS"}:
        raise ValidationError(
            "The last workspace initialisation is not executable; use guided remediation and rerun initialisation",
    ...
    if (
        ...
        raise InputRequiredError(
            "Effective settings changed after initialisation; rerun `./sage workspace initialize`",
            code="WORKSPACE_INITIALIZATION_INPUT_REQUIRED",
            received="STALE",
            suggestions=[
                {
                    "value": "sage workspace initialize",
                    "label": "Rerun guided workspace initialisation",
                    "score": 1.0,
                    "confidence": "AUTHORITATIVE",
                }
            ],
            affected_scope=scope.label(),
            next_action="Rerun workspace initialisation and recreate the task.",
        )
    ...
    if workflow_state.get("controller_available") is False:
        raise ValidationError(
            f"{workflow.upper()} controller is unavailable according to the last initialisation",
    ...
    if int(workflow_state.get("pending_transactions", 0) or 0) != 0 or incomplete_transactions(
        ...
        raise ValidationError(
            ...
            next_action=f"Run `./sage transaction recover --workflow {workflow}`.",
            affected_scope=scope.label(),
        )
    ...
    if not job_id:
        ...
        if workflow == "bic":
            ...
            job = store.create_job(
                tool="saw",
                job_id=project_id,
                display_name=f"{output_project_id} analysed against {contemporary_source_id}",
                bindings={
...
def validate_act_request_readiness(
    ...
    *,
    ...
    """Validate initialisation and task-specific project readiness without creating a task."""
...
def _scope_units(path: Path, scope: ScriptureScope) -> tuple[list[dict[str, Any]], set[VerseRef], str]:
    """Select authorised USJ units from one atomic read and return its exact byte hash."""
    try:
        raw = path.read_bytes()
...
def _write_scope_packet(source: Path, scope: ScriptureScope, destination: Path) -> dict[str, Any]:
    """Write only the selected USFM scope into one immutable task packet."""
    units, refs, source_sha256 = _scope_units(source, scope)
    lines = [f"\\id {scope.book} SAGE bounded evidence packet"]
    ...
    return {
        ...
        "marker_sequence": list(marker_sequence(text)),
    }
...
def _write_bic_donor_vocabulary(
    ...
    """Harvest decontextualised DONOR vocabulary without routing donor Scripture wording."""
...
def _write_reference_inventory_packet(
    source: Path,
    ...
    *,
    ...
    """Write only exact verse coordinates named by a composite-QA stage inventory."""
    ...
    requested_scopes = [parse_scope(value) for value in reference_values]
    if not requested_scopes:
        raise ValidationError("Composite QA stage reference inventory must not be empty")
    for requested in requested_scopes:
        if requested.book != parent_scope.book:
            raise ValidationError("Composite QA stage reference inventory crosses the parent book")
    selected: list[dict[str, Any]] = []
    refs: set[VerseRef] = set()
    for unit in parse_usj_units(usj):
        ...
        intersection = {
            ref for ref in unit_refs
            if any(requested.contains(ref) for requested in requested_scopes)
        }
        if not intersection:
            continue
        if intersection != unit_refs:
    ...
    lines = [f"\\id {parent_scope.book} SAGE bounded stage evidence packet"]
    current_chapter: int | None = None
...
def _write_bic_governance_packets(config: EcosystemConfig, packet_root: Path, scope: ScriptureScope) -> list[Path]:
    """Write bounded BIC memory, challenge, and review evidence into the task packet."""
    memory_root = _memory_root(config)
    approved = eligible_memory_records(_load_list(memory_root / "approved-memory.json"))
    challenges = [
        row
        for row in _load_list(memory_root / "translation-challenges.json")
        if str(row.get("scope", "")) == scope.label()
...
def _write_saw_preflight(
    ...
    preflight = {
        ...
        "evidence_ids": [
            ...
            "PROJECT-GRAMMAR",
            *(
                ["ORIGINAL_LANGUAGE", "VRS-ORIGINAL_LANGUAGE"]
                if ol_project is not None
                else []
            ),
        ],
...
def _load_predecessor(
    ...
    *,
    ...
    if not challenge_path.is_file():
        challenge_path = path.parent / "validation" / "normalised-translation-challenges.json"
    ...
    if ol_used:
        ol_path = path.parent / "packet" / "original-language.usfm"
        ...
        inherited_ol = {
            "usfm_path": ol_path,
            "usfm_sha256": sha256_file(ol_path),
            "vrs_path": vrs_path,
...
def _context_measurement(
    ...
    """Measure the complete ACT, manifest, and routed evidence context for budget reporting."""
    ...
    return {
        ...
        "measurement_scope": "allowed_reads+ACT+manifest",
...
def _enforce_context_budget(
    ...
    *,
    ...
    """Reject a task whose complete generated context exceeds configured byte or token limits."""
    failures: list[str] = []
    ...
    if failures:
        raise EvidenceLimitError(
            f"{operation} complete ACT context exceeds governed limits: " + "; ".join(failures),
            affected_scope=scope.label(),
...
def _stage_record(result: Mapping[str, Any], stage: str) -> dict[str, Any]:
    """Normalise one task or partitioned-plan result into a composite QA stage record."""
...
def _create_saw_qa_composite(
    ...
    *,
    ...
    output = config.project(output_project_id)
    candidates = _structural_candidates(config, output, scope)
    first_stage = "STRUCTURAL_ADJUDICATION" if candidates else "TRANSLATION_AND_MEANING_QA"
    ...
    plan = {
        ...
        "contemporary_source": contemporary_source_id,
        "grammar_override_id": grammar_override_id,
        "structural_stage_required": bool(candidates),
        "stages": [stage],
        "created_utc": utc_now(),
    }
    ...
    *,
    ...
    if operation == "focused":
        ...
        if normalized_check_type not in SAW_CHECK_TYPES:
            raise ValidationError(f"Unsupported SAW focused check type: {normalized_check_type}")
    elif normalized_check_type:
        raise ValidationError("--type is valid only for SAW focused checks")
    ...
    # An explicit discourse-unit ceiling remains available as a hard policy limit.
    # Normal SAW leaves it unlimited so the token target can coalesce adjacent
    # natural units instead of producing context-starved microtasks.
    if (
        auto_partition
        and workflow == "saw"
        and operation == "qa"
        ...
        qa_policy = load_workflow_profile(config, config.workflow(workflow)).evidence_policy(operation)
        if qa_policy.maximum_primary_discourse_units > 0:
            qa_records = records_from_project_result(
                output.project_id,
                compiled[output.project_id],
                resource_role="WIP",
            )
            qa_selected = select_records_for_scope(qa_records, scope)
            discourse_ids = {
                record.discourse_unit_id or record.reference
                for record in qa_selected
            }
            if len(discourse_ids) > qa_policy.maximum_primary_discourse_units:
                plan_seed = sha256_bytes(
                    json.dumps(
                        {
                            "workflow": workflow,
    ...
    try:
        ...
        contemporary_packet = packet_root / ("source.usfm" if workflow == "bic" else "reference.usfm")
        packet_records["contemporary_source"] = (
            _write_reference_inventory_packet(source_file, stage_reference_values, contemporary_packet, parent_scope=scope)
            if workflow == "saw" and operation == "qa" and stage_reference_values
            else _write_scope_packet(source_file, scope, contemporary_packet)
        )
        ...
        if context_references:
            context_reference_packet = packet_root / "context-reference.usfm"
            packet_records["context_contemporary_source"] = _write_reference_inventory_packet(
                source_file,
                context_references,
        ...
        if route_ol or conditional_ol:
            assert ol_file is not None
            ol_packet = packet_root / "original-language.usfm"
            packet_records["original_language"] = (
                _write_reference_inventory_packet(ol_file, stage_reference_values, ol_packet, parent_scope=scope)
                if workflow == "saw" and operation == "qa" and stage_reference_values
                else _write_scope_packet(ol_file, scope, ol_packet)
            )
            packet_records["original_language"]["evidence_id"] = "ORIGINAL_LANGUAGE"
        ...
        if predecessor and predecessor.get("inherited_ol"):
            ...
            atomic_write_bytes(ol_packet, Path(inherited["usfm_path"]).read_bytes())
            atomic_write_bytes(inherited_vrs_packet, Path(inherited["vrs_path"]).read_bytes())
            if sha256_file(ol_packet) != inherited["usfm_sha256"] or sha256_file(inherited_vrs_packet) != inherited["vrs_sha256"]:
                raise ValidationError(
            ...
            packet_records["original_language"] = {
                "path": ol_packet.name,
                "packet_sha256": inherited["usfm_sha256"],
                "scope": scope.label(),
        ...
        if predecessor:
            target_packet = packet_root / "staged-target.usfm"
            atomic_write_text(target_packet, predecessor["rewrite_path"].read_text(encoding="utf-8"))
            packet_records["output_project"] = {
                "path": target_packet.name,
                "source_task": predecessor["task_id"],
                "source_sha256": predecessor["rewrite_sha256"],
                "packet_sha256": sha256_file(target_packet),
                "scope": scope.label(),
                "atomic_references": list(packet_records["contemporary_source"]["atomic_references"]),
                "marker_sequence": list(marker_sequence(target_packet.read_text(encoding="utf-8"))),
                "evidence_id": "CANDIDATE",
            }
        elif output_file is not None:
            target_packet = packet_root / "wip.usfm"
            packet_records["output_project"] = (
                _write_reference_inventory_packet(output_file, stage_reference_values, target_packet, parent_scope=scope)
                if workflow == "saw" and operation == "qa" and stage_reference_values
                else _write_scope_packet(output_file, scope, target_packet)
            )
            packet_records["output_project"]["evidence_id"] = "WIP"
        elif workflow == "saw":
            raise ValidationError("SAW WIP has no bounded USFM input")

        context_wip_packet: Path | None = None
        if context_references:
            if output_file is None:
                raise ValidationError("SAW context routing requires a WIP Scripture file")
            context_wip_packet = packet_root / "context-wip.usfm"
            packet_records["context_output_project"] = _write_reference_inventory_packet(
                output_file,
                context_references,
                context_wip_packet,
                parent_scope=scope,
        ...
        semantic_sources = [("source" if workflow == "bic" else "reference", source, contemporary_packet)]
        if workflow == "saw" and target_packet is not None:
            semantic_sources.append(("wip", output, target_packet))
        if route_ol and ol_packet is not None and ol_project is not None:
            semantic_sources.append(("original-language", ol_project, ol_packet))
        for semantic_label, semantic_project, semantic_source_path in semantic_sources:
            semantic_packet = scope_evidence_for_project(
                config,
                project_id=semantic_project.project_id,
                text=semantic_source_path.read_text(encoding="utf-8-sig", errors="replace"),
            )
        ...
        if workflow == "bic":
            governance_packets = _write_bic_governance_packets(config, packet_root, scope)
        ...
        if workflow == "saw":
            extra_inputs, preflight = _write_saw_preflight(
                config, packet_root, output, source, ol_project if route_ol else None, scope, packet_records
            )
        else:
        ...
        read_paths: list[Path] = [
            ...
            *extra_inputs,
            *(
                [config.root / "meta" / "schemas" / "saw-findings.schema.yml"]
                if workflow == "saw"
                else []
            ),
            *_skill_files(skill),
        ...
        if protected_verb_selection_contract is not None:
            read_paths.append(
                config.root / protected_verb_selection_contract["canonical_file"]
            )
        deduped: list[Path] = []
        seen: set[Path] = set()
        ...
        allowed_reads = [
            {"path": _relative(config.root, read_path), "sha256": sha256_file(read_path)}
        ...
        default_evidence_ids = (
            ...
            if workflow == "bic"
                ...
                "WIP",
                "PROJECT-GRAMMAR",
                *(["ORIGINAL_LANGUAGE"] if route_ol else []),
            ]
        )
        ...
        identity = {
            ...
            "review_requirements": (
                {
                    "required_checks": _required_review_checks(operation, normalized_check_type, qa_stage),
        ...
        submit_posix = (
            f'./sage --settings {config.settings_path.name} task submit '
            f'--task "{_relative(config.root, manifest_path)}"'
        )
        submit_windows = (
            f'sage.cmd --settings {config.settings_path.name} task submit '
            f'--task "{_relative(config.root, manifest_path)}"'
        )
        ...
        act_lines = [
            ...
            *(
                ...
                if workflow == "bic"
                    ...
                    f"- Authorised REFERENCE: `{source.project_id}`",
                ]
            ),
            *(
        ...
        if workflow == "bic" and operation == "inspect":
            act_lines.extend([
                "1. Inspect only the bounded SOURCE packet, decontextualised DONOR vocabulary evidence, and routed source-language grammar evidence; routine INSPECT does not route OL Scripture.",
            ...
            act_lines.extend([
                "1. Use the committed INSPECT record, SOURCE, decontextualised DONOR vocabulary evidence, applicable approved memory, routed source and target grammar contracts, and protected rewrite rules; do not open conditional OL files during ordinary drafting.",
            ...
            if qa_stage == "STRUCTURAL_ADJUDICATION":
                ...
                act_lines.extend([
                    "1. Resolve exactly the OL review requests inherited from the meaning stage.",
                    "2. Compare the WIP directly with the routed authoritative GRK or HEB packet and use the REFERENCE as the normal LWC benchmark.",
                    "3. Do not broaden into general QA; every OL conclusion must answer one inherited request.",
                    "4. Return exactly one structured ol_resolutions object per inherited request. A FINDING outcome must use that request's deferred_finding_id and cite actual routed OL evidence.",
                ])
        elif operation == "focused":
            act_lines.extend([
                "1. Answer the one focus question only, using the routed evidence and check type.",
                "2. Do not broaden into normal QA.",
                "3. Reconcile every supplied structural candidate and bounded coordinate.",
            ])
        else:
            act_lines.extend([
                "1. Compare the WIP translation directly with the relevant authoritative OL packet for the one question.",
                "2. Use the authorised REFERENCE as the normal benchmark.",
                "3. Do not broaden into commentary or a general book study.",
                "4. Reconcile every supplied structural candidate and bounded coordinate.",
            ])
        act_lines.extend(["", "## Allowed reads", ""])
        act_lines.extend(f"- `{item['path']}`" for item in allowed_reads)
        if conditional_reads:
            ...
            act_lines.extend(
                f"- `{item['path']}` — condition: `{item['condition']}`"
        ...
        if workflow == "bic" and operation == "inspect":
            act_lines.extend([
                "- Match `meta/schemas/bic-inspect-submission.schema.yml`.",
            ...
            act_lines.extend([
                ...
                "- Match `meta/schemas/bic-grammar-assessment.schema.yml` for the companion assessment.",
                "- The assessment must cover every project-grammar rule ID exactly once.",
                *( [
                    "- Match `meta/schemas/bic-translation-challenges.schema.yml` version 1.1 for `output/translation-challenges.json`.",
            ...
            act_lines.extend([
                ...
                *( [f"- Set `stage` exactly to `{qa_stage}`."] if operation == "qa" and qa_stage else [] ),
        ...
        budget_reads = [*allowed_reads, *conditional_reads]
        telemetry = _context_measurement(config.root, budget_reads, act_text, base_manifest)
        ...
        try:
            ...
            raise
        identity["context_budget"] = {**telemetry, "policy": policy.to_dict()}
        ...
        # Final measurement includes the complete manifest. The recorded values are
        # diagnostic, while enforcement above conservatively included the same content.
        final_telemetry = _context_measurement(config.root, budget_reads, act_text, manifest)
        identity["context_budget"]["final_serialized_bytes"] = final_telemetry["serialized_bytes"]
        identity["context_budget"]["final_estimated_tokens"] = final_telemetry["estimated_tokens"]
        fingerprint = sha256_bytes(
            json.dumps(identity, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
        )
        manifest = {
...
def aggregate_act_plan(config: EcosystemConfig, plan_path: Path) -> dict[str, Any]:
    ...
    for unit in plan.get("work_units", []):
        manifest_path = Path(str(unit.get("manifest_path", ""))).resolve()
        ...
        observed.extend(normalized["coverage"]["reviewed_references"])
        receipts.extend(normalized.get("review_receipts", []))
        adjudications.extend(normalized.get("structural_adjudications", []))
        ol_review_requests.extend(normalized.get("ol_review_requests", []))
        ol_resolutions.extend(normalized.get("ol_resolutions", []))
        resolved_ol_request_ids.extend(normalized.get("resolved_ol_request_ids", []))
        for finding in normalized.get("findings", []):
            finding_id = str(finding.get("finding_id", ""))
            if finding_id in finding_ids:
                raise ValidationError(f"Duplicate finding_id across work units: {finding_id}")
...
def submit_act_task(config: EcosystemConfig, task_manifest: Path) -> dict[str, Any]:
    ...
    if control.get("settings_sha256") != sha256_file(config.settings_path):
        raise ValidationError(
            ...
            next_action="Use Reset / restart active Run with current configuration; the old Run will be preserved.",
        )
    if Path(str(control.get("manifest_path", ""))).resolve() != path:
        raise ValidationError("ACT manifest path differs from the trusted control record")
    if Path(str(control.get("task_root", ""))).resolve() != task_root:
    ...
    act_path = Path(str(control.get("act_path", ""))).resolve()
    ...
    if raw.get("schema_version") != "2.2":
        raise ValidationError("ACT task schema_version must be 2.2")

    for item in raw.get("allowed_reads", []):
        if not isinstance(item, dict):
            raise ValidationError("ACT allowed_reads contains a non-object entry")
        read_path = (config.root / str(item.get("path", ""))).resolve()
        try:
            read_path.relative_to(config.root.resolve())
        except ValueError as exc:
            raise ValidationError("ACT input path escapes the SAGE workspace") from exc
    ...
    for item in raw.get("conditional_reads", []):
        ...
        try:
            ...
            raise ValidationError("ACT conditional input path escapes the SAGE workspace") from exc
        if not read_path.is_file():
        ...
        if operation == "rewrite":
            ...
            challenge_document = validate_rewrite_challenges(
                ...
                ol_evidence_available=any(
                    str(item.get("path", "")).endswith("original-language.usfm")
                    for item in raw.get("conditional_reads", [])
                ),
            )
            ...
            atomic_write_json(validation_root / "normalised-translation-challenges.json", challenge_document)
