...
from .iso_languages import suggest_profile_alias
...
class Job:
    ...
    def runtime_settings_path(self) -> Path:
        """Manage `runtime settings path` for Job-scoped state and storage."""
        return self.root / ".sage" / "runtime.yml"

    @property
    def runtime_profile_path(self) -> Path:
        """Manage `runtime profile path` for Job-scoped state and storage."""
        return self.root / ".sage" / "profile.yml"
...
class JobStore:
    ...
    def __init__(self, sage_root: Path, settings_path: Path | None = None) -> None:
        ...
        self.jobs_root = self.sage_root / "jobs"
        self.state_root = self.sage_root / "state"
        ...
        *,
        ...
        def resolve_profile(project: Any, role: str, key: str) -> str:
            ...
            if len(candidates) == 1:
                return candidates[0]
            if not candidates:
                alias_suggestion = suggest_profile_alias(
                ...
                raise ValidationError(
                    f"Language profile required for {project.language_code}. No compatible SAGE grammar "
                    f"profile is configured for {project.project_id} as {role}.",
                    code="LANGUAGE_PROFILE_NOT_CONFIGURED",
                    next_action=(
                        f"Configure or approve a {project.language_code} grammar profile for {role}, "
                        "then create the Job again. The Project remains available in SAGE."
                    ),
                    details={
                        "project": project.project_id,
                        "language": project.language_code,
                        "role": role,
                        **({"profile_alias_suggestion": alias_suggestion} if alias_suggestion else {}),
                    },
                )
            raise ValidationError(
    ...
    def create_job(
        ...
        *,
        ...
        """Create one canonical RC7 Job from fixed SAGE Project bindings."""
    ...
    def remove_job(self, project: Job) -> None:
        """Remove one SAGE Job and its Job-owned Runs/reports without touching Scripture Projects."""
        ...
        inactive = self.state_root / "inactive-runtime" / current.tool / current.job_id
        if inactive.exists():
    ...
    def _exportable_project_files(project: Job, destination: Path) -> list[Path]:
        ...
        excluded_roots = {
            project.root / ".sage" / "cache",
            project.root / ".sage" / "workspace-data",
    ...
    def export_run(
        ...
        """Create a deterministic project-labelled bundle for one bounded run."""
    ...
    def _base_raw(self) -> dict[str, Any]:
        """Load source settings plus operator-owned external resource mounts."""
        raw = merge_registered_projects(load_yaml(self.settings_path), self.sage_root)
    ...
    def _base_profile(self, tool: str, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        if not profile_path.is_absolute():
            profile_path = self.sage_root / profile_path
    ...
    def write_runtime_files(self, project: Job) -> Path:
        ...
        paths["sage_root"] = _relative_from(project.runtime_settings_path.parent, self.sage_root)
        paths["cache_root"] = (project.root / ".sage" / "cache").relative_to(self.sage_root).as_posix()
        paths["workspace_data_root"] = (
            project.root / ".sage" / "workspace-data"
        ).relative_to(self.sage_root).as_posix()
        raw["paths"] = paths
        ...
        projects_root_value = Path(str(paths.get("internal_scripture_root", paths.get("projects_root", "workspace-data/scripture-projects"))))
        projects_root = (
            projects_root_value
            if projects_root_value.is_absolute()
            else self.sage_root / projects_root_value
        )
        custom_default = str(
        ...
        for workflow_id in TOOL_IDS:
            ...
            if workflow_id == project.tool:
                entry["profile"] = project.runtime_profile_path.relative_to(self.sage_root).as_posix()
                entry["state_root"] = (project.root / ".sage" / "state").relative_to(self.sage_root).as_posix()
                entry["lock_root"] = (project.root / ".sage" / "locks").relative_to(self.sage_root).as_posix()
                entry["transaction_root"] = (
                    project.root / ".sage" / "transactions"
                ).relative_to(self.sage_root).as_posix()
                entry["output_root"] = project.root.relative_to(self.sage_root).as_posix()
                if workflow_id == "bic":
                    entry["memory_root"] = (
                        project.root / "memory"
                    ).relative_to(self.sage_root).as_posix()
                    entry["publication_root"] = (
                        project.root / "generations"
                    ).relative_to(self.sage_root).as_posix()
                else:
                ...
                entry["state_root"] = (inactive / "state").relative_to(self.sage_root).as_posix()
                entry["lock_root"] = (inactive / "locks").relative_to(self.sage_root).as_posix()
                entry["transaction_root"] = (
                    inactive / "transactions"
                ).relative_to(self.sage_root).as_posix()
                entry["output_root"] = (inactive / "output").relative_to(self.sage_root).as_posix()
                if workflow_id == "bic":
                    entry["memory_root"] = (inactive / "memory").relative_to(self.sage_root).as_posix()
                    entry["publication_root"] = (inactive / "generations").relative_to(self.sage_root).as_posix()
                else:
                    entry.pop("memory_root", None)
                    entry.pop("publication_root", None)
        ...
        # Reporting follows the translation Project that owns the human-facing output: BIC TARGET
        # or SAW WIP. Project overrides inherit from the global bilingual defaults when absent.
        reporting_project_id = (
            project.bindings.get("generated_target") if project.tool == "bic" else project.bindings.get("wip")
        )
        reporting_record = registered_project_records(self.sage_root).get(str(reporting_project_id), {})
        reporting_override = reporting_record.get("reporting") if isinstance(reporting_record, dict) else None
        if isinstance(reporting_override, dict):
            primary = str(reporting_override.get("primary_language") or "").strip()
            secondary = str(reporting_override.get("secondary_language") or "").strip()
            bilingual = bool(reporting_override.get("bilingual", True))
            if primary and secondary:
                ...
                # Challenge reports remain bilingual but put the translation language first.
                challenges.update({"primary_language": secondary, "secondary_language": primary, "bilingual": bilingual})
    ...
    def create_run(
        ...
        *,
        ...
        """Create one deterministic RC7 Run and make it current for the selected Job."""
        ...
        for relative in ("tasks", "plans", "decisions", "findings", "reports", "operator-note-text"):
            (root / relative).mkdir(parents=True, exist_ok=True)
        now = _utc_now()
        payload = {
        ...
        atomic_write_text(root / "run.yml", _safe_yaml(payload))
        atomic_write_json(root / "status.json", payload)
        atomic_write_json(
            project.root / ".sage" / "state" / "active-run.json",
            {
                "schema_version": "1.0",
                "tool": project.tool,
                "job_id": project.job_id,
    ...
    def update_run(self, run: Run, **changes: Any) -> Run:
        ...
        raw = load_yaml(run.manifest_path)
        allowed = {"status", "current_stage", "task_manifests", "plan_path", "focus", "check_type", "notes"}
        ...
        atomic_write_text(run.manifest_path, _safe_yaml(raw))
    ...
    def list_runs(self, project: Job, *, include_archived: bool = True) -> list[Run]:
        ...
        result: list[Run] = []
        for path in sorted((project.root / "runs").glob("*/run.yml"), reverse=True):
    ...
    def active_run(self, project: Job) -> Run | None:
        """Return the active Run for one Job."""
        path = project.root / ".sage" / "state" / "active-run.json"
        if not path.is_file():
            return None
        try:
