"""Operator-owned Scripture-project inventory for SAGE RC7."""
...
from .project_codes import DEFAULT_TYPE_CODES, parse_project_code
...
def project_inventory_path(root: Path) -> Path:
    ...
    return root.resolve() / "state" / "project-inventory.json"


def project_registry_path(root: Path) -> Path:
    """Compatibility alias for the RC7.04 SAGE Project Inventory path."""
...
def register_project(
    ...
    *,
    ...
    if not PROJECT_ID_RE.fullmatch(project_id):
        raise ValidationError("Paratext project code must be at most 8 cross-platform-safe characters", code="PROJECT_ID_INVALID")
...
def set_project_reporting_languages(
    root: Path,
    *,
    project_id: str,
    ...
    """Persist one Project's bilingual reporting override; UI language remains globally English."""
    from .language_codes import canonical_language_tag

    primary = canonical_language_tag(primary_language, "project reporting primary language")
    secondary = canonical_language_tag(secondary_language, "project reporting secondary language")
    if primary == secondary:
        raise ValidationError("Primary and secondary reporting languages must differ", code="PROJECT_REPORTING_LANGUAGE_INVALID")
...
def clear_project_reporting_languages(root: Path, *, project_id: str) -> dict[str, Any]:
    """Remove one Project override so Job runtime falls back to global bilingual defaults."""
    state = load_project_registry(root)
    current = state["projects"].get(project_id)
    if not isinstance(current, dict):
        raise ValidationError(f"Project is not in SAGE: {project_id}", code="PROJECT_NOT_IN_SAGE")
    record = dict(current)
    record.pop("reporting", None)
    state["projects"][project_id] = record
    write_project_registry(root, state)
    return record
