...
# These untrusted input synonyms are recognised but are never emitted as SAGE action vocabulary.
...
class CommandProposal:
    ...
    def canonical_command(self) -> str:
        ...
        return "./sage " + shlex.join(list(self.argv))
...
def _contains_phrase(text: str, phrase: str) -> bool:
    """Return whether a complete routed phrase occurs in the normalised request."""
...
def _intent_candidates(request: str) -> list[IntentCandidate]:
    ...
    add(
        ...
        operation="focused",
        group="saw",
        explanation="Create one bounded SAW Focused Check with an explicit question.",
        needs_scope=True,
        needs_projects=True,
        needs_focus=True,
    )
    ...
    add(
        ...
        operation="inspect",
        group="bic",
        explanation="Start the governed BIC sequence by analysing translation challenges without writing Scripture.",
        needs_scope=True,
        needs_projects=True,
    )
    add(
    ...
    init_score = _phrase_score(
        text,
        (("initialize workspace", 1.0), ("initialise workspace", 1.0), ("workspace initialize", 1.0)),
    )
    setup_score = _phrase_score(
        text,
        (("project init", 1.0), ("set up", 0.82), ("setup", 0.82), ("configure", 0.76), ("initialize", 0.70), ("initialise", 0.70)),
    )
    add(
        "workspace.initialize",
        "Initialise workspace resources",
        ("workspace", "initialize"),
        init_score,
        read_only=False,
        state_changing=True,
    ...
    submit_score = _phrase_score(text, (("submit task", 1.0), ("finalise task", 0.94), ("finalize task", 0.94)))
    add(
        "task.submit",
        ...
        state_changing=True,
        group="task",
        explanation="Validate and finalise one completed ACT task; the task-manifest path is required.",
    )
    ...
    add(
        ...
        state_changing=True,
        group="task",
        explanation="Aggregate finalised SAW work units through their governed partition plan.",
    )
...
def related_operations() -> list[dict[str, str]]:
    ...
    return [
        {"command_id": "bic.inspect", "label": "BIC INSPECT - analyse a bounded source scope"},
        ...
        {"command_id": "saw.focused", "label": "SAW Focused Check - answer one bounded question"},
...
def request_log_path(config: EcosystemConfig) -> Path:
    ...
    return config.workspace_data_root / "sage" / "audit" / "natural-language-requests.jsonl"
