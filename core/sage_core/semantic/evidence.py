...
def evidence_for_lemma(config: EcosystemConfig, *, language: str, lemma: str) -> dict[str, Any]:
    ...
    return {
        ...
        "authority_rule": "Local index retrieval supplies evidence only; contextual semantic judgment remains separate.",
    }
...
def scope_evidence_for_project(
    ...
    *,
    ...
    for form_key in sorted(token_counts):
        ...
        raw_senses = [
            value
            for value in senses.values()
            if isinstance(value, dict) and str(value.get("record_id", "")) in set(new_ids)
        ]
    ...
    return {
        ...
        "local_first": True,
        "authority_rule": "This packet is retrieval evidence only. SEMDOM does not authorise a translation choice.",
    }
...
def evidence_for_form(config: EcosystemConfig, *, language: str, form: str) -> dict[str, Any]:
    ...
    senses = [
        value
        for value in (sense_doc.get("senses", {}) or {}).values()
        if isinstance(value, dict) and str(value.get("record_id", "")) in set(record_ids)
    ]
    ...
    return {
        ...
        "authority_rule": "Lookup is reference evidence only; reviewed state and translation authority remain separate.",
    }
