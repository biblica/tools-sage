"""Immutable import adapters for RWC seeds, SIL SemDom, FLEx/Combine LIFT, and Greek reference data."""
...
def _write_import(
    ...
    *,
    ...
    """Persist one immutable normalised import with source provenance and hash."""
...
def import_greek_reference_xlsx(
    config: EcosystemConfig,
    path: Path,
    *,
    source_id: str,
    language: str = "grc",
    sheet: str = "Luke-keyterms",
) -> dict[str, Any]:
    """Import the supplied Greek Luke workbook as occurrence-form semantic reference data, not a lemma authority."""
    path = path.resolve()
    workbook = load_workbook(path, read_only=True, data_only=True)
    if sheet not in workbook.sheetnames:
        raise ValidationError(f"Greek reference workbook does not contain sheet {sheet!r}")
    worksheet = workbook[sheet]
    header = [str(value or "").strip() for value in next(worksheet.iter_rows(min_row=1, max_row=1, values_only=True))]
    expected = ["Greek word", "Meaning", "Key Term?", "Semantic domain", "Semantic domain name", "Reference(s)"]
    if header[:6] != expected:
        raise ValidationError(f"Unexpected Greek reference columns: {header[:6]!r}")
    grouped: dict[str, dict[str, Any]] = {}
    for row_number, row in enumerate(worksheet.iter_rows(min_row=2, values_only=True), start=2):
        form = str(row[0] or "").strip()
        if not form:
            continue
        meaning = str(row[1] or "").strip()
        code = str(row[3] or "").strip()
        label = str(row[4] or "").strip()
        if not re.fullmatch(r"\d+(?:\.\d+)*", code):
            raise ValidationError(f"Invalid Greek Semantic Domain code at row {row_number}: {code!r}")
        key = _stable_id(source_id, form, meaning, str(row[5] or ""))
        item = grouped.setdefault(
            key,
            {
                "record_id": key,
                "record_type": "BIBLICAL_GREEK_FORM_REFERENCE",
                "language": language,
                "headword": form,
                "lemma": None,
                "lemma_status": "SURFACE_FORM_ONLY",
                "surface_forms": [form],
                "key_term": False,
                "senses": [
                    {
                        "sense_id": _stable_id(source_id, form, meaning, "sense"),
                        "glosses": {"en": meaning} if meaning else {},
                        "definitions": {},
                        "semdom": [],
                        "part_of_speech": None,
                        "references": [],
                ...
                "provenance": {
                    "source_id": source_id,
                    "source_type": "GREEK_LUKE_SEMDOM_XLSX",
                    "rows": [],
                },
            },
        )
        item["key_term"] = bool(item["key_term"] or _bool_yes(row[2]))
        semdoms = item["senses"][0]["semdom"]
        if not any(entry["code"] == code for entry in semdoms):
            semdoms.append({"code": code, "label": label, "role": "REFERENCE"})
        references = str(row[5] or "").strip()
        if references and references not in item["senses"][0]["references"]:
            item["senses"][0]["references"].append(references)
        item["provenance"]["rows"].append(row_number)
    ...
    return _write_import(
        ...
        source_path=path,
        records=records,
        extra={"sheet": sheet, "status": "REFERENCE", "lemma_authority": False},
    )
...
def _child_text(parent: ET.Element, tag: str) -> str:
    """Return normalised text from one direct LIFT child."""
...
def import_semdom_authority_json(
    ...
    *,
    ...
    root = authority_root(config) / "sil-semdom" / safe_id(source_id, "source_id")
    root.mkdir(parents=True, exist_ok=True)
    ...
    ensure_authority_selected(config, authority_type="semdom", source_id=source_id)
...
def import_specific_first_docx(
    ...
    *,
    ...
    write_manifest(root / "manifest.json", manifest)
    ensure_authority_selected(config, authority_type="folders", source_id=source_id)
    return {**manifest, "authority_root": str(root)}
