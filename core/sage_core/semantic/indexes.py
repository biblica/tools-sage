...
def _load_imports(config: EcosystemConfig, language: str) -> tuple[list[dict[str, Any]], list[Path]]:
    """Load immutable normalised imports for one semantic language namespace."""
...
def _active_domain_catalog(config: EcosystemConfig) -> tuple[dict[str, dict[str, Any]], dict[str, str]]:
    ...
    semdom_source = active.get("sil_semdom")
    if semdom_source:
    ...
    folder_source = active.get("rapidwords_folders")
    if folder_source:
...
def build_semantic_indexes(config: EcosystemConfig, *, language: str) -> dict[str, Any]:
    ...
    coverage = {
        ...
        "notes": [
            ...
            "SURFACE_FORM_ONLY Greek reference records are excluded from lemma authority.",
        ],
    }
...
def semantic_status(config: EcosystemConfig, *, language: str) -> dict[str, Any]:
    ...
    semdom_sources = sorted(
        path.parent.name
        for path in (authority_root(config) / "sil-semdom").glob("*/manifest.json")
    ) if (authority_root(config) / "sil-semdom").is_dir() else []
    folder_sources = sorted(
        path.parent.name
        for path in (authority_root(config) / "rapidwords-folders").glob("*/manifest.json")
    ) if (authority_root(config) / "rapidwords-folders").is_dir() else []
    ...
    return {
        ...
        "available_authority": {"sil_semdom": semdom_sources, "rapidwords_folders": folder_sources},
        "root": str(root),
    }
