...
def configure_prompt_renderer(renderer: Callable[[str], str] | None) -> None:
    """Set the localised prompt-label renderer for the active CLI configuration."""
...
def normalize_value(value: str) -> str:
    """Normalise an Operator token for conservative fuzzy comparison."""
...
def suggestions_payload(items: Sequence[Suggestion]) -> list[dict[str, Any]]:
    """Serialise ranked correction suggestions for JSON and prompt consumers."""
...
def prompt_for_value(
    *,
    ...
    if received is not None:
        print(prompt_text("prompt.value_not_recognised", "{label} {received} was not recognised.").format(label=label, received=repr(received)), file=output_stream)
    else:
    ...
    if suggestions:
        ...
        for index, suggestion in enumerate(suggestions, start=1):
            ...
            print(f"  {index}. {detail} [{prompt_text('prompt.confidence', 'confidence')}={suggestion.confidence.lower()}]", file=output_stream)
        ...
        print(f"  {enter_number}. {prompt_text('prompt.enter_another_value', 'Enter another value')}", file=output_stream)
        if cancel_number is not None:
            print(f"  {cancel_number}. {prompt_text('prompt.cancel', 'Cancel')}", file=output_stream)
            ...
            if unrecognized:
                ...
                if not interactive or attempts >= max_attempts:
                    raise InputRequiredError(
                        ...
                        received=token,
                        suggestions=suggestions_payload(suggestions),
                        next_action="Use a recognised option and rerun the command.",
                    )
                corrected = prompt_for_value(
                    label="Command option",
                    received=token,
