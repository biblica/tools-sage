...
def ecosystem_state_path(workspace_data_root: Path) -> Path:
    """Return the ecosystem state document under the configured data root."""
    return workspace_data_root / "sage" / "state" / "ecosystem.json"


def ecosystem_event_path(workspace_data_root: Path) -> Path:
    """Return the ecosystem event log under the configured data root."""
    return workspace_data_root / "sage" / "transactions" / "events.jsonl"
...
def write_ecosystem_state(
    ...
    write_state(ecosystem_state_path(workspace_data_root), document)
    append_event(
        ecosystem_event_path(workspace_data_root),
        {
