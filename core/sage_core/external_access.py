...
def _inside(path: Path, root: Path) -> bool:
    """Return whether a resolved path remains inside one authorised root."""
    try:
...
def validate_external_file(
    ...
    *,
    ...
    if not any(_inside(resolved, root) for root in allowed_roots):
        raise ValidationError(f"External Scripture path escapes its authorised root: {candidate}", code="EXTERNAL_PATH_ESCAPE")
