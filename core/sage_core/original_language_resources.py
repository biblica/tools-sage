...
def ol_state_path(root: Path) -> Path:
    ...
    return root.expanduser().resolve() / "state" / "original-language-resources.json"
...
def bundled_ol_path(root: Path, resource_id: str) -> Path:
    ...
    return root.expanduser().resolve() / "resources" / "scripture" / "original-language" / OL_BUNDLED_SUBDIRS[rid]
...
def paratext_ol_candidates(catalogue: Mapping[str, Any], resource_id: str) -> tuple[dict[str, Any], ...]:
    """Return recognised ``grcSRCv#``/``hboSRCv#`` override candidates from the catalogue."""
    rid = resource_id.strip().upper()
