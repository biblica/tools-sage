"""Command-line interface for the SAGE v0.01-rc7.04 release candidate."""
...
def _guide_payload(topic: str) -> dict[str, Any]:
    ...
    launcher = "sage.cmd" if os.name == "nt" else "./sage"
    ...
    topics: dict[str, dict[str, Any]] = {
        ...
        "setup": {
            "title": "SAGE SETUP",
            "summary": "Setup is resumable and menu-driven rather than documentation-driven.",
            "steps": [
                f"Run `{launcher}` and choose Setup / system configuration when needed.",
                "SAGE checks Codex CLI, ChatGPT sign-in, workflows/projects, and project initialisation in sequence.",
                "Exit safely at any point; the next launch resumes from state/setup-state.json.",
            ],
        },
        "surfaces": {
            "title": "SAGE SURFACES",
        ...
        "recovery": {
            ...
            "steps": [
                f"Run `{launcher}` and choose Recovery & diagnostics.",
...
def command_overview(args: argparse.Namespace) -> int:
    ...
    workspace_state = read_state(ecosystem_state_path(config.workspace_data_root))
    ...
    if bool(getattr(args, "live", False)) and provider_ready is False:
        ...
        print("Live provider check: ./sage status --live")
    return 0


def command_setup(args: argparse.Namespace) -> int:
    """Run the short first-use setup directly without navigating through System."""
...
def _raise_init_input_required(requirements: dict[str, Any]) -> None:
    ...
    raise InputRequiredError(
        "INIT requires Operator input before validation or initialisation can continue",
        ...
        next_action="Run interactively or run `./sage project init` to review the proposed settings.",
    )


def _request_needs_runtime_configuration(args: argparse.Namespace) -> bool:
    """Return whether the selected command requires initialised runtime configuration."""
...
def _remediate_runtime_configuration(
    ...
    if not _interactive(args):
        ...
        raise InputRequiredError(
            ...
            suggestions=suggestions,
            next_action="Run interactively or run `./sage project init` and confirm the required settings.",
        )
...
def _initialization_is_stale(config: EcosystemConfig, state: dict[str, Any]) -> bool:
    """Return whether effective settings have changed since the last initialisation receipt."""
...
def _ensure_workspace_initialized_input(
    ...
    """Offer an immediate safe initialisation retry for analytical task creation."""
    ...
    if not _interactive(args):
        raise InputRequiredError(
            f"Workspace initialisation has {reason}",
            ...
            suggestions=[
                {
                    ...
                    "label": "Run guided workspace initialisation, then retry this task",
                    "score": 1.0,
                    "confidence": "AUTHORITATIVE",
                }
            ],
            next_action="Run `./sage workspace initialize` or repeat this command interactively.",
        )
    choice = prompt_for_value(
        label="Initialisation action",
        received="NOT_RUN" if missing else "STALE",
        suggestions=[
            Suggestion(
                value="initialize",
                label="Run guided workspace initialisation now",
                score=1.0,
                confidence="AUTHORITATIVE",
            ),
            Suggestion(
    ...
    if choice != "initialize":
        raise InputRequiredError(
            "Workspace initialisation remains required",
            code="WORKSPACE_INITIALIZATION_INPUT_REQUIRED",
            received=choice,
            suggestions=[],
            next_action="Run `./sage workspace initialize` before task creation.",
        )
    ...
    if result_code != 0:
        raise ValidationError(
            "Guided initialisation completed with in-scope blocking errors",
            code="WORKSPACE_INITIALIZATION_BLOCKED",
            next_action="Resolve the initialisation report errors and retry task creation.",
        )
    return load_ecosystem(_settings_path(args.settings))
...
def _resolve_path_input(
    ...
    """Resolve a governed input path and keep it inside the authorised root."""
    value = getattr(args, field, None)
    if value in (None, ""):
        return
    raw = str(value)
    ...
    for pattern in patterns:
        candidates.update(item.resolve() for item in config.workspace_data_root.glob(pattern) if item.is_file())
    ...
    setattr(args, field, str(corrected_path.resolve()))
    _record_correction(
        args,
        {
            "field": field,
            "original": raw,
            "resolved": str(corrected_path.resolve()),
            "resolution": "OPERATOR_CONFIRMED",
        },
    )
...
def _prepare_runtime_inputs(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "command", None) in {"guide", "help", "menu"}:
        return
...
def _print_corrections(args: argparse.Namespace) -> None:
    ...
    if canonical:
        print(f"Canonical command: sage {shlex.join(canonical)}", file=sys.stderr)
...
def _workflow_output_path(value: str | None, output_root: Path, default_name: str) -> Path:
    """Resolve a generated artefact inside its owning workflow output root."""
...
def _render_initialization_report(config: EcosystemConfig, result: dict[str, Any]) -> str:
    """Render the ecosystem initialisation result in configured report languages."""
...
def _render_project_init_report(config: EcosystemConfig, result: dict[str, Any]) -> str:
    """Render project INIT results in the independently configured report languages."""
    yes = _report_text(config, "label.yes")
    no = _report_text(config, "label.no")
    none = _report_text(config, "label.none")
    lines = [
...
def command_init(args: argparse.Namespace) -> int:
    ...
    result = {
        ...
        "rules": [
            ...
            "Every corrected or inferred setting is revalidated before initialisation continues.",
    ...
    output_root = config.workspace_data_root / "sage" / "init"
...
def command_model_refresh(args: argparse.Namespace) -> int:
    """Refresh the live model catalogue exposed to the current provider account."""
    config, _ = _load(args)
    ...
    if result.get("catalog_cache"):
        print(f"Catalogue cache: {result['catalog_cache']}")
    return 0
    ...
    if args.json:
        ...
        if result.get("status") == "PARTITIONED":
            ...
            if result["workflow"] == "bic":
                ...
                print(f"REFERENCE: {result['contemporary_source']} (authorised benchmark)")
...
def command_act_submit(args: argparse.Namespace) -> int:
    ...
    if not task_path.is_absolute():
        task_path = config.root / task_path
...
def command_resource_list(args: argparse.Namespace) -> int:
    ...
    result = {
        "status": "READY",
        "mounts_path": str(config.root / "state" / "resource-mounts.json"),
...
def command_resource_vrs_root(args: argparse.Namespace) -> int:
    ...
    if bool(args.clear):
        ...
        destination = config.root / "state" / "resource-mounts.json"
...
def command_evaluation_plan(args: argparse.Namespace) -> int:
    ...
    for index, entry in enumerate(evaluation.entries, start=1):
        ...
        command = (
            f'./sage --settings {config.settings_path.name} task create --workflow saw '
            f'--operation {args.operation} --wip {entry.output_project} '
            f'--reference {entry.contemporary_source} --scope "{scope.label()}"'
        )
        if focus:
            command += f' --focus "{focus}"'
        if getattr(args, "check_type", None):
            command += f' --type {args.check_type}'
        command_windows = command.replace("./sage ", "sage.cmd ", 1)
        entries.append(
            {
                "sequence": index,
    ...
    if args.json:
        ...
        for item in entries:
            print(f"{item['sequence']}. macOS/Linux: {item['commands']['posix']}")
...
def command_initialize(args: argparse.Namespace) -> int:
    ...
    if needs_input and _interactive(args):
        ...
        if init_remediation.get("changed"):
            config, standard = _load(args)
    lock_path = config.workspace_data_root / "sage" / "locks" / "initialize.lock"
    with WorkspaceLock(lock_path, "SAGE_INITIALIZE", break_stale=args.break_stale_lock):
        ...
        if not config.configured:
            errors.append(
                "ecosystem.configured is false; configure project paths before operational initialisation"
            )
        if not errors:
            for project_id, project in config.projects.items():
                try:
            ...
            for workflow_id, item in workflows.items():
                ...
                for role, project_status in binding_statuses.items():
                    ...
                    if not acceptable:
                        blocked_roles.append(f"{role}={project_status}")
                resource_state = "BLOCKED" if blocked_roles else "READY"
                controller_available = (
                    item["qualification_status"] == "VALIDATED"
                    and item["pending_transactions"] == 0
                )
                default_execution_available = controller_available and resource_state == "READY"
                ...
                write_state(
                    config.workflows[workflow_id].state_root / "workflow.json",
                    {
                        ...
                        "state": "READY" if resource_state == "READY" else "BLOCKED",
        ...
        if errors:
            next_action = (
                "Correct the listed ecosystem configuration errors, then rerun `./sage workspace initialize`."
            )
        elif restrictions:
            next_action = (
        ...
        write_ecosystem_state(config.workspace_data_root, standard, state, payload)
        result = {"state": state, **payload}
        report_root = config.workspace_data_root / "sage" / "output"
    ...
    if args.json:
        ...
        print("SAGE INITIALISATION RESULT")
        print(f"State: {state}")
...
def command_status(args: argparse.Namespace) -> int:
    ...
    if not state:
        state = {
            "state": "NOT_RUN",
            "version": standard.version,
            "next_action": "Run `./sage workspace initialize`.",
        }
    if args.json:
        _print_json(state)
    else:
...
def command_workflow_status(args: argparse.Namespace) -> int:
    ...
    ecosystem_state = read_state(ecosystem_state_path(config.workspace_data_root))
...
def command_plan(args: argparse.Namespace) -> int:
    ...
    if workflow.publication_root is not None:
        try:
            ...
            raise ValidationError(
                "Planning artefacts may not be written inside the immutable publication root"
            )
    ...
    if args.recover:
        ...
        with WorkspaceLock(lock_path, "TRANSACTION_RECOVERY"):
            result = recover_transaction(
                transaction_path,
                mode="rollback",
                allowed_roots=(config.workspace_data_root / args.workflow_id,),
            )
    else:
        pending = incomplete_transactions(workflow.transaction_root)
        result = {
    ...
    if isinstance(proposals, list) and proposals:
        ...
        for index, proposal in enumerate(proposals, start=1):
            if not isinstance(proposal, dict):
                continue
            print(
                f"{index}. {proposal.get('title')} "
                f"[{str(proposal.get('confidence', 'LOW')).lower()} confidence]"
            )
...
def _edited_request_argv(value: str) -> list[str]:
    ...
    if tokens and tokens[0] in {"sage", "./sage"}:
        tokens = tokens[1:]
    if not tokens:
        raise InputRequiredError(
    ...
    while True:
        ...
        if executable:
            ...
            if selection == 3:
                ...
                edited_command = "./sage " + shlex.join(edited_argv)
...
def _add_task_create_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help="BIC SOURCE content authority or SAW authorised REFERENCE",
    )
    parser.add_argument(
        "--lexical-donor", "--donor",
        dest="lexical_donor",
    ...
    parser.add_argument(
        ...
        help="Optional SAW Focused Check type; defaults to CUSTOM_BOUNDED_CHECK",
    )
    parser.add_argument(
        "--predecessor-task",
...
def build_parser(*, include_internal: bool = False) -> GuidedArgumentParser:
    ...
    parser = GuidedArgumentParser(
        prog="sage",
        description="SAGE Scripture Analysis and Generation Ecosystem",
        epilog=(
    ...
    workspace = subparsers.add_parser("workspace", help="Review, validate, initialise, and inspect the workspace")
    ...
    model_provision = model_actions.add_parser("provision", help="Configure a disabled local provider without enabling execution")
    model_provision.add_argument("--provider", choices=("ollama", "lmstudio"), required=True)
    ...
    task_submit = task_actions.add_parser("submit", help="Validate and finalise one completed ACT task")
    ...
    transaction = subparsers.add_parser("transaction", help="List or recover journalled workflow transactions")
    ...
    workflow_plan = workflow_actions.add_parser("plan", help="Build bounded section-led work units without analysis")
...
def _operational_logger(args: argparse.Namespace | None, *, error_stream: bool = False) -> OperationalLogger | None:
    ...
    if args is not None and getattr(args, "command", None) in {"guide", "help", "menu", "status"}:
        return None
    if (
        args is not None
...
def _configure_console_streams(args: argparse.Namespace) -> None:
    """Localise recognised non-JSON report lines through the configured log/report channel."""
    if getattr(args, "command", None) in {"guide", "help", "menu"}:
        return
...
def main() -> None:
    ...
    try:
        ...
        parser = build_parser(include_internal="shortcut" in sys.argv[1:])
