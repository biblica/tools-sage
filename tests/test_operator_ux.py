...
def test_blocking_menu_action_has_visible_status_and_clears_line(make_workspace) -> None:
    ...
    assert "\rPreparing governed SAW task plan... |" in rendered
...
def test_operator_cue_journal_is_append_only_and_not_workflow_state(make_workspace) -> None:
    ...
    assert not (root / "state" / "transaction-journal.jsonl").exists()
...
def test_saw_composite_creation_reports_plan_without_requiring_act_path(
    ...
    def fake_continue(_job, current):
        """Capture automatic plan continuation after the operator authorises the Run."""
    ...
    assert "Created SAW Normal QA composite plan" in center.io.output.getvalue()
    assert "(2 tasks)" in center.io.output.getvalue()
...
def test_unexpected_run_continuation_error_is_bounded_at_menu_boundary(
    ...
    rendered = center.io.output.getvalue()
    assert "SAGE RUN ERROR" in rendered
...
def test_saw_plan_continuation_advances_all_submitted_units_without_menu_round_trips(
    ...
    """One authorised Continue action advances every ready unit until the plan completes."""
    root = make_workspace(configured=True, qualification_status="VALIDATED")
    store = JobStore(root, root / "ecosystem.yml")
    job = next(item for item in store.bootstrap_default_jobs() if item.tool == "saw")
    run = store.create_run(job, operation="qa", scope="MAT 1")
...
def test_rejected_saw_submission_points_directly_to_preserving_reset(make_workspace, monkeypatch) -> None:
    """A sealed malformed result gives the operator the shortest recoverable menu path."""
    root = make_workspace(configured=True, qualification_status="VALIDATED")
    store = JobStore(root, root / "ecosystem.yml")
    job = next(item for item in store.bootstrap_default_jobs() if item.tool == "saw")
    run = store.create_run(job, operation="qa", scope="MAT 1")
    ...
    assert "SAW Job menu" in str(caught.value.next_action)
    assert "preserved" in str(caught.value.next_action)
...
def test_restart_run_preserves_old_outputs_and_recreates_operator_request(make_workspace) -> None:
    ...
    retained = run.root / "reports" / "retained.txt"
