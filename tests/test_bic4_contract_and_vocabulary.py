...
from sage_core.act_tasks import ACT_OPERATIONS, create_act_task, submit_act_task
from sage_core.bic_memory import record_human_memory_review
from sage_core.runtime_paths import workflow_memory_root
from sage_core.cli import ALLOWED_OPERATIONS, SHORTCUT_COMMANDS, build_parser
from sage_core.guided_input import ArgumentChoiceProblem
from sage_core.hashing import sha256_file
from sage_core.natural_language import interpret_request
from sage_core.registry import load_ecosystem
from sage_core.jobs import JobStore
from sage_core.vocabulary import (
...
EXPECTED_BIC4_CONTRACT_SHA256 = "be82c2d71e0dd0c52096099c5b2b0dfef252785960abe5addcc9c07c135c791d"
...
def _initialise(package_root: Path, workspace: Path) -> None:
    """Initialise one disposable workspace before creating governed ACT tasks."""
...
def _current_operational_files(root: Path):
    ...
    for path in sorted(root.rglob("*")):
        ...
        if rel in exempt_files:
            continue
        if parts and parts[0] in {"projects", "tests", "cache", "workspace-data"}:
            continue
...
def test_bic_verb_selection_policy_is_hash_pinned_independently() -> None:
    ...
    policy_path = ROOT / "meta" / "contracts" / "BIC-VERB-SELECTION-POLICY.yml"
    pin_path = ROOT / "meta" / "bic-protected-verb-selection-contract.yml"
    pin = yaml.safe_load(pin_path.read_text(encoding="utf-8"))["contract"]
    ...
    assert pin["canonical_file"] == "meta/contracts/BIC-VERB-SELECTION-POLICY.yml"
...
def test_complete_bic4_contract_is_hash_pinned_without_abbreviation() -> None:
    ...
    rewrite_path = ROOT / "skills" / "bic-rewrite" / "references" / "PROTECTED-REWRITE-DETAIL-RULES.md"
    self_check_path = ROOT / "skills" / "bic-self-check" / "references" / "PROTECTED-REWRITE-DETAIL-RULES.md"
    ...
    pin = yaml.safe_load((ROOT / "meta" / "bic-protected-rewrite-contract.yml").read_text(encoding="utf-8"))["contract"]
...
def test_review_and_flow_have_distinct_project_grammar_meanings() -> None:
    """Verify that project grammar separates assessment language from sequence language."""
    grammar = (ROOT / "docs" / "PROJECT-DOCUMENT-GRAMMAR.md").read_text(encoding="utf-8")
...
def test_emitted_operational_surfaces_exclude_prohibited_target_text_verbs() -> None:
    ...
    assert failures == []
    bic_text = (ROOT / "skills" / "bic-inspect" / "SKILL.md").read_text(encoding="utf-8")
...
def test_generated_rewrite_and_self_check_acts_enforce_contract_and_vocabulary(
    ...
    routed_rewrite_contract = [
        item for item in rewrite["allowed_reads"]
        if item["path"].endswith("PROTECTED-REWRITE-DETAIL-RULES.md")
    ]
    assert routed_rewrite_contract == [
        {
            "path": "skills/bic-rewrite/references/PROTECTED-REWRITE-DETAIL-RULES.md",
    ...
    rewrite_output.write_text(
        (rewrite_manifest.parent / "packet" / "source.usfm").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
    ...
    routed_self_contract = [
        item for item in self_check["allowed_reads"]
        if item["path"].endswith("PROTECTED-REWRITE-DETAIL-RULES.md")
    ]
    assert routed_self_contract == [
        {
            "path": "skills/bic-self-check/references/PROTECTED-REWRITE-DETAIL-RULES.md",
            "sha256": EXPECTED_BIC4_CONTRACT_SHA256,
        }
    ]
