...
from sage_core.rewrite_risk import (
...
def _initialised_rewrite(package_root: Path, make_workspace) -> tuple[object, dict, Path]:
    ...
    output.write_text(
        (manifest.parent / "packet" / "source.usfm").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
...
def _challenge_document(task: dict, output: Path, *, resolved: bool) -> dict:
    ...
    if "human_output" in task:
        document["reporting_languages"] = ["id", "en"]
    return document
...
def test_unresolved_critical_choice_completes_without_operator_input(
    ...
    assert any(path.endswith("original-language.usfm") for path in paths)
    ...
    inherited_usfm = self_root / "packet" / "original-language.usfm"
    inherited_vrs = self_root / "packet" / "inherited-ol-vrs-evidence.json"
    assert sha256_file(inherited_usfm) == next(
        digest for path, digest in conditional.items() if path.endswith("original-language.usfm")
    )
...
def test_generated_rewrite_act_and_skill_share_current_risk_contract(
    ...
    skill_text = (package_root / "skills/bic-rewrite/SKILL.md").read_text(encoding="utf-8")
    ...
    challenge_schema = yaml.safe_load(
        (package_root / "meta/schemas/bic-translation-challenges.schema.yml").read_text(encoding="utf-8")
    )
    policy = yaml.safe_load(
        (package_root / "meta/contracts/BIC-VERB-SELECTION-POLICY.yml").read_text(encoding="utf-8")
    )["policy"]
    ...
    assert not (package_root / "meta/schemas/bic-operator-decisions.schema.yml").exists()
...
def test_bic_ol_evidence_is_conditional_not_an_ordinary_read(
    ...
    assert any(path.endswith("original-language.usfm") for path in conditional)
