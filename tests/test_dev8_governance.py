...
from sage_core.errors import InputRequiredError, ConfigurationError, MemoryGovernanceError, ValidationError
...
def initialize(package_root: Path, root: Path) -> None:
    """Initialise the isolated test workspace and return its result."""
...
def test_act_creation_requires_initialization(make_workspace):
    """Verify that ACT creation requires prior initialisation."""
...
def test_job_runtime_initialization_authorizes_act_without_root_state(package_root, make_workspace):
    ...
    root_state = root / "workspace-data" / "sage" / "state" / "ecosystem.json"
    job_state = job.root / ".sage" / "workspace-data" / "sage" / "state" / "ecosystem.json"
...
def test_act_task_routes_ol_only_for_ol_operation(package_root, make_workspace):
    ...
    assert not (Path(qa["manifest_path"]).parent / "packet" / "original-language.usfm").exists()
    packet = Path(qa["manifest_path"]).parent / "packet" / "reference.usfm"
    ...
    assert (Path(ol["manifest_path"]).parent / "packet" / "original-language.usfm").is_file()
...
def test_blocked_project_cannot_create_task(package_root, make_workspace):
    ...
    root = make_workspace(qualification_status="VALIDATED")
    initialize(package_root, root)
    source = root / "projects" / "usNIVv2" / "41MAT.SFM"
...
def test_bic_rewrite_and_self_check_are_separate_and_commit(package_root, make_workspace):
    ...
    rewrite_manifest = Path(rewrite["manifest_path"])
    packet = rewrite_manifest.parent / "packet" / "source.usfm"
    rewrite_output = rewrite_manifest.parent / "output" / "rewrite.usfm"
    rewrite_output.write_text(packet.read_text(encoding="utf-8"), encoding="utf-8")
    ...
    assert not any(item["path"].endswith("original-language.usfm") for item in self_check["allowed_reads"])
    assert not (self_manifest.parent / "packet" / "original-language.usfm").exists()
    assert not (self_manifest.parent / "packet" / "inherited-ol-vrs-evidence.json").exists()
    staged = self_manifest.parent / "packet" / "staged-target.usfm"
    self_output = self_manifest.parent / "output" / "self-check.usfm"
    self_output.write_text(staged.read_text(encoding="utf-8"), encoding="utf-8")
...
def test_evaluation_queue_is_sequential_and_has_platform_commands(package_root, make_workspace):
    ...
    assert commands["posix"].startswith("./sage ")
    assert commands["windows"].startswith("sage.cmd ")
...
def test_settings_drift_is_rejected_at_submission(package_root, make_workspace):
    ...
    assert "Reset / restart active Run" in str(caught.value.next_action)
