...
from sage_core.act_outputs import render_operator_note_text
...
def test_workflow_profile_schema_is_executable(make_workspace) -> None:
    ...
    root = make_workspace(qualification_status="VALIDATED")
    path = root / "workflows/saw/profile.yml"
    data = yaml.safe_load(path.read_text(encoding="utf-8"))
...
def test_direct_task_always_persists_job_and_run(package_root: Path, make_workspace) -> None:
    ...
    assert (job.root / "runs" / manifest["run_id"] / "run.yml").is_file()
...
def test_normal_qa_starts_as_composite_meaning_stage_without_ol(package_root: Path, make_workspace) -> None:
    """Create one composite Normal QA plan whose first model stage has no OL evidence."""
    root = make_workspace(qualification_status="VALIDATED")
    _initialize(package_root, root)
    config = load_ecosystem(root / "ecosystem.yml")
    ...
    if manifest["qa_stage"] in {"STRUCTURAL_ADJUDICATION", "TRANSLATION_AND_MEANING_QA"}:
        ...
        assert not any(str(row["path"]).endswith("original-language.usfm") for row in manifest["allowed_reads"])
...
def test_operator_note_text_is_plain_copy_paste_material() -> None:
    ...
    assert "ISSUE:" in text and "RECOMMENDATION:" in text
...
def test_live_skill_contracts_have_no_stale_authority_terms(package_root: Path) -> None:
    ...
    files = []
    for root in (package_root / "skills").iterdir():
        if not root.is_dir():
            continue
        skill = root / "SKILL.md"
        if skill.is_file():
    ...
    assert "reviewed target" not in text.casefold()
    inspect_text = "\n".join((package_root / "skills/bic-inspect" / rel).read_text(encoding="utf-8") for rel in [Path("SKILL.md"), Path("references/INSPECT-CONTRACT.md"), Path("references/SOURCE-AUTHORITY-AND-USFM.md")])
...
def test_composite_qa_meaning_to_selective_ol_to_final_text(package_root: Path, make_workspace) -> None:
    """Carry one Normal QA Run through bounded meaning -> OL delta -> deterministic finalisation."""
    root = make_workspace(qualification_status="VALIDATED")
    _initialize(package_root, root)
    config = load_ecosystem(root / "ecosystem.yml")
    result = create_act_task(
    ...
    reports_root = Path(result["plan_path"]).parent.parent.parent.parent / "reports" / "MAT"
    reports_root.mkdir(parents=True, exist_ok=True)
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    (reports_root / f"MAT-001-001_{today}_001_EXISTING.txt").write_text("reserved\n", encoding="utf-8")
    ...
    assert report_path.name == f"MAT-001-001_{today}_002_ACTION-REPORT.md"
    note_path = Path(final["operator_note_text_path"])
    assert note_path.is_file()
    assert note_path.parent == reports_root
    assert note_path.name == f"MAT-001-001_{today}_002_OPERATOR-NOTE.txt"
    ...
    assert f"Task: `{legacy_plan['plan_id']}`" in Path(repaired["report_path"]).read_text(
        encoding="utf-8"
    )
