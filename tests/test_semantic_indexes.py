...
from sage_core.semantic.diagnostics import saw_signals_from_scope_evidence
from sage_core.semantic.evidence import scope_evidence_for_project
from sage_core.semantic.indexes import semantic_status
from sage_core.semantic.importers import (
...
from sage_core.semantic.indexes import build_semantic_indexes
from sage_core.semantic.lift import export_lift
from sage_core.semantic.store import load_import_selection, set_binding, set_import_active, set_review_state
...
def _greek_workbook(path: Path) -> None:
    """Create a compact Greek form-level reference workbook."""
    book = Workbook()
    sheet = book.active
    sheet.title = "Luke-keyterms"
    sheet.append(
        [
            "Greek word",
            "Meaning",
    ...
    sheet.append(["ἔδωκεν", "gave", "Yes", "7.4", "Give", "LUK 1:1"])
    sheet.append(["ἔδωκεν", "gave", "Yes", "4.3", "Exchange", "LUK 1:1"])
    book.save(path)
...
def test_greek_reference_never_promotes_surface_forms_to_lemmas(make_workspace, tmp_path: Path) -> None:
    """Verify the supplied Greek-style reference remains form-level evidence after indexing."""
    root = make_workspace(qualification_status="VALIDATED")
    config = load_ecosystem(root / "ecosystem.yml")
    source = tmp_path / "Greek-Luke.xlsx"
    _greek_workbook(source)
    result = import_greek_reference_xlsx(config, source, source_id="Greek-Luke-KeyTerms")
    assert result["record_count"] == 1
    coverage = build_semantic_indexes(config, language="grc")
    assert coverage["lemmas"] == 0
    assert coverage["senses"] == 1
    records_path = Path(result["import_root"]) / "records.json"
    record = json.loads(records_path.read_text(encoding="utf-8"))[0]
    assert record["lemma"] is None
    assert record["lemma_status"] == "SURFACE_FORM_ONLY"
    assert len(record["senses"][0]["semdom"]) == 2
...
def test_lift_import_cannot_grant_approval_and_review_state_is_separate(make_workspace, tmp_path: Path) -> None:
    ...
    build_semantic_indexes(config, language="KKH")
    sense_doc = json.loads((root / "workspace-data" / "sage" / "semantic" / "languages" / "KKH" / "indexes" / "sense-semdom.json").read_text(encoding="utf-8"))
...
def test_review_changes_can_be_batched_without_rebuilding_between_senses(make_workspace, tmp_path: Path) -> None:
    ...
    sense_doc = json.loads(
        (root / "workspace-data" / "sage" / "semantic" / "languages" / "KKH" / "indexes" / "sense-semdom.json").read_text(encoding="utf-8")
    )
    ...
    from sage_core.semantic.evidence import evidence_for_form
