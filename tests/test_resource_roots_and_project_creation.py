"""Cross-platform path normalisation, reusable project roots, and in-context resource registration."""
...
from sage_core.project_inventory import register_project
from sage_core.resource_mounts import (
...
def test_operator_path_normalisation_accepts_shell_paste_forms() -> None:
    """Verify that quoted and escaped Unix path pastes normalise without shell evaluation."""
...
def test_root_relative_mount_persists_root_and_subfolder(make_workspace, tmp_path: Path) -> None:
    ...
    persisted = json.loads((root / "state" / "resource-mounts.json").read_text(encoding="utf-8"))
...
def test_saw_job_creation_can_add_new_detected_wip(make_workspace, tmp_path: Path) -> None:
    ...
    register_project(
        root,
        project_id="usNIVv2",
        project_path=root / "projects" / "usNIVv2",
        language_code="en",
    ...
    responses = [
        "A",  # WIP: temporarily enter Add Projects to SAGE
        "1",  # discovered newWIP
    ...
    inventory = json.loads((root / "state" / "project-inventory.json").read_text(encoding="utf-8"))
...
def test_release_package_starts_with_empty_scripture_inventory(package_root: Path) -> None:
    """Verify the shipped RC configuration contains no pre-registered Scripture project data."""
    import yaml
    from sage_core.resource_validation import validate_scripture_resources

    raw = yaml.safe_load((package_root / "ecosystem.yml").read_text(encoding="utf-8"))
    assert raw["projects"] == {}
    assert not (package_root / "state" / "project-inventory.json").exists()
...
def test_original_language_resources_are_not_ordinary_project_registration_roles(make_workspace, tmp_path: Path) -> None:
    ...
    from sage_core.resource_registration import compatible_language_options
