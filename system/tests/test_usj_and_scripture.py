...
from sage.usj import compile_usfm_file, compile_usfm_text, parse_usj_units
...
def test_note_fields_preserve_nested_styles_and_separate_punctuation_streams() -> None:
    """Footnote punctuation and nested styles remain separate from surrounding body punctuation."""
    usj = compile_usfm_text(
        "\\id 1PE Fixture\n\\c 1\n\\p\n"
        "\\v 1 Peter says \"some text\\f + \\fr 1:1 \\ft footnote \\+it emphasis\\+it*.\\f*.\" New story.\n"
    )
    assert usj["sage"]["errors"] == []
    record = usj["sage"]["verse_records"][0]
    assert record["body_text_exact"] == 'Peter says "some text." New story.'
    note = next(node for node in record["content"] if isinstance(node, dict) and node.get("type") == "note")
    fields = [node for node in note["content"] if isinstance(node, dict) and node.get("type") == "char"]
    assert [field["marker"] for field in fields] == ["fr", "ft"]
    nested = next(node for node in fields[1]["content"] if isinstance(node, dict))
    assert nested == {"type": "char", "marker": "it", "content": ["emphasis"]}
    assert fields[1]["content"][-1] == "."


def test_note_structural_fields_accept_explicit_usfm_31_closures() -> None:
    """USFM 3.1 allows explicit closing markers for note structural fields."""
    usj = compile_usfm_text(
        "\\id RUT Fixture\n\\c 1\n\\p\n"
        "\\v 1 Text.\\f + \\fr 1:1 \\fr*\\ft Note text.\\ft*\\f*\n"
    )
    assert usj["sage"]["errors"] == []
    unit = parse_usj_units(usj)[0]
    assert unit["body_text_exact"] == "Text."
    note = next(node for node in unit["content"] if isinstance(node, dict) and node.get("type") == "note")
    fields = [node for node in note["content"] if isinstance(node, dict) and node.get("type") == "char"]
    assert [field["marker"] for field in fields] == ["fr", "ft"]
    assert fields[0]["content"] == ["1:1 "]
    assert fields[1]["content"] == ["Note text."]


def test_verse_parser_error_is_reported_only_with_its_scripture_coordinate() -> None:
    """A verse parser failure must not also survive as a book-wide duplicate."""
    usj = compile_usfm_text(
        "\\id RUT Fixture\n\\c 1\n\\p\n"
        "\\v 1 Good.\n"
        "\\v 17 Text.\\f + \\fr 1:17 \\ft*\\f*\n"
    )
    errors = usj["sage"]["errors"]
    assert "RUT 1:17:UNEXPECTED_NOTE_CLOSING_MARKER:ft" in errors
    assert "UNEXPECTED_NOTE_CLOSING_MARKER:ft" not in errors


def test_mismatched_explicit_note_field_closure_remains_an_error() -> None:
    """Explicit note-field closure must still match the currently active structural field."""
    usj = compile_usfm_text(
        "\\id RUT Fixture\n\\c 1\n\\p\n"
        "\\v 1 Text.\\f + \\fr 1:1 \\ft*\\f*\n"
    )
    assert "RUT 1:1:UNEXPECTED_NOTE_CLOSING_MARKER:ft" in usj["sage"]["errors"]
...
def test_excluded_coordinate_absence_is_not_missing(make_workspace) -> None:
    ...
    project_root = storage_layout(root).projects_root / "usNIVv2"
...
def test_excluded_coordinate_present_is_versification_advisory(make_workspace) -> None:
    """A VRS coordinate disagreement is reported but does not block Scripture processing."""
    root = make_workspace(verse_max=3)
    project_root = storage_layout(root).projects_root / "usNIVv2"
    (project_root / "custom.vrs").write_text("#! -MAT 1:2\n", encoding="utf-8")
    config = load_ecosystem(root / "ecosystem.yml")
    ...
    assert not any(item["code"] == "EXCLUDED_COORDINATE_PRESENT" for item in result["issues"])
    assert any(item["code"] == "EXCLUDED_COORDINATE_PRESENT" for item in result["warnings"])
