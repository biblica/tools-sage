...
def isolated_sage_data_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Keep every test's mutable SAGEdata outside both the real Core and other tests."""
    monkeypatch.setenv("SAGE_DATA_HOME", str(tmp_path / "SAGEdata"))
...
def make_workspace(tmp_path: Path):
    ...
    def factory(
        *,
        ...
        data_root = root.parent / "SAGEdata"
