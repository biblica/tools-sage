...
- [ ] Keep all `SAGEdata/` content, top-level `jobs/`/`reports/`/`workspace_data/` legacy roots,
...
- [ ] Stage the governed immutable `ecosystem.yml`; mutable workstation/operator policy belongs only
  in `SAGEdata/.system/config/` and must never be copied into Core.
...
- [ ] Confirm no top-level `jobs/`, `reports/`, `workspace_data/`, or `SAGEdata/` directory exists in
  the staged Core tree.
- [ ] Confirm no host-specific managed runtime exists in Core; first launch must create
  `SAGEdata/.system/runtime/venv` from the governed pinned manifests.
...
- [ ] Confirm no local runtime data, `SAGEdata`, caches, `.venv`, receipts, or release artifacts entered the ZIP.
