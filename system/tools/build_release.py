...
PACKAGE_NAME = f"SAGE-v{(ROOT / 'VERSION').read_text(encoding='utf-8').strip()}-Full-Distribution"
...
def _run_hardening_gate(stage: Path) -> tuple[bool, dict[str, object] | str]:
    """Run deterministic hardening shards and the formal combine gate on this exact stage."""
    ...
    with tempfile.TemporaryDirectory(prefix="sage-release-hardening-report-") as td:
        ...
        for shard_index in range(shard_count):
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    str(stage / "system" / "tools" / "hardening.py"),
...
def _audit_stage(stage: Path) -> tuple[bool, str]:
    ...
    env["SAGE_DISABLE_OPERATIONAL_LOG"] = "1"
    result = subprocess.run(
        [sys.executable, str(stage / "system" / "tools" / "deep_audit.py"), str(stage), "--mode", "source"],
        cwd=stage,
        env=env,
        text=True,
        capture_output=True,
