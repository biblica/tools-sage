...
def _venv_python(root: Path) -> Path:
    """Return the platform-native Python path in SAGEdata's managed runtime."""
...
def _create_venv(root: Path) -> bool:
    ...
    _write("Creating SAGE Python environment (SAGEdata/.system/runtime/venv)...")
...
def _write_runtime_state(
    ...
    *,
    ...
    payload = {
        ...
        "status": "READY",
        "python_environment": "SAGEdata/.system/runtime/venv",
...
def ensure_runtime(root: Path, *, profile: str = "base") -> int:
    ...
    if not venv_python.is_file() or not _python_is_supported(venv_python):
        ...
        if venv_python.exists():
            _write("The managed SAGEdata Python environment is incomplete or uses an unsupported Python version; rebuilding it deterministically.")
        else:
            _write("The managed SAGE Python environment has not been created; creating it deterministically.")
        if not _create_venv(root):
            return _blocked("Could not create the managed SAGEdata Python environment with the available Python interpreter.")
        created_now = True
        venv_python = _venv_python(root)
        if not venv_python.is_file() or not _python_is_supported(venv_python):
    ...
    if not _pip_is_available(venv_python):
        ...
        if not _ensure_pip(venv_python):
            return _blocked("pip is unavailable inside the managed SAGEdata Python environment and repair failed.")
    ...
    _write(f"SAGEdata root: {layout.data_root}")
    ...
    if release_receipt.get("status") != "NOT_APPLICABLE":
        ...
        if release_receipt.get("version_changed"):
            _write("Pre-release version changed: persistent SAGEdata PRESERVED")
        else:
