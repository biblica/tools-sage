...
def _job_reports_root(plan_path: Path) -> Path:
    ...
    Canonical Job paths live below ``SAGEdata/jobs/<tool>/<job>``. Reports are a
    sibling Operator-owned collection below the same data root, so this resolution
    ...
    data_root = job_root.parent.parent.parent
    ...
    *,
    ...
    for candidate in sorted((job_root / "runs").glob("*/plans/*.json")):
        ...
        if not raw_path.is_absolute():
            raw_path = (job_root.parent.parent.parent / raw_path).resolve()
        else:
