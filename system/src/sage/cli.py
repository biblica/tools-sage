    ...
    topics: dict[str, dict[str, Any]] = {
        ...
        "setup": {
            ...
            "steps": [
                ...
                "Exit safely at any point; the next launch resumes from SAGEdata/.system/state/setup-state.json.",
            ],
        },
        "surfaces": {
            "title": "SAGE SURFACES",
...
def _app_root_for_settings(settings_path: Path) -> Path:
    """Resolve the Core application root without requiring SAGEdata to be available."""
...
def command_data_home_show(args: argparse.Namespace) -> int:
    """Show the effective SAGEdata location for this Core checkout."""
    ...
    if args.json:
        ...
        print(f"SAGEdata:  {layout.data_root}")
    return 0


def command_data_home_set(args: argparse.Namespace) -> int:
    """Persist a custom SAGEdata location without moving or deleting existing data."""
    root = _app_root_for_settings(_settings_path(args.settings))
    ...
    if args.json:
        ...
        print(f"SAGEdata location configured: {layout.data_root}")
        print("Existing data was not moved or copied.")
    return 0


def command_data_home_reset(args: argparse.Namespace) -> int:
    """Clear only the persisted custom pointer; no SAGEdata content is deleted."""
    root = _app_root_for_settings(_settings_path(args.settings))
    locator = clear_persisted_data_home(root)
    default = root.parent / "SAGEdata"
    result = {
        "status": "RESET",
        "sage_root": str(root),
    ...
    if args.json:
        ...
        print(f"Custom SAGEdata pointer cleared. Default: {default}")
...
def build_parser(*, include_internal: bool = False) -> GuidedArgumentParser:
    ...
    parser.add_argument("--data-home", help="Override SAGEdata for this invocation (also available as SAGE_DATA_HOME)")
    ...
    data_home = subparsers.add_parser("data-home", help="Show or configure the persistent SAGEdata location")
    data_home_actions = data_home.add_subparsers(dest="data_home_command", required=True)
    data_home_show = data_home_actions.add_parser("show", help="Show the effective SAGEdata location")
    data_home_show.set_defaults(handler=command_data_home_show)
    data_home_set = data_home_actions.add_parser("set", help="Persist a custom SAGEdata location without moving data")
    data_home_set.add_argument("path")
    data_home_set.set_defaults(handler=command_data_home_set)
    data_home_reset = data_home_actions.add_parser("reset", help="Return to the sibling SAGEdata default without deleting data")
