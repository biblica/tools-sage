        ...
        *,
        ...
        if context_window != 16_384:
            raise ValidationError(
                "The SAGE local admin assistant context window must be 16384 tokens",
                code="LLM_CONTEXT_WINDOW_INVALID",
            )
        if keep_alive != 0:
            raise ValidationError(
                "The SAGE local admin assistant must unload after each response",
                code="LLM_KEEP_ALIVE_INVALID",
            )
