...
def _relative(root: Path, path: Path) -> str:
    """Return a portable governed Core/SAGEdata path reference."""
    try:
...
def _required_review_checks(
    ...
    if operation == "qa":
        ...
        if checks.get("translation_meaning", True):
            required.append("MEANING_EQUIVALENCE")
        ...
        if checks.get("structure_completeness", True):
            required.append("QUOTATION_STRUCTURE")
        return required
...
def _approved_saw_qa_work_plan(
    ...
    *,
    ...
    if not run.approved_work_plan_path:
        return None
    plan_path = Path(run.approved_work_plan_path).expanduser()
    ...
    expected_refs = [ref.label() for record in selected for ref in sorted(record.refs)]
    ...
    for index, unit in enumerate(units, start=1):
        ...
        for value in refs:
            for ref_scope in parse_scope_set(value):
                ...
                if end_chapter != ref_scope.start_chapter or any(
                    not parsed_unit.contains(
