...
from .resource_discovery import quick_resource_discovery


def validate_scripture_resources(
    root: Path,
    ...
    *,
    ...
    A clean installation with zero SAGE Projects is valid. Once a Project is
    ...
    ``persist_discovery`` is reserved for startup/operator checks that should advance the
    lightweight added/removed resource baseline; ordinary validation remains read-only.
    """
    ...
    if projects_root_value:
        try:
            ...
            # Project-level path validation below remains authoritative; catalog failure is
            # reported as a warning so general SAGE setup is not trapped by discovery metadata.
            pass
    catalogue_info = catalog_summary(catalogue)
    discovery = quick_resource_discovery(sage_root, persist=persist_discovery)
    ...
    return {
        ...
        "catalog": {**catalogue_info, "path": str(storage_layout(sage_root).state_root / "paratext-project-catalog.json")},
