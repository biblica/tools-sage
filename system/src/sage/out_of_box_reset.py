"""Restore SAGEdata to an explicit, bounded first-run state without modifying Core."""
...
def _remove(path: Path, data_root: Path, removed: list[str]) -> None:
    """Remove one path only when it is governed by this SAGEdata root."""
    if not path.exists() and not path.is_symlink():
        return
    try:
        label = path.resolve().relative_to(data_root.resolve()).as_posix()
    except ValueError as exc:
        raise ValidationError(f"Out-of-box reset target is outside SAGEdata: {path}") from exc
...
def reset_to_out_of_box(sage_root: Path) -> dict[str, Any]:
    ...
    if after_hash != before_hash:
        raise ValidationError("Core ecosystem.yml changed during SAGEdata reset", code="CORE_MUTATION_DETECTED")
    ...
    receipt = {
        ...
        "preserved": [
            "SAGE Core",
            "SAGEdata/.system/runtime/venv" if preserved_venv is not None else "managed runtime not yet created",
        ],
