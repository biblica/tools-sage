"""Cross-platform SAGE application/data storage contract.

The Git-controlled application tree is immutable at runtime. Persistent operator and
machine-local state lives beneath a separately resolved SAGEdata directory.
"""
...
def default_data_home(app_root: Path) -> Path:
    """Return the zero-configuration sibling SAGEdata directory."""
    root = _resolved(app_root)
    return root.parent / "SAGEdata"
...
def locator_path(app_root: Path, environ: Mapping[str, str] | None = None) -> Path:
    """Return the OS-local pointer used only when a custom SAGEdata home is persisted."""
...
def read_persisted_data_home(app_root: Path, environ: Mapping[str, str] | None = None) -> Path | None:
    ...
    if not candidate.exists():
        raise StorageError(
            f"Configured SAGEdata location is unavailable: {candidate}. "
...
def validate_data_home(app_root: Path, data_home: Path) -> Path:
    ...
    if data == app or _is_relative_to(data, app):
        raise StorageError(f"SAGEdata must be outside the Git-controlled SAGE tree: {data}")
    return data
...
def resolve_data_home(
    ...
    *,
    ...
    """Resolve SAGEdata by explicit path, environment, persisted locator, then sibling default."""
    root = _resolved(app_root)
    env = os.environ if environ is None else environ
...
class StorageLayout:
    ...
    def projects_root(self) -> Path:
        """Return the visible operator Project root."""
    ...
    def marker_path(self) -> Path:
        """Return the SAGEdata ownership marker path."""
    ...
    def ensure(self) -> "StorageLayout":
        ...
        if self.marker_path.is_file():
            try:
                ...
                raise StorageError(f"Invalid SAGEdata marker: {self.marker_path}: {exc}") from exc
            if (
                not isinstance(existing, dict)
                or existing.get("product") != "SAGE"
                or existing.get("schema_version") != DATA_LAYOUT_SCHEMA
            ):
                raise StorageError(f"Directory is not a recognized SAGEdata root: {self.data_root}")
        elif self.data_root.exists():
            # Validate before creating anything so an unrelated destination is never modified.
            visible_known = {"projects", "jobs", "resources", "plugins", "reports", "exports", ".system"}
...
def declare_governed_path(app_root: Path, path: Path, label: str) -> str:
    """Encode a Core/SAGEdata path as a portable governed reference."""
    root = _resolved(app_root)
    ...
    raise StorageError(f"{label} is outside governed SAGE Core/SAGEdata roots: {candidate}")
