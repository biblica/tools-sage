# RC7.04 hardening and context refinement

Version: `0.01-rc7.04`

RC7.04 is a hardening/refinement release. It preserves the established project-cardinality grammar, BIC/SAW authority boundaries, provider architecture, and byte-pinned protected BIC linguistic contracts while tightening bounded TARGET safety, provider handoff budgeting, OL micro-scoping, SAW discourse segmentation, history ordering, and release validation.
...
## SAW Normal QA discourse units

Translation-and-meaning QA preserves deterministic discourse units, then coalesces adjacent units toward the governed token target. This supersedes the earlier one-discourse-unit-per-task rule, which created context-starved microtasks for short paragraphs.
...
This is an operational segmentation rule for model context, not a claim that every generated unit is a literary stanza. Partitioned tasks receive explicitly labelled adjacent context-only WIP and REFERENCE evidence, excluded from primary coverage and ordinary findings.

## Release gate

`scripts/hardening.py` discovers every `tests/test_*.py` module and automatically schedules any module not present in the curated batches. Timed-out subprocess trees are explicitly terminated. The hardening report records the exact governed source-tree SHA-256.
