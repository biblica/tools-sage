...
rights:
  ...
  license_identifier: Governed licence or agreement identifier
...
```bash
./sage resource validate-rights
```

Legacy metadata remains evidence but does not pass schema `1.0` until a responsible human supplies the missing authority fields. SAGE must not infer copyright, licence, import authority, or redistribution permission from file presence, project role, or technical validation status.
