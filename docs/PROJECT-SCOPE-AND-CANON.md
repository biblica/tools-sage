...
Lifecycle does **not** grant filesystem write permission. The SAGE Project Inventory records only maximum capability. Effective access is resolved from the Job: SOURCE, DONOR, REFERENCE, WIP, and OL are read-only; only an explicitly authorised BIC TARGET may write `.SFM`. `.VRS` is always read-only.
