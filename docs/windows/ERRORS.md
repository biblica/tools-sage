...
| Cue / code | Action |
|---|---|
| Managed Python environment missing / dependency missing | Run `.\sage.cmd` from the SAGE root and approve **Create/Repair SAGE Python environment**. The managed environment is under the effective `SAGEdata\.system\runtime\venv`; use `.\sage.cmd data-home show` to confirm the data root. |
