...
Run SAGE from Command Prompt/PowerShell; do not start `codex` first. If the Codex CLI is missing, installation requires explicit operator confirmation. SAGE validates/repairs its local `.venv`, then checks Codex and the persisted ChatGPT login.

`.venv` is **not shipped in the ZIP** and SAGE does not use `.env`. It is created in the extracted SAGE root on first approved launch. Its interpreter is `.venv\Scripts\python.exe`.
