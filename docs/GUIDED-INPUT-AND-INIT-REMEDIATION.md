...
|---|---|---|
| `INPUT_REQUIRED` | A missing, unknown, mistyped, ambiguous, stale, or unconfirmed Operator value can be corrected safely. | Prompt, select or enter a value, confirm it, revalidate, then continue. |
...
3. asks the Operator to select an alternative, enter another value, or cancel;
...
Scripture book 'JUN' was not recognised.
...
- missing or stale workspace initialisation before task creation.
...
```text
workspace-data/sage/init/operator-overrides.yml
```
...
```bash
./sage --settings FILE.yml project init --clear-overrides
```
...
## Initialisation before tasks

If an analytical task is requested before initialisation, or after source/effective settings changed, interactive SAGE offers to run `workspace initialize` immediately. Non-interactive mode returns `WORKSPACE_INITIALIZATION_INPUT_REQUIRED`.

For a menu-owned BIC or SAW Run, readiness is checked against that active Job's runtime settings and Job-local initialisation receipt. A root-workspace receipt is used only for direct root CLI/API compatibility when no Job-local receipt exists.
...
If initialisation completes but reports a genuine in-scope defect, task creation remains `BLOCKED`.
