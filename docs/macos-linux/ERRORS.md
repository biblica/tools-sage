...
| Cue / code | Action |
|---|---|
| Managed Python environment missing / Python dependency missing | Run `./sage` from the SAGE root and approve **Create/Repair SAGE Python environment**. The environment is under the effective `SAGEdata/.system/runtime/venv`; use `./sage data-home show` to confirm the data root. |
