...
`./sage project init`, `./sage workspace validate`, and `./sage workspace initialize` use guided remediation for recoverable settings.
...
```bash
./sage workspace reset-state
```

preserves it. Clear it explicitly with:

```bash
./sage project init --clear-overrides
```
...
```text
workspace-data/sage/init/operator-overrides.yml
workspace-data/sage/init/operator-review.json
workspace-data/sage/init/PROJECT-INIT-REPORT.md
workspace-data/sage/output/auto-resolution-report.json
workspace-data/sage/output/auto-resolution-report.md
```
