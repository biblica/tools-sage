# Vanilla Installation Manifest

This is the complete governed Core directory/file inventory for a clean SAGE v0.01beta source installation.

## Installation boundary

`SAGE/` is replaceable Git-controlled Core. The distribution contains code, immutable baseline configuration, schemas, approved/tested Core resources, Skills, documentation, tests, and deterministic bootstrap/release tooling.

`SAGEdata/` is not part of the source distribution. On first launch the SAGE launcher resolves the configured data home (default: sibling `SAGEdata`), creates the governed local folder structure, and deterministically creates the host Python runtime at `SAGEdata/.system/runtime/venv` from the pinned Core dependency manifests.

A vanilla source package contains no Operator Projects, Jobs, Runs, reports, local/candidate resources, plugins, machine settings, runtime state, caches, copied virtual environments, or developer-host metadata. The bundled Greek/Hebrew Scripture resources are approved Core resources and therefore remain under `system/resources/scripture/original-language/`.

This inventory is a release contract: package tests require it to match the clean source tree exactly.

## Complete shipped Core tree

- `.gitattributes`
- `.github`
- `.github/workflows`
- `.github/workflows/ci.yml`
- `.gitignore`
- `docs`
- `docs/advanced`
- `docs/advanced/architecture`
- `docs/advanced/architecture/ARCHITECTURE.md`
- `docs/advanced/architecture/CARDINALITY-AND-BINDING-GRAMMAR.md`
- `docs/advanced/architecture/FILE-NAMING-AND-SERIALIZATION.md`
- `docs/advanced/architecture/HUMAN-OUTPUT-AND-LOGGING.md`
- `docs/advanced/architecture/PROJECT-TREE.md`
- `docs/advanced/architecture/SAGE-SYSTEM-GRAMMAR.md`
- `docs/advanced/architecture/STORAGE-AND-CORE-BOUNDARY.md`
- `docs/advanced/future`
- `docs/advanced/future/BASE-TARGET-REVISION-WORKFLOW.md`
- `docs/advanced/future/README.md`
- `docs/advanced/future/RESOURCE-HUB.md`
- `docs/advanced/future/WDA-WORD-DATA-ANALYSIS.md`
- `docs/advanced/maintenance`
- `docs/advanced/maintenance/GIT-WORKFLOW.md`
- `docs/advanced/maintenance/HARDENING-AND-CONTEXT-REFINEMENT.md`
- `docs/advanced/maintenance/MACOS-LINUX-PATH-EXECUTION-REPORT.md`
- `docs/advanced/maintenance/MENU-LOCALIZATION.md`
- `docs/advanced/maintenance/PURPOSE-FUNCTION-DRIFT-REPORT.md`
- `docs/advanced/maintenance/PYTHON-MAINTENANCE.md`
- `docs/advanced/maintenance/UI-PRESENTATION.md`
- `docs/advanced/maintenance/WINDOWS-CODEX-EXECUTION-AUDIT.md`
- `docs/advanced/models-and-ai`
- `docs/advanced/models-and-ai/LOCAL-AI-ASSISTIVE-MODE.md`
- `docs/advanced/models-and-ai/LOCAL-EVIDENCE-AND-LINGUISTIC-COMPETENCE.md`
- `docs/advanced/models-and-ai/LOCAL-FIRST-DESIGN-RULE.md`
- `docs/advanced/models-and-ai/MODEL-HANDOFF-OPTIMIZATION.md`
- `docs/advanced/models-and-ai/MODEL-LANGUAGE-COMPETENCY.md`
- `docs/advanced/models-and-ai/MODEL-SELECTION-AND-REASONING.md`
- `docs/advanced/projects-and-resources`
- `docs/advanced/projects-and-resources/FLEX-COMBINE-INTERCHANGE.md`
- `docs/advanced/projects-and-resources/GRAMMAR-PROFILES.md`
- `docs/advanced/projects-and-resources/LANGUAGE-IDENTIFICATION-AND-PROFILES.md`
- `docs/advanced/projects-and-resources/ORIGINAL-LANGUAGE-RESOURCES.md`
- `docs/advanced/projects-and-resources/PROJECT-CATALOG-AND-MAINTENANCE.md`
- `docs/advanced/projects-and-resources/PROJECT-INVENTORY.md`
- `docs/advanced/projects-and-resources/PROJECT-SCOPE-AND-CANON.md`
- `docs/advanced/projects-and-resources/RESOURCE-RIGHTS-AND-PROVENANCE.md`
- `docs/advanced/projects-and-resources/RWC-SEMDOM-INDEXES.md`
- `docs/advanced/projects-and-resources/VERSIFICATION.md`
- `docs/advanced/README.md`
- `docs/advanced/release`
- `docs/advanced/release/HANDOVER.md`
- `docs/advanced/release/IMPLEMENTATION-REPORT.md`
- `docs/advanced/release/RELEASE-GATES.md`
- `docs/advanced/release/RELEASE-NOTES.md`
- `docs/advanced/release/TEST-AND-VALIDATION-REPORT.md`
- `docs/advanced/release/VANILLA-INSTALL-MANIFEST.md`
- `docs/advanced/workflows`
- `docs/advanced/workflows/AUTO-RESOLUTION.md`
- `docs/advanced/workflows/BIC-SAW-AUTHORITY-BOUNDARIES.md`
- `docs/advanced/workflows/EXECUTION-BLOCK-AND-RETRY.md`
- `docs/advanced/workflows/FULL-PROCESS-FLOW.md`
- `docs/advanced/workflows/GUIDED-INPUT-AND-INIT-REMEDIATION.md`
- `docs/advanced/workflows/JOB-STORAGE-MAINTENANCE.md`
- `docs/advanced/workflows/NATURAL-LANGUAGE-COMMAND-ROUTING.md`
- `docs/advanced/workflows/REWRITE-LEXICAL-RISK-AND-OL.md`
- `docs/advanced/workflows/SAW-CHECK-POLICY.md`
- `docs/advanced/workflows/SAW-OPERATOR-MODES.md`
- `docs/advanced/workflows/STRUCTURE-PLANNING.md`
- `docs/advanced/workflows/TARGET-GENERATIONS.md`
- `docs/BIC-CHEAT-SHEET.md`
- `docs/GOOD-PRACTICE.md`
- `docs/INDEX.md`
- `docs/KNOWN-LIMITATIONS.md`
- `docs/macos-linux`
- `docs/macos-linux/CHEAT-SHEET.md`
- `docs/macos-linux/ERRORS.md`
- `docs/macos-linux/RECOVERY.md`
- `docs/OPERATOR-GUIDE.md`
- `docs/PROJECT-OPERATOR-CHEAT-SHEET.md`
- `docs/SAW-CHEAT-SHEET.md`
- `docs/TUI.md`
- `docs/windows`
- `docs/windows/CHEAT-SHEET.md`
- `docs/windows/ERRORS.md`
- `docs/windows/RECOVERY.md`
- `ecosystem.yml`
- `README.md`
- `sage`
- `sage.cmd`
- `system`
- `system/bin`
- `system/bin/bic`
- `system/bin/bic.cmd`
- `system/bin/sage`
- `system/bin/sage.cmd`
- `system/bin/saw`
- `system/bin/saw.cmd`
- `system/config`
- `system/config/bic-protected-rewrite-pin.json`
- `system/config/bic-protected-verb-selection-pin.json`
- `system/config/CHANGELOG.md`
- `system/config/contracts`
- `system/config/contracts/bic-verb-selection-policy.yml`
- `system/config/DEVELOPMENT-STATUS.md`
- `system/config/languages`
- `system/config/languages/competency.yml`
- `system/config/languages/registry.yml`
- `system/config/languages/relationships.yml`
- `system/config/localization`
- `system/config/localization/menu-localization.json`
- `system/config/localization/README.md`
- `system/config/model-language-competency.yml`
- `system/config/model-policy.yml`
- `system/config/NEXT-DEVELOPMENT-WORK.md`
- `system/config/profiles`
- `system/config/profiles/grammar`
- `system/config/profiles/grammar/am-ET`
- `system/config/profiles/grammar/am-ET/wip.yml`
- `system/config/profiles/grammar/ar-145`
- `system/config/profiles/grammar/ar-145/wip.yml`
- `system/config/profiles/grammar/ar-SA`
- `system/config/profiles/grammar/ar-SA/wip.yml`
- `system/config/profiles/grammar/de-DE`
- `system/config/profiles/grammar/de-DE/wip.yml`
- `system/config/profiles/grammar/en-GB`
- `system/config/profiles/grammar/en-GB/wip.yml`
- `system/config/profiles/grammar/en-US`
- `system/config/profiles/grammar/en-US/wip.yml`
- `system/config/profiles/grammar/es-419`
- `system/config/profiles/grammar/es-419/wip.yml`
- `system/config/profiles/grammar/es-BR`
- `system/config/profiles/grammar/es-BR/wip.yml`
- `system/config/profiles/grammar/fa-IR`
- `system/config/profiles/grammar/fa-IR/wip.yml`
- `system/config/profiles/grammar/fr-011`
- `system/config/profiles/grammar/fr-011/wip.yml`
- `system/config/profiles/grammar/fr-FR`
- `system/config/profiles/grammar/fr-FR/wip.yml`
- `system/config/profiles/grammar/ha-NE`
- `system/config/profiles/grammar/ha-NE/wip.yml`
- `system/config/profiles/grammar/ha-NG`
- `system/config/profiles/grammar/ha-NG/wip.yml`
- `system/config/profiles/grammar/hi-IN`
- `system/config/profiles/grammar/hi-IN/wip.yml`
- `system/config/profiles/grammar/id-ID`
- `system/config/profiles/grammar/id-ID/wip.yml`
- `system/config/profiles/grammar/pt-419`
- `system/config/profiles/grammar/pt-419/wip.yml`
- `system/config/profiles/grammar/pt-BR`
- `system/config/profiles/grammar/pt-BR/wip.yml`
- `system/config/profiles/grammar/README.md`
- `system/config/profiles/grammar/ti-ER`
- `system/config/profiles/grammar/ti-ER/wip.yml`
- `system/config/profiles/grammar/ti-ET`
- `system/config/profiles/grammar/ti-ET/wip.yml`
- `system/config/profiles/grammar/uk-UA`
- `system/config/profiles/grammar/uk-UA/wip.yml`
- `system/config/project-management`
- `system/config/project-management/BUILD-ISSUES.md`
- `system/config/project-management/IMPLEMENTED-UPDATES.md`
- `system/config/project-management/MILESTONES.md`
- `system/config/project-management/README.md`
- `system/config/project-management/RELEASE-CLEANUP.md`
- `system/config/project-management/TODO.md`
- `system/config/project-management/VERSIONING-POLICY.md`
- `system/config/qualification-baselines.json`
- `system/config/sage-standard.json`
- `system/config/schemas`
- `system/config/schemas/act-control.schema.yml`
- `system/config/schemas/act-task.schema.yml`
- `system/config/schemas/active-jobs.schema.yml`
- `system/config/schemas/bic-grammar-assessment.schema.yml`
- `system/config/schemas/bic-human-review-receipt.schema.yml`
- `system/config/schemas/bic-inspect-submission.schema.yml`
- `system/config/schemas/bic-translation-challenges.schema.yml`
- `system/config/schemas/ecosystem.schema.yml`
- `system/config/schemas/evaluation-set.schema.yml`
- `system/config/schemas/execution-event.schema.yml`
- `system/config/schemas/generated-target-manifest.schema.yml`
- `system/config/schemas/grammar-profile.schema.yml`
- `system/config/schemas/job.schema.yml`
- `system/config/schemas/language-profile-registry.schema.yml`
- `system/config/schemas/llm-execution-receipt.schema.yml`
- `system/config/schemas/model-language-competency.schema.yml`
- `system/config/schemas/model-policy.schema.yml`
- `system/config/schemas/original-language-resources.schema.yml`
- `system/config/schemas/paratext-project-catalog.schema.yml`
- `system/config/schemas/project-code.schema.yml`
- `system/config/schemas/project-inventory.schema.yml`
- `system/config/schemas/project-manifest.schema.yml`
- `system/config/schemas/project-scope.schema.yml`
- `system/config/schemas/resource-discovery.schema.yml`
- `system/config/schemas/resource-rights.schema.yml`
- `system/config/schemas/run.schema.yml`
- `system/config/schemas/saw-findings.schema.yml`
- `system/config/schemas/semantic-export-manifest.schema.yml`
- `system/config/schemas/semantic-import-manifest.schema.yml`
- `system/config/schemas/semantic-index-contract.schema.yml`
- `system/config/schemas/skill-registry.schema.yml`
- `system/config/schemas/structure-planning.schema.yml`
- `system/config/schemas/transaction-journal.schema.yml`
- `system/config/schemas/work-unit-manifest.schema.yml`
- `system/config/schemas/workflow-profile.schema.yml`
- `system/config/skills.json`
- `system/config/structure-planning.yml`
- `system/config/workflows`
- `system/config/workflows/bic`
- `system/config/workflows/bic/profile.yml`
- `system/config/workflows/bic/README.md`
- `system/config/workflows/saw`
- `system/config/workflows/saw/profile.yml`
- `system/config/workflows/saw/README.md`
- `system/pyproject.toml`
- `system/requirements-dev.txt`
- `system/requirements-tui.txt`
- `system/requirements.txt`
- `system/resources`
- `system/resources/rwc`
- `system/resources/rwc/authority`
- `system/resources/rwc/authority/sources.json`
- `system/resources/rwc/README.md`
- `system/resources/scripture`
- `system/resources/scripture/eng.vrs`
- `system/resources/scripture/org.vrs`
- `system/resources/scripture/original-language`
- `system/resources/scripture/original-language/grk`
- `system/resources/scripture/original-language/grk/41MATgrkRES.SFM`
- `system/resources/scripture/original-language/grk/42MRKgrkRES.SFM`
- `system/resources/scripture/original-language/grk/43LUKgrkRES.SFM`
- `system/resources/scripture/original-language/grk/44JHNgrkRES.SFM`
- `system/resources/scripture/original-language/grk/45ACTgrkRES.SFM`
- `system/resources/scripture/original-language/grk/46ROMgrkRES.SFM`
- `system/resources/scripture/original-language/grk/471COgrkRES.SFM`
- `system/resources/scripture/original-language/grk/482COgrkRES.SFM`
- `system/resources/scripture/original-language/grk/49GALgrkRES.SFM`
- `system/resources/scripture/original-language/grk/50EPHgrkRES.SFM`
- `system/resources/scripture/original-language/grk/51PHPgrkRES.SFM`
- `system/resources/scripture/original-language/grk/52COLgrkRES.SFM`
- `system/resources/scripture/original-language/grk/531THgrkRES.SFM`
- `system/resources/scripture/original-language/grk/542THgrkRES.SFM`
- `system/resources/scripture/original-language/grk/551TIgrkRES.SFM`
- `system/resources/scripture/original-language/grk/562TIgrkRES.SFM`
- `system/resources/scripture/original-language/grk/57TITgrkRES.SFM`
- `system/resources/scripture/original-language/grk/58PHMgrkRES.SFM`
- `system/resources/scripture/original-language/grk/59HEBgrkRES.SFM`
- `system/resources/scripture/original-language/grk/60JASgrkRES.SFM`
- `system/resources/scripture/original-language/grk/611PEgrkRES.SFM`
- `system/resources/scripture/original-language/grk/622PEgrkRES.SFM`
- `system/resources/scripture/original-language/grk/631JNgrkRES.SFM`
- `system/resources/scripture/original-language/grk/642JNgrkRES.SFM`
- `system/resources/scripture/original-language/grk/653JNgrkRES.SFM`
- `system/resources/scripture/original-language/grk/66JUDgrkRES.SFM`
- `system/resources/scripture/original-language/grk/67REVgrkRES.SFM`
- `system/resources/scripture/original-language/grk/BookNames.xml`
- `system/resources/scripture/original-language/grk/README.md`
- `system/resources/scripture/original-language/grk/Settings.xml`
- `system/resources/scripture/original-language/heb`
- `system/resources/scripture/original-language/heb/01GENhebRES.SFM`
- `system/resources/scripture/original-language/heb/02EXOhebRES.SFM`
- `system/resources/scripture/original-language/heb/03LEVhebRES.SFM`
- `system/resources/scripture/original-language/heb/04NUMhebRES.SFM`
- `system/resources/scripture/original-language/heb/05DEUhebRES.SFM`
- `system/resources/scripture/original-language/heb/06JOShebRES.SFM`
- `system/resources/scripture/original-language/heb/07JDGhebRES.SFM`
- `system/resources/scripture/original-language/heb/08RUThebRES.SFM`
- `system/resources/scripture/original-language/heb/091SAhebRES.SFM`
- `system/resources/scripture/original-language/heb/102SAhebRES.SFM`
- `system/resources/scripture/original-language/heb/111KIhebRES.SFM`
- `system/resources/scripture/original-language/heb/122KIhebRES.SFM`
- `system/resources/scripture/original-language/heb/131CHhebRES.SFM`
- `system/resources/scripture/original-language/heb/142CHhebRES.SFM`
- `system/resources/scripture/original-language/heb/15EZRhebRES.SFM`
- `system/resources/scripture/original-language/heb/16NEHhebRES.SFM`
- `system/resources/scripture/original-language/heb/17ESThebRES.SFM`
- `system/resources/scripture/original-language/heb/18JOBhebRES.SFM`
- `system/resources/scripture/original-language/heb/19PSAhebRES.SFM`
- `system/resources/scripture/original-language/heb/20PROhebRES.SFM`
- `system/resources/scripture/original-language/heb/21ECChebRES.SFM`
- `system/resources/scripture/original-language/heb/22SNGhebRES.SFM`
- `system/resources/scripture/original-language/heb/23ISAhebRES.SFM`
- `system/resources/scripture/original-language/heb/24JERhebRES.SFM`
- `system/resources/scripture/original-language/heb/25LAMhebRES.SFM`
- `system/resources/scripture/original-language/heb/26EZKhebRES.SFM`
- `system/resources/scripture/original-language/heb/27DANhebRES.SFM`
- `system/resources/scripture/original-language/heb/28HOShebRES.SFM`
- `system/resources/scripture/original-language/heb/29JOLhebRES.SFM`
- `system/resources/scripture/original-language/heb/30AMOhebRES.SFM`
- `system/resources/scripture/original-language/heb/31OBAhebRES.SFM`
- `system/resources/scripture/original-language/heb/32JONhebRES.SFM`
- `system/resources/scripture/original-language/heb/33MIChebRES.SFM`
- `system/resources/scripture/original-language/heb/34NAMhebRES.SFM`
- `system/resources/scripture/original-language/heb/35HABhebRES.SFM`
- `system/resources/scripture/original-language/heb/36ZEPhebRES.SFM`
- `system/resources/scripture/original-language/heb/37HAGhebRES.SFM`
- `system/resources/scripture/original-language/heb/38ZEChebRES.SFM`
- `system/resources/scripture/original-language/heb/39MALhebRES.SFM`
- `system/resources/scripture/original-language/heb/BookNames.xml`
- `system/resources/scripture/original-language/heb/README.md`
- `system/resources/scripture/original-language/heb/Settings.xml`
- `system/resources/scripture/original-language/README.md`
- `system/resources/scripture/README.md`
- `system/skills`
- `system/skills/bic-inspect`
- `system/skills/bic-inspect/agents`
- `system/skills/bic-inspect/agents/openai.yaml`
- `system/skills/bic-inspect/references`
- `system/skills/bic-inspect/references/BIC-WORKFLOW-CONTROLLER.md`
- `system/skills/bic-inspect/references/INSPECT-CONTRACT.md`
- `system/skills/bic-inspect/references/LOCAL-EVIDENCE-AND-LINGUISTIC-COMPETENCE.md`
- `system/skills/bic-inspect/references/MEMORY-AND-OPERATOR-BOUNDARY.md`
- `system/skills/bic-inspect/references/ORIGINAL-SKILL.md`
- `system/skills/bic-inspect/references/PROJECT-GRAMMAR-AND-SAW.md`
- `system/skills/bic-inspect/references/SEMANTIC-INDEX-AND-LOCAL-FIRST.md`
- `system/skills/bic-inspect/references/SOURCE-AUTHORITY-AND-USFM.md`
- `system/skills/bic-inspect/SKILL.md`
- `system/skills/bic-rewrite`
- `system/skills/bic-rewrite/agents`
- `system/skills/bic-rewrite/agents/openai.yaml`
- `system/skills/bic-rewrite/references`
- `system/skills/bic-rewrite/references/BIC-WORKFLOW-CONTROLLER.md`
- `system/skills/bic-rewrite/references/LOCAL-EVIDENCE-AND-LINGUISTIC-COMPETENCE.md`
- `system/skills/bic-rewrite/references/MEMORY-AND-OPERATOR-BOUNDARY.md`
- `system/skills/bic-rewrite/references/ORIGINAL-SKILL.md`
- `system/skills/bic-rewrite/references/PROJECT-GRAMMAR-AND-SAW.md`
- `system/skills/bic-rewrite/references/PROTECTED-REWRITE-DETAIL-RULES.md`
- `system/skills/bic-rewrite/references/REWRITE-CONTRACT.md`
- `system/skills/bic-rewrite/references/REWRITE-RISK-AND-OL.md`
- `system/skills/bic-rewrite/references/SEMANTIC-INDEX-AND-LOCAL-FIRST.md`
- `system/skills/bic-rewrite/references/SOURCE-AUTHORITY-AND-USFM.md`
- `system/skills/bic-rewrite/SKILL.md`
- `system/skills/bic-self-check`
- `system/skills/bic-self-check/agents`
- `system/skills/bic-self-check/agents/openai.yaml`
- `system/skills/bic-self-check/references`
- `system/skills/bic-self-check/references/BIC-WORKFLOW-CONTROLLER.md`
- `system/skills/bic-self-check/references/LOCAL-EVIDENCE-AND-LINGUISTIC-COMPETENCE.md`
- `system/skills/bic-self-check/references/MEMORY-AND-OPERATOR-BOUNDARY.md`
- `system/skills/bic-self-check/references/ORIGINAL-SKILL.md`
- `system/skills/bic-self-check/references/PROJECT-GRAMMAR-AND-SAW.md`
- `system/skills/bic-self-check/references/PROTECTED-REWRITE-DETAIL-RULES.md`
- `system/skills/bic-self-check/references/REWRITE-RISK-AND-OL.md`
- `system/skills/bic-self-check/references/SELF-CHECK-CONTRACT.md`
- `system/skills/bic-self-check/references/SEMANTIC-INDEX-AND-LOCAL-FIRST.md`
- `system/skills/bic-self-check/references/SOURCE-AUTHORITY-AND-USFM.md`
- `system/skills/bic-self-check/SKILL.md`
- `system/skills/saw-focused-check`
- `system/skills/saw-focused-check/agents`
- `system/skills/saw-focused-check/agents/openai.yaml`
- `system/skills/saw-focused-check/references`
- `system/skills/saw-focused-check/references/LOCAL-EVIDENCE-AND-LINGUISTIC-COMPETENCE.md`
- `system/skills/saw-focused-check/references/ORIGINAL-WORKFLOW-PROMPT.md`
- `system/skills/saw-focused-check/references/RUN-CHECK.md`
- `system/skills/saw-focused-check/references/SAW-EXECUTION-RULES.md`
- `system/skills/saw-focused-check/references/SEMANTIC-INDEX-AND-LOCAL-FIRST.md`
- `system/skills/saw-focused-check/SKILL.md`
- `system/skills/saw-ol-review`
- `system/skills/saw-ol-review/agents`
- `system/skills/saw-ol-review/agents/openai.yaml`
- `system/skills/saw-ol-review/references`
- `system/skills/saw-ol-review/references/LOCAL-EVIDENCE-AND-LINGUISTIC-COMPETENCE.md`
- `system/skills/saw-ol-review/references/ORIGINAL-WORKFLOW-PROMPT.md`
- `system/skills/saw-ol-review/references/RUN-OL.md`
- `system/skills/saw-ol-review/references/SAW-EXECUTION-RULES.md`
- `system/skills/saw-ol-review/references/SEMANTIC-INDEX-AND-LOCAL-FIRST.md`
- `system/skills/saw-ol-review/SKILL.md`
- `system/skills/saw-qa`
- `system/skills/saw-qa/agents`
- `system/skills/saw-qa/agents/openai.yaml`
- `system/skills/saw-qa/references`
- `system/skills/saw-qa/references/LOCAL-EVIDENCE-AND-LINGUISTIC-COMPETENCE.md`
- `system/skills/saw-qa/references/OPERATION-CONTRACT.md`
- `system/skills/saw-qa/references/ORIGINAL-WORKFLOW-PROMPT.md`
- `system/skills/saw-qa/references/SAW-EXECUTION-RULES.md`
- `system/skills/saw-qa/references/SEMANTIC-INDEX-AND-LOCAL-FIRST.md`
- `system/skills/saw-qa/SKILL.md`
- `system/src`
- `system/src/sage`
- `system/src/sage/__init__.py`
- `system/src/sage/act_outputs.py`
- `system/src/sage/act_tasks.py`
- `system/src/sage/atomic.py`
- `system/src/sage/auto_resolution.py`
- `system/src/sage/bic_memory.py`
- `system/src/sage/bounded_target.py`
- `system/src/sage/build_policy.py`
- `system/src/sage/canon.py`
- `system/src/sage/cli.py`
- `system/src/sage/config.py`
- `system/src/sage/consolidation.py`
- `system/src/sage/coverage.py`
- `system/src/sage/data`
- `system/src/sage/data/iso-3166-1.json`
- `system/src/sage/data/iso-639-3.json`
- `system/src/sage/errors.py`
- `system/src/sage/evidence.py`
- `system/src/sage/evidence_policy.py`
- `system/src/sage/execution_events.py`
- `system/src/sage/executors`
- `system/src/sage/executors/__init__.py`
- `system/src/sage/executors/base.py`
- `system/src/sage/executors/codex_cli.py`
- `system/src/sage/executors/http.py`
- `system/src/sage/executors/ollama.py`
- `system/src/sage/external_access.py`
- `system/src/sage/findings.py`
- `system/src/sage/generations.py`
- `system/src/sage/grammar.py`
- `system/src/sage/grammar_governance.py`
- `system/src/sage/guided_input.py`
- `system/src/sage/hashing.py`
- `system/src/sage/human_output.py`
- `system/src/sage/init_remediation.py`
- `system/src/sage/interface_localization.py`
- `system/src/sage/iso_languages.py`
- `system/src/sage/job_layout.py`
- `system/src/sage/jobs.py`
- `system/src/sage/language_codes.py`
- `system/src/sage/language_identification.py`
- `system/src/sage/language_profiles.py`
- `system/src/sage/llm_settings.py`
- `system/src/sage/llm_tasks.py`
- `system/src/sage/local_assistive.py`
- `system/src/sage/locking.py`
- `system/src/sage/menu.py`
- `system/src/sage/model_language_competency.py`
- `system/src/sage/model_policy.py`
- `system/src/sage/model_service.py`
- `system/src/sage/natural_language.py`
- `system/src/sage/ollama_admin.py`
- `system/src/sage/ollama_policy.py`
- `system/src/sage/operator_overrides.py`
- `system/src/sage/original_language_resources.py`
- `system/src/sage/out_of_box_reset.py`
- `system/src/sage/paratext_catalog.py`
- `system/src/sage/plan_continuation.py`
- `system/src/sage/platform_commands.py`
- `system/src/sage/portability.py`
- `system/src/sage/profiles.py`
- `system/src/sage/progress.py`
- `system/src/sage/project_codes.py`
- `system/src/sage/project_inventory.py`
- `system/src/sage/references.py`
- `system/src/sage/registry.py`
- `system/src/sage/report_translation.py`
- `system/src/sage/reset_state.py`
- `system/src/sage/resource_discovery.py`
- `system/src/sage/resource_mounts.py`
- `system/src/sage/resource_registration.py`
- `system/src/sage/resource_rights.py`
- `system/src/sage/resource_validation.py`
- `system/src/sage/rewrite_risk.py`
- `system/src/sage/runtime_paths.py`
- `system/src/sage/runtime_status.py`
- `system/src/sage/saw_policy.py`
- `system/src/sage/schema_validation.py`
- `system/src/sage/scripture.py`
- `system/src/sage/sections.py`
- `system/src/sage/semantic`
- `system/src/sage/semantic/__init__.py`
- `system/src/sage/semantic/authority_registry.py`
- `system/src/sage/semantic/diagnostics.py`
- `system/src/sage/semantic/evidence.py`
- `system/src/sage/semantic/freshness.py`
- `system/src/sage/semantic/importers.py`
- `system/src/sage/semantic/indexes.py`
- `system/src/sage/semantic/lift.py`
- `system/src/sage/semantic/policy.py`
- `system/src/sage/semantic/semdom.py`
- `system/src/sage/semantic/store.py`
- `system/src/sage/semantic_cli.py`
- `system/src/sage/stage_reset.py`
- `system/src/sage/standard.py`
- `system/src/sage/state.py`
- `system/src/sage/storage.py`
- `system/src/sage/structure_policy.py`
- `system/src/sage/task_retry.py`
- `system/src/sage/transactions.py`
- `system/src/sage/tui.py`
- `system/src/sage/ui_format.py`
- `system/src/sage/ui_services.py`
- `system/src/sage/usfm_stream.py`
- `system/src/sage/usj.py`
- `system/src/sage/validation.py`
- `system/src/sage/vocabulary.py`
- `system/src/sage/vrs.py`
- `system/src/sage/work_units.py`
- `system/tests`
- `system/tests/conftest.py`
- `system/tests/test_authority_boundaries.py`
- `system/tests/test_beta_saw_policy.py`
- `system/tests/test_bic4_contract_and_vocabulary.py`
- `system/tests/test_bootstrap_runtime.py`
- `system/tests/test_bundled_regional_profiles.py`
- `system/tests/test_cardinality.py`
- `system/tests/test_cli_dev3.py`
- `system/tests/test_clone_and_install.py`
- `system/tests/test_codex_app_server_session.py`
- `system/tests/test_command_contract.py`
- `system/tests/test_consolidation.py`
- `system/tests/test_controller_capabilities.py`
- `system/tests/test_core_hardening.py`
- `system/tests/test_dev8_governance.py`
- `system/tests/test_documentation_contracts.py`
- `system/tests/test_findings_and_coverage.py`
- `system/tests/test_generations.py`
- `system/tests/test_grammar_contracts.py`
- `system/tests/test_guided_remediation.py`
- `system/tests/test_hardening_and_segmentation.py`
- `system/tests/test_human_output.py`
- `system/tests/test_llm_harness.py`
- `system/tests/test_local_ai_assistive.py`
- `system/tests/test_memory_governance.py`
- `system/tests/test_menu_localization.py`
- `system/tests/test_menu_projects.py`
- `system/tests/test_model_language_competency.py`
- `system/tests/test_natural_language_routing.py`
- `system/tests/test_ollama_admin.py`
- `system/tests/test_operator_ux.py`
- `system/tests/test_out_of_box_reset.py`
- `system/tests/test_package.py`
- `system/tests/test_paratext_catalog_and_ol.py`
- `system/tests/test_parent_runtime.py`
- `system/tests/test_portability.py`
- `system/tests/test_posix_portability.py`
- `system/tests/test_progress_tracking.py`
- `system/tests/test_project_codes.py`
- `system/tests/test_project_grammar_convergence.py`
- `system/tests/test_project_inventory_and_job_isolation.py`
- `system/tests/test_provider_and_boundary_policy.py`
- `system/tests/test_rc_block_and_job_cleanup.py`
- `system/tests/test_registry_and_profiles.py`
- `system/tests/test_release_builder.py`
- `system/tests/test_report_dynamic_naming.py`
- `system/tests/test_report_translation.py`
- `system/tests/test_resource_discovery.py`
- `system/tests/test_resource_roots_and_project_creation.py`
- `system/tests/test_rewrite_risk.py`
- `system/tests/test_runtime.py`
- `system/tests/test_schema_validation.py`
- `system/tests/test_scope_and_auto_resolution.py`
- `system/tests/test_sections_and_work_units.py`
- `system/tests/test_semantic_indexes.py`
- `system/tests/test_storage_layout.py`
- `system/tests/test_storage_qa_boundaries.py`
- `system/tests/test_transactions_and_memory.py`
- `system/tests/test_tui_services.py`
- `system/tests/test_tui_textual.py`
- `system/tests/test_usj_and_scripture.py`
- `system/tests/test_vrs.py`
- `system/tests/test_windows_portability.py`
- `system/tools`
- `system/tools/bootstrap_runtime.py`
- `system/tools/build_release.py`
- `system/tools/CLONE-AND-INSTALL.md`
- `system/tools/clone_and_install.cmd`
- `system/tools/clone_and_install.py`
- `system/tools/clone_and_install.sh`
- `system/tools/deep_audit.py`
- `system/tools/hardening.py`
- `system/tools/reset_project_state.py`
- `system/tools/validate_package.py`
- `system/tools/validate_schemas.py`
- `VERSION`
