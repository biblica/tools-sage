...
- Job report ownership: use `Job report catalog` for the canonical machine record under
  ...
  `SAGEdata/reports/<job-id>/<BOOK>/`. The `<job-id>` identifies the owning Job, never a Project.
...
- Generated path grammar: never emit identical adjacent directory segments. For polished SAW output, use `SAGEdata/reports/<job-id>/<BOOK>/` for a whole-book scope; add a distinct scope directory only when it contributes additional coordinates. The same non-duplication rule applies to Job `report_data/`.
