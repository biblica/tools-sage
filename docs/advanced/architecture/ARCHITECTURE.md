...
SAGE has two ownership domains. `SAGE/` is immutable-at-runtime Git-controlled Core. `SAGEdata/` is
...
- Visible SAGEdata: Projects, Jobs, local/candidate resources, plugins, reports, and exports.
- Hidden `SAGEdata/.system`: mutable configuration overlays, machine state, controller Job state,
...
workstation/operator settings are overlaid from SAGEdata. Only reviewed/tested/approved resources
...
then resolves SAGEdata by explicit `--data-home`, environment, persisted pointer, or sibling default.
