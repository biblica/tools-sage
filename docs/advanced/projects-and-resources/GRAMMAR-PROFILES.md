...
- **Build guided regional profile** — create a `PROJECT_REVIEW_REQUIRED` local profile under `SAGEdata/resources/grammar-profiles/<tag>/`; the Operator may start from the generic governed template or explicitly derive from an existing profile.
