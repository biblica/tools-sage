# SAGE Project Inventory and Paratext Catalog — v0.01beta

SAGE ships with **zero SAGE translation Projects**. Static product policy remains in `ecosystem.yml`. Operator-selected Projects are persisted in `SAGEdata/.system/state/project-inventory.json`; derived Paratext discovery metadata is stored separately in `SAGEdata/.system/state/paratext-project-catalog.json`.
