...
| `system/bin/sage.cmd` | Resolves an absolute root without a trailing backslash, enters it with `pushd`, bootstraps the managed runtime under `SAGEdata/.system/runtime/venv`, then executes `python -m sage.cli`. Paths are quoted. |
...
| `system/tools/clone_and_install.cmd` | Selects Python 3.10+ and forwards untouched argv. |
