...
A disposable SAGE copy under a root containing spaces was run using the managed runtime outside Core at `SAGEdata/.system/runtime/venv`, containing only declared pinned dependencies. The real launchers produced:
...
- `SAGEdata/.system/runtime/venv/bin/python` is the default managed-interpreter path when SAGEdata uses the sibling location.
...
3. create a fresh `SAGEdata/.system/runtime/venv` with governed dependency installation;
