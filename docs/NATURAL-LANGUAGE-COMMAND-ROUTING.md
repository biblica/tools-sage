...
`./sage request` is a terminal/CLI controller command, not an analytical model prompt. It may propose canonical controller actions but may not bypass confirmation or perform the routed analysis directly. After a confirmed command creates `ACT.md`, execute that task through the selected SAGE provider.
...
```bash
./sage request "Run QA on Amos for NPU"
```

For a machine-readable proposal without execution:

```bash
./sage --json --no-prompt request "Run QA on Amos for NPU"
```
...
```bash
./sage task create --workflow saw --operation qa \
  --wip ukrNPUv0 \
  --reference usNIVv2 \
  --scope "JHN 10-11"
```
...
## Interactive and automated behaviour
...
```bash
./sage --json --no-prompt request "show BIC status" --execute
```
...
```text
workspace-data/sage/audit/natural-language-requests.jsonl
```
