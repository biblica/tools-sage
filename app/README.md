...
`0.02alpha1` includes an **EXPERIMENTAL / UNSTABLE** Textual migration surface. It may change incompatibly, contains incomplete and read-only workflows, and is not the authoritative Operator interface. The TUI targets `100 x 30`, uses persistent System / Active AI / Project / Active Job dashboard blocks, keeps numeric `1`-`6` navigation plus global `A`-`F` controls, and reports sequential Run progress with a compact 10-cell bar derived from governed ACT work. `F Status` is a live overlay that preserves the current view. Use the classic menu/CLI for authoritative workflow operations; see `docs/TUI.md`.
