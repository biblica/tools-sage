...
`0.02b1` retains a frozen **EXPERIMENTAL / UNSTABLE** Textual preview. It may change incompatibly and is not the authoritative Operator interface. The retained preview provides `100 x 30` navigation, status/readiness views, and the already-implemented bounded Projects-root, Quick Scan, and AI-retest actions. Project registration, Job, Run, report, and recovery workflow actions remain in the classic menu/CLI. No further TUI workflow functionality will be added in the `0.02b1` line; that work resumes in a later release. See `docs/TUI.md`.
