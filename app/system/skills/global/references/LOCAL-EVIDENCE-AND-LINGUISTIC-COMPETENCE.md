...
## RTC and STC authority

- `WIP`: subject under analysis, never its own comparison authority.
- RTC `REFERENCE`: configured LWC Reference Project comparison. STC has no Reference dependency and consumes no RTC findings; its primary authority is the routed testament-correct GRK/HEB slice. In an explicitly OL-routed bounded source-text question, the configured routed GRK/HEB packet is primary textual authority for that question; REFERENCE does not override contrary OL evidence.
- RTC original-language content is conditional bounded evidence only when the configured local GRK/HEB resource is explicitly routed. STC receives its configured GRK/HEB primary authority for every assigned correspondence work unit.
...
- Where a workflow requires conditional OL access, those reads remain sealed until its governed trigger opens the exact micro-scope. STC primary-source routing is not conditional RTC adjudication.
