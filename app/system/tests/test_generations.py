...
from sage.errors import GenerationError
from sage.generations import (
...
def test_note_only_generated_verse_cannot_be_published(make_workspace) -> None:
    ...
    target = storage_layout(root).projects_root / "usBOLx1" / "41MAT.SFM"
    target.write_text(
        "\\id MAT Fixture\n\\c 1\n\\p\n"
...
def test_publication_blocks_when_custom_vrs_changes_after_validation(make_workspace) -> None:
    ...
    root = make_workspace()
    custom = storage_layout(root).projects_root / "usBOLx1" / "custom.vrs"
