...
def test_projects_resolve_relative_to_projects_root(make_workspace) -> None:
    ...
    assert config.project("idKKHv0").path == (root.parent / "localdata" / "work" / "projects" / "idKKHv0").resolve()
...
def test_project_absolute_path_is_rejected(make_workspace) -> None:
    ...
    data["projects"]["idKKHv0"]["path"] = str((root.parent / "localdata" / "work" / "projects" / "idKKHv0").resolve())
