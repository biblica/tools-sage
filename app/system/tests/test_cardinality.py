...
def _initialize(package_root: Path, root: Path) -> None:
    """Initialize a disposable workspace through the public CLI."""
    env = dict(os.environ)
...
def test_machine_cardinality_vocabulary_is_explicit(package_root: Path) -> None:
    ...
    tool = yaml.safe_load((package_root / "system/config/schemas/job.schema.yml").read_text(encoding="utf-8"))
...
def test_ol_free_saw_operation_does_not_require_ol_binding(package_root: Path, make_workspace) -> None:
    """Targeted Check remains valid without configured Greek/Hebrew resources."""
    root = make_workspace(qualification_status="VALIDATED")
...
def test_ot_ol_operation_routes_only_job_bound_hebrew(package_root: Path, make_workspace) -> None:
    """OT scopes route Hebrew and never carry the configured Greek resource as evidence."""
    root = make_workspace(qualification_status="VALIDATED")
    settings_path = root / "ecosystem.yml"
    settings = yaml.safe_load(settings_path.read_text(encoding="utf-8"))
    for project_id in ("usWIP", "usNIVv2", "HEB"):
        settings["projects"][project_id]["scope"]["expected_books"].append("EXO")
        project_path = root.parent / "localdata" / "work" / "projects" / settings["projects"][project_id]["path"]
        (project_path / "02EXO.SFM").write_text(
            "\\id EXO Fixture\n\\c 1\n\\p\n\\v 1 Verse 1.\n",
            encoding="utf-8",
        )
    ...
    task = create_act_task(
        ...
        contemporary_source_id="usNIVv2",
        scope_value="EXO 1:1",
        focus="Adjudicate the bounded source relationship.",
    )
    manifest = json.loads(Path(task["manifest_path"]).read_text(encoding="utf-8"))

    assert manifest["original_language_sources"] == [{
        "role": "ORIGINAL_LANGUAGE_HEBREW",
        "project": "HEB",
        "routing": "DIRECT",
    }]
    assert manifest["packets"]["original_language"]["evidence_id"] == "ORIGINAL_LANGUAGE_HEBREW"
    assert "ORIGINAL_LANGUAGE_HEBREW" in manifest["allowed_evidence_ids"]
    assert "ORIGINAL_LANGUAGE_GREEK" not in manifest["allowed_evidence_ids"]
...
def test_current_operator_surfaces_use_binding_not_schema_jargon(package_root: Path) -> None:
    ...
    assert "selected TARGET grammar profile" in (package_root / "docs/advanced/architecture/SAGE-SYSTEM-GRAMMAR.md").read_text(encoding="utf-8")
    assert "resolved effective VRS" in (package_root / "docs/advanced/architecture/SAGE-SYSTEM-GRAMMAR.md").read_text(encoding="utf-8")
