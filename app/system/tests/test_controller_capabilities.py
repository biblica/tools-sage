...
from sage.grammar_governance import (
...
from sage.resource_rights import validate_resource_rights
from sage.stage_reset import reset_workflow_stage
...
def _task_control(config, workflow: str, operation: str, task_id: str) -> tuple[Path, Path]:
    """Create one governed-looking task root and control fixture."""
    workflow_spec = config.workflow(workflow)
    task_root = workflow_spec.output_root / "active" / task_id
    task_root.mkdir(parents=True)
    _write_json(
        task_root / "task-manifest.json",
        {"task_id": task_id, "workflow": workflow, "operation": operation},
    )
    control_path = workflow_spec.state_root / "act-tasks" / f"{task_id}.json"
    _write_json(
        control_path,
        {
            "task_id": task_id,
...
def test_bic_stage_reset_requires_downstream_first(make_workspace) -> None:
    ...
    root = make_workspace()
    config = load_ecosystem(root / "ecosystem.yml")
    _task_control(config, "bic", "inspect", "BIC-INSPECT-001")
    _task_control(config, "bic", "rewrite", "BIC-REWRITE-001")
...
def test_saw_continuation_returns_exact_next_unit_and_aggregate_action(make_workspace) -> None:
    """Advance one sequential plan only after each prior submission is finalized."""
    root = make_workspace()
    config = load_ecosystem(root / "ecosystem.yml")
...
def test_saw_continuation_detects_out_of_order_finalization(make_workspace) -> None:
    """Reject later finalized work when its predecessor is unfinished."""
    root = make_workspace()
    config = load_ecosystem(root / "ecosystem.yml")
    job, runtime = _saw_job(config)
    active_root = runtime.workflow("saw").output_root / "active"
