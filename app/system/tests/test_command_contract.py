...
def test_launcher_files_use_consistent_platform_forms(package_root: Path) -> None:
    ...
    attributes = (bundle_root / ".gitattributes").read_text(encoding="utf-8")
...
def test_root_launchers_are_thin_forwarders(package_root: Path, tmp_path: Path) -> None:
    ...
    bundle_root = package_root.parent
    posix = (bundle_root / "sage").read_text(encoding="utf-8")
    windows = (bundle_root / "sage.cmd").read_text(encoding="utf-8")
    assert 'exec "$APP_ROOT/system/bin/sage" "$@"' in posix
    assert 'call "%~dp0app\\system\\bin\\sage.cmd" %*' in windows

    isolated_root = tmp_path / "SAGE root with spaces"
    implementation = isolated_root / "app" / "system" / "bin" / "sage"
...
def test_launchers_delegate_runtime_creation_to_bootstrap(package_root: Path) -> None:
    ...
    assert "bootstrap_python.sh" in posix
