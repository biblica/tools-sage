...
def test_windows_launcher_passes_unc_roots_without_changing_cmd_directory(package_root: Path) -> None:
    """Pass an absolute UNC-capable root to PowerShell without asking cmd.exe to enter it."""
    launcher = (package_root / "system" / "bin" / "sage.cmd").read_text(encoding="utf-8")
    lowered = launcher.lower()
    assert '"%root_dir%" "%bootstrap_profile%" "launch" %*' in lowered
    assert "powershell.exe" in lowered
    assert "pushd" not in lowered and "popd" not in lowered
