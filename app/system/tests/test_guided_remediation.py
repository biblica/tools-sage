...
from sage.cli import _init_input_requirements, _resolve_grammar_override_input
from sage.errors import ConfigurationError
from sage.init_remediation import run_targeted_init_remediation
from sage.operator_overrides import operator_override_path
...
def test_interactive_initialize_persists_governed_sidecar_and_revalidates(
    ...
    """Verify that interactive initialization persists its sidecar and revalidates."""
    root = make_workspace(configured=False)
...
def test_init_requirements_are_limited_to_requested_projects(make_workspace, monkeypatch) -> None:
    ...
    root = make_workspace(configured=True)
    monkeypatch.setattr(
        "sage.cli.resolve_auto_settings",
        lambda config: [
            {
                "project_id": "idKKHv0",
...
def test_reset_preserves_operator_projects_and_removes_regenerable_system_state(make_workspace) -> None:
    """Runtime reset preserves operator Project data and clears regenerable .system state."""
    root = make_workspace(configured=False)
    layout = storage_layout(root, create=True)
    project_file = layout.projects_root / "local-project" / "41MAT.SFM"
    project_file.parent.mkdir(parents=True, exist_ok=True)
    project_file.write_text("\\id MAT\n", encoding="utf-8")
    ...
    assert project_file.read_text(encoding="utf-8") == "\\id MAT\n"
    assert not generated.exists()
    assert layout.cache_root.is_dir()


def test_reset_preserves_visible_operator_resources_and_local_configuration(make_workspace) -> None:
    """Runtime reset keeps operator resources and .system/config while clearing generated diagnostics."""
    root = make_workspace(configured=False)
    layout = storage_layout(root, create=True)
    resource = layout.resources_root / "notes" / "README.md"
    resource.parent.mkdir(parents=True, exist_ok=True)
    resource.write_text("# Local resource\n", encoding="utf-8")
    local_config = layout.config_root / "local-note.txt"
    local_config.write_text("keep\n", encoding="utf-8")
    generated = layout.diagnostics_root / "generated.json"
...
def test_provisional_grammar_does_not_prompt_or_gate_task_creation(make_workspace, monkeypatch) -> None:
    ...
    root = make_workspace(configured=True)
    profile_path = root / "system" / "config" / "profiles" / "grammar" / "en" / "bol-target.yml"
