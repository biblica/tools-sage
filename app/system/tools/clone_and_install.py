#!/usr/bin/env python3
"""Clone a portable SAGE bundle, bootstrap localdata, and optionally bind Paratext Projects."""
...
def _check_python_version() -> bool:
    """Return whether the invoking SAGE-managed Python meets the helper's minimum."""
...
def _app_root(bundle_root: Path) -> Path:
    """Return the immutable application root inside one portable bundle."""
    return bundle_root.resolve() / "app"


def _data_home(root: Path, requested: str | None) -> Path:
    """Resolve an explicit absolute data home or the bundle-local default."""
    if requested:
        ...
        data = root.resolve() / "localdata"
    app = _app_root(root)
    try:
        data.relative_to(app)
    except ValueError:
        pass
    else:
        raise ValueError("localdata must be outside the immutable SAGE app directory")
    return data


def _environment(data_home: Path, app_root: Path | None = None) -> dict[str, str]:
    """Build the deterministic bootstrap environment for the selected localdata root."""
    env = dict(os.environ)
    env[DATA_HOME_ENV] = str(data_home)
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    if app_root is not None:
        source = str(app_root / "system" / "src")
...
def _venv_python(data_home: Path) -> Path:
    """Return the platform-native interpreter inside localdata managed runtime."""
...
def _run_bootstrap(root: Path, data_home: Path) -> bool:
    """Run the standard SAGE bootstrap against the selected portable data home."""
    app = _app_root(root)
    launcher = app / ("sage-python.cmd" if os.name == "nt" else "sage-python")
    result = subprocess.run(
        [str(launcher), "-c", "pass"],
        cwd=app,
        env=_environment(data_home, app),
        check=False,
    )
...
def _configure_projects_root(root: Path, data_home: Path, projects_root: Path) -> bool:
    ...
    result = subprocess.run(
        [str(python), "-c", program, str(app), str(projects_root)],
        cwd=app,
        env=_environment(data_home, app),
        check=False,
    )
...
def _validate_installation(root: Path, data_home: Path) -> bool:
    ...
    env = _environment(data_home, app)
    imports = subprocess.run([str(python), "-c", program, str(app), str(data_home)], cwd=app, env=env, check=False)
    consistency = subprocess.run([str(python), "-m", "pip", "check"], cwd=app, env=env, check=False)
...
def _write_install_receipt(root: Path, data_home: Path, *, success: bool, mode: str, projects_root: Path | None) -> Path:
    """Write a machine-local installation receipt without modifying the app."""
    ...
    payload = {
        ...
        "operator_data_policy": "PRESERVE_EXISTING_RECOGNIZED_LOCALDATA",
    }
...
def _print_completion(root: Path, data_home: Path, mode: str, projects_root: Path | None, receipt: Path) -> None:
    ...
    print(f"Mode: {mode}")
    print(f"SAGE bundle: {root}")
    print(f"SAGE app: {_app_root(root)}")
    print(f"localdata: {data_home}")
    if projects_root:
    ...
    if platform.system() == "Windows":
        ...
        print(r"Python shell: .\app\sage-python.cmd")
    else:
        print(f"Next: cd {shlex.quote(str(root))}")
        print("Then: ./sage")
        print("Python shell: ./app/sage-python")
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--data-home", help="Absolute localdata location; default is <SAGE>/localdata")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not _persist_custom_data_home(target, data_home):
            print("ERROR: Could not persist the custom localdata location.", file=sys.stderr)
            return 1
