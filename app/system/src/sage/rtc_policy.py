"""Canonical RTC check selection and USFM-context policy."""
...
def _policy_code(workflow: str, suffix: str) -> str:
    """Return a canonical RTC code or the preserved code for a sealed legacy Job."""
    prefix = "SAW" if str(workflow).strip().lower() == "saw" else "RTC"
    return f"{prefix}_{suffix}"


def default_rtc_policy(profile_path: Path | None = None) -> dict[str, Any]:
    """Load governed RTC defaults, falling back to compiled defaults."""
    ...
    if profile_path and profile_path.is_file():
        ...
        drift = str(
            ol_section.get("source_text_drift_adjudication")
            or original_language["source_text_drift_adjudication"]
        ).upper()
    ...
    *,
    ...
    for key, value in dict(policy.get("usfm_contexts") or {}).items():
        ...
        if mode not in POLICY_MODES:
            raise ValidationError(
                f"Unsupported RTC text policy mode: {value}",
                code=_policy_code(workflow, "POLICY_MODE_INVALID"),
            )
    ...
    if ol_value is not None:
        ...
        if drift not in OL_DRIFT_STATES:
            raise ValidationError(
                f"Unsupported RTC original-language policy state: {ol_value}",
                code=_policy_code(workflow, "OL_POLICY_INVALID"),
            )
        original_language["source_text_drift_adjudication"] = drift
    return {
...
def should_elevate(
    *,
    ...
    """Return whether a detected issue becomes a finding; never alter severity."""
    mode = str(mode).upper()
    if mode == "NORMAL":
        return True
    ...
    if mode == "STRUCTURE_ONLY":
        return structural
    raise ValidationError(
        f"Unsupported RTC text policy mode: {mode}",
        code=_policy_code(workflow, "POLICY_MODE_INVALID"),
    )


def load_run_policy_snapshot(
    run_root: Path,
    *,
    profile_path: Path | None = None,
    workflow: str = "rtc",
) -> dict[str, Any]:
    """Load one immutable Run policy snapshot, or governed defaults when absent."""
    ...
    try:
        ...
        raise ValidationError(
            "Run check-policy snapshot is invalid",
            code=_policy_code(workflow, "POLICY_INVALID"),
        ) from exc
    if not isinstance(raw, dict):
        raise ValidationError(
            "Run check-policy snapshot must contain an object",
            code=_policy_code(workflow, "POLICY_INVALID"),
        )
    return validate_rtc_policy(raw, workflow=workflow)


def write_run_policy_snapshot(
    run_root: Path,
    policy: Mapping[str, Any],
    *,
    workflow: str = "rtc",
) -> Path:
    """Persist an immutable effective RTC policy snapshot inside one Run."""
    normalized = validate_rtc_policy(policy, workflow=workflow)
    path = run_root / "check-policy.json"
    if path.exists():
        existing = json.loads(path.read_text(encoding="utf-8"))
        if existing != normalized:
            raise ValidationError(
                "Run check-policy snapshot is immutable",
                code=_policy_code(workflow, "POLICY_IMMUTABLE"),
            )
        return path
