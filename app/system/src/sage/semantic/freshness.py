...
def _canonical_hash(payload: Any) -> str:
    """Hash one JSON-serializable semantic descriptor in canonical form."""
...
def semantic_input_descriptor(config: EcosystemConfig, language: str) -> dict[str, Any]:
    ...
    for authority_type in ("semdom", "folders"):
        spec = authority_source_for_type(config, authority_type)
        source_id = selection.get(spec.selection_key)
        if not source_id:
            continue
        path = authority_root(config) / spec.storage_directory / source_id / spec.content_file
        if not path.is_file():
            raise ValidationError(
                f"Selected {authority_type} authority source is missing: {source_id}"
            )
        authorities[spec.selection_key] = {
            "source_id": source_id,
