...
def semantic_root(config: EcosystemConfig) -> Path:
    ...
    return config.system_root / "indexes" / "semantic"
...
def set_authority_selection(
    ...
    *,
    ...
    key = spec.selection_key
    root = authority_root(config) / spec.storage_directory
    safe_source = safe_id(source_id, "source_id")
...
def ensure_authority_selected(
    ...
    *,
    ...
    key = authority_source_for_type(config, authority_type).selection_key
    active = load_authority_selection(config)
    if key in active:
        return active
