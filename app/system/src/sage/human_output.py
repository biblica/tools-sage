...
class HumanOutputSpec:
    ...
    operator_language_policy: "OperatorLanguagePolicy"
...
class OperatorLanguagePolicy:
    ...
    pilot_only: tuple[str, ...]

    def selectable(self) -> tuple[str, ...]:
        """Return approved and locally enabled evaluation candidates in stable order."""
        return tuple(dict.fromkeys((*self.approved, *self.candidates)))

    def status(self, language: str) -> str:
        """Return the effective local governance status for one canonical tag."""
        if language in self.approved:
            return "APPROVED"
        if language in self.candidates:
            return "CANDIDATE"
        if language in self.pilot_only:
            return "PILOT_ONLY"
        return "UNLISTED"


_DEFAULT_OPERATOR_LANGUAGE_POLICY = {
    "approved": ("en",),
...
_CATALOGUE: dict[str, dict[str, str]] = {
    "en": {
        ...
        "report.saw_action_report": "Legacy Analysis Action Report",
        "report.rtc_action_report": "RTC Action Report",
        "report.stc_action_report": "STC Action Report",
        "report.actionable_findings": "Actionable findings",
        "report.consolidation": "Consolidation",
        "label.stage": "Stage",
        "label.coverage": "Coverage",
        "label.action_level": "Action level",
        "label.confidence": "Confidence",
        "label.issue": "Issue",
        "label.required_action": "Proposed action",
        "label.proposed_action": "Proposed action",
        "label.evidence_ids": "Evidence IDs",
        "label.grammar_rule_ids": "Grammar rule IDs",
        "label.original_language_evidence": "Original-language",
        "message.no_action_required": "No action required.",
        "message.no_actionable_findings": "No actionable findings were submitted.",
        "message.secondary_rendering_unavailable": "Secondary report rendering is unavailable; the governing primary report remains valid.",
        ...
        "urgency.3": "Urgent",
        "urgency.4": "Critical",
        "report.ecosystem_initialisation": "SAGE ecosystem initialization report",
        ...
        "report.initialisation_result": "SAGE initialization result",
        ...
        "label.largest_estimated_tokens": "Largest estimated routed-SFM tokens",
        "label.largest_serialized_bytes": "Largest routed-SFM bytes",
        ...
        "report.auto_resolution_edit": "Operators may replace any `auto` value with an explicit value and rerun initialization.",
        ...
        "prompt.value_not_recognised": "{label} {received} was not recognized.",
    ...
    "uk": {
        "report.saw_action_report": "Звіт застарілого аналізу про необхідні дії",
        "report.rtc_action_report": "Звіт RTC про необхідні дії",
        "report.stc_action_report": "Звіт STC про необхідні дії",
        "report.actionable_findings": "Висновки, що потребують дій",
        "report.consolidation": "Консолідація",
        "label.task": "Завдання",
        "label.operation": "Операція",
        "label.stage": "Етап",
        "label.scope": "Діапазон",
        "label.coverage": "Охоплення",
        "label.status": "Статус",
        "label.category": "Категорія",
        "label.action_level": "Рівень дії",
        "label.confidence": "Впевненість",
        "label.issue": "Проблема",
        "label.required_action": "Необхідна дія",
        "label.proposed_action": "Запропонована дія",
        "label.evidence_ids": "Ідентифікатори доказів",
        "label.grammar_rule_ids": "Ідентифікатори граматичних правил",
        "label.original_language_evidence": "Мова оригіналу",
        "message.no_action_required": "Дія не потрібна.",
        "message.no_actionable_findings": "Висновків, що потребують дій, не подано.",
        "message.secondary_rendering_unavailable": "Вторинний переклад звіту недоступний; основний звіт залишається чинним.",
        "label.report_languages": "Мови звіту",
    },
    "id": {
        ...
        "report.project_registry": "Inventaris Proyek SAGE",
        ...
        "label.largest_estimated_tokens": "Token SFM terarah perkiraan terbesar",
        "label.largest_serialized_bytes": "Byte SFM terarah terbesar",
        ...
        "prompt.project_disabled": "Proyek {project_id} sudah ada di SAGE tetapi dinonaktifkan dalam konfigurasi efektif.",
...
def _operator_language_list(value: Any, label: str, default: tuple[str, ...]) -> tuple[str, ...]:
    """Parse one ordered Operator-language policy list without accepting aliases."""
    raw = default if value is None else value
    if not isinstance(raw, (list, tuple)):
        raise ConfigurationError(f"{label} must be a list of canonical language tags")
    result = tuple(canonical_language_tag(str(item), label) for item in raw)
    if len(result) != len(set(result)):
        raise ConfigurationError(f"{label} must not contain duplicate language tags")
    return result


def parse_human_output(raw: Any) -> HumanOutputSpec:
    """Parse the global Operator language and effective report channels."""
    ...
    policy_raw = require_mapping(
        root.get("operator_language_policy", {}),
        "human_output.operator_language_policy",
    )
    operator_language_policy = OperatorLanguagePolicy(
        approved=_operator_language_list(
            policy_raw.get("approved"),
            "human_output.operator_language_policy.approved",
            _DEFAULT_OPERATOR_LANGUAGE_POLICY["approved"],
        ),
        candidates=_operator_language_list(
            policy_raw.get("candidates"),
            "human_output.operator_language_policy.candidates",
            _DEFAULT_OPERATOR_LANGUAGE_POLICY["candidates"],
        ),
        operational_priorities=_operator_language_list(
            policy_raw.get("operational_priorities"),
            "human_output.operator_language_policy.operational_priorities",
            _DEFAULT_OPERATOR_LANGUAGE_POLICY["operational_priorities"],
        ),
        pilot_only=_operator_language_list(
            policy_raw.get("pilot_only"),
            "human_output.operator_language_policy.pilot_only",
            _DEFAULT_OPERATOR_LANGUAGE_POLICY["pilot_only"],
        ),
    )
    if "en" not in operator_language_policy.approved:
        raise ConfigurationError(
            "human_output.operator_language_policy.approved must retain canonical fallback 'en'"
        )
    invalid_priorities = tuple(
        language
        for language in operator_language_policy.operational_priorities
        if language not in operator_language_policy.candidates
    )
    if invalid_priorities:
        raise ConfigurationError(
            "human_output.operator_language_policy.operational_priorities must be enabled "
            "candidates: " + ", ".join(invalid_priorities)
        )
    if operator_language not in operator_language_policy.selectable():
        raise ConfigurationError(
            f"human_output.operator_language {operator_language!r} is not approved or enabled "
            "as a candidate; an advanced Operator must add it to "
            "human_output.operator_language_policy.candidates first"
        )
    ...
    return HumanOutputSpec(
        ...
        operator_language_policy=operator_language_policy,
        logs_and_reports=OutputLanguageChannel(
            ...
            secondary_language=_optional_language_token(
                logs.get("secondary_language"),
                "human_output.logs_and_reports.secondary_language",
            ),
            bilingual=bool(logs.get("bilingual", False)),
        ),
        translation_challenges=TranslationChallengeChannel(
            primary_language=_language_token(
                challenges.get("primary_language", "OPERATOR_LANGUAGE"),
                "human_output.translation_challenges.primary_language",
            ),
            secondary_language=_optional_language_token(
                challenges.get("secondary_language"),
                "human_output.translation_challenges.secondary_language",
            ),
            bilingual=bool(challenges.get("bilingual", False)),
...
def report_language_authority(
    ...
    *,
    ...
    """Return the governed authority record required for bilingual reports."""
    ...
    if len(languages) < 2:
        return None
    secondary = next((language for language in languages if language != operator_language), None)
...
def render_report_language_authority(
    authority: Mapping[str, Any] | None,
    *,
    markdown: bool,
) -> str:
    """Render the mandatory bilingual-report confidence and authority notice."""
    if not authority:
        return ""
    ...
    notice = (
        ...
        "taking action. Producing it adds model usage and report compilation time, and it requires "
        "more human review than a single-language report. Canonical machine records, reason codes, "
        "evidence IDs, and Scripture "
        "coordinates remain authoritative."
    )
    return f"> **{notice}**" if markdown else notice.replace("`", "")


# NCA report labels share the existing human-output catalog; machine tokens stay unchanged.
_NCA_REPORT_LABELS = {'en': ['SAGE NUMBERS CHECK',
        'Authority',
        'Summary',
        'Coverage',
        'Limitations',
        "Findings are limited by the selected LLM's language understanding and numeric-interpretation "
        'capabilities. SQS confidence checks have not been applied.',
        'Checks',
        'Units',
        'Findings',
        ...
        'Restrictions',
        'Model identity'],
 'id': ['SAGE NUMBERS CHECK',
        'Otoritas',
        'Ringkasan',
        'Cakupan',
        'Keterbatasan',
        'Temuan dibatasi oleh kemampuan pemahaman bahasa dan penafsiran angka LLM yang dipilih. Pemeriksaan '
        'keyakinan SQS belum diterapkan.',
        'Pemeriksaan',
        'Unit',
        'Temuan',
        'Tidak ada temuan yang dihasilkan.',
        'Referensi target',
        'Lokasi target',
        'Referensi Barat',
        'Referensi OL',
        'Bacaan yang dipilih',
        'ID sumber',
        'Tindakan catatan kaki',
        'Status catatan kaki',
        'Catatan yang disarankan',
        'Kode',
        'Kategori',
        'Hasil',
        'Pembatasan',
        'Identitas model'],
 'fr': ['SAGE NUMBERS CHECK',
        'Autorité',
        'Résumé',
        'Couverture',
        'Limites',
        'Les constatations sont limitées par les capacités du LLM choisi à comprendre la langue et à '
        'interpréter les nombres. Les contrôles de confiance SQS n’ont pas été appliqués.',
        'Contrôles',
        'Unités',
        'Constatations',
        'Aucune constatation émise.',
        'Référence cible',
        'Emplacement cible',
        'Référence occidentale',
        'Référence OL',
        'Lecture choisie',
        ...
        'Restrictions',
        'Identité du modèle'],
 'ru': ['SAGE NUMBERS CHECK',
        'Авторитетный источник',
        'Сводка',
        'Охват',
        'Ограничения',
        'Выводы ограничены способностью выбранной LLM понимать язык и интерпретировать числа. Проверки '
        'уверенности SQS не применялись.',
        'Проверки',
        'Единицы',
        'Замечания',
        'Замечания не сформированы.',
        'Ссылка целевого текста',
        'Расположение в целевом тексте',
        'Западная ссылка',
        'Ссылка OL',
        'Выбранное чтение',
 ...
 'pt-BR': ['SAGE NUMBERS CHECK',
           'Autoridade',
           'Resumo',
           'Cobertura',
           'Limitações',
           'Os resultados são limitados pela capacidade do LLM selecionado de compreender a língua e '
           'interpretar números. As verificações de confiança SQS não foram aplicadas.',
           'Verificações',
           'Unidades',
           'Constatações',
           'Nenhuma constatação foi emitida.',
           'Referência do alvo',
           'Localização no alvo',
           'Referência ocidental',
           'Referência OL',
           'Leitura escolhida',
           'IDs das fontes',
           'Ação da nota',
 ...
 'uk': ['SAGE NUMBERS CHECK',
        'Авторитетне джерело',
        'Підсумок',
        'Охоплення',
        'Обмеження',
        'Висновки обмежені здатністю вибраної LLM розуміти мову та інтерпретувати числа. Перевірки '
        'впевненості SQS не застосовувалися.',
        'Перевірки',
        'Одиниці',
        'Зауваження',
        'Зауважень не сформовано.',
        'Посилання цільового тексту',
        'Розташування в цільовому тексті',
        'Західне посилання',
        'Посилання OL',
        'Вибране читання',
        'Ідентифікатори джерел',
        'Дія з приміткою',
        'Стан примітки',
        'Запропонована примітка',
        'Код',
        'Категорія',
        'Результат',
        'Обмеження',
        'Ідентифікація моделі'],
 'en-US': ['SAGE NUMBERS CHECK',
           'Authority',
           'Summary',
           'Coverage',
           'Limitations',
 ...
 'en-GB': ['SAGE NUMBERS CHECK',
           ...
           'Restrictions',
           'Model identity']}
_NCA_REPORT_KEYS = ('title', 'authority', 'summary', 'coverage', 'limitations', 'capability_limitation', 'checks', 'units', 'findings', 'no_findings', 'target_reference', 'target_locator', 'western_reference', 'ol_reference', 'selected_reading', 'source_ids', 'footnote_action', 'footnote_status', 'suggested_note', 'code', 'category', 'outcome', 'restrictions', 'model_identity')
for _nca_locale, _nca_labels in _NCA_REPORT_LABELS.items():
...
_NCA_OPTIMIZED_LABELS = {
    'chapters': {'en': 'WIP chapters', 'id': 'Bab WIP', 'fr': 'Chapitres WIP', 'ru': 'Главы WIP', 'pt-BR': 'Capítulos WIP', 'uk': 'Розділи WIP'},
    'unlocated': {'en': 'Unlocated WIP coverage', 'id': 'Cakupan WIP tanpa lokasi', 'fr': 'Couverture sans emplacement WIP', 'ru': 'Покрытие без расположения WIP', 'pt-BR': 'Cobertura WIP sem localização', 'uk': 'Покриття без розташування WIP'},
    'alignment': {'en': 'Alignment', 'id': 'Penyelarasan', 'fr': 'Alignement', 'ru': 'Сопоставление', 'pt-BR': 'Alinhamento', 'uk': 'Зіставлення'},
    'cross_reference': {'en': 'See primary evidence', 'id': 'Lihat bukti utama', 'fr': 'Voir les éléments principaux', 'ru': 'См. основные данные', 'pt-BR': 'Ver evidências principais', 'uk': 'Див. основні дані'},
    'parent_group': {'en': 'Protected parent group', 'id': 'Kelompok induk terlindungi', 'fr': 'Groupe parent protégé', 'ru': 'Защищённая родительская группа', 'pt-BR': 'Grupo pai protegido', 'uk': 'Захищена батьківська група'},
    'planned_inputs': {'en': 'Planned extraction inputs', 'id': 'Masukan ekstraksi terencana', 'fr': 'Entrées d’extraction prévues', 'ru': 'Запланированные входы извлечения', 'pt-BR': 'Entradas de extração planejadas', 'uk': 'Заплановані входи видобування'},
    'planned_extraction_calls': {'en': 'Planned extraction calls (initial batches)', 'id': 'Panggilan ekstraksi terencana (batch awal)', 'fr': 'Appels d’extraction prévus (lots initiaux)', 'ru': 'План вызовов извлечения (исходные пакеты)', 'pt-BR': 'Chamadas de extração planejadas (lotes iniciais)', 'uk': 'План викликів видобування (початкові пакети)'},
    'executed_calls': {'en': 'Executed calls', 'id': 'Panggilan terlaksana', 'fr': 'Appels exécutés', 'ru': 'Выполненные вызовы', 'pt-BR': 'Chamadas executadas', 'uk': 'Виконані виклики'},
    'reused_checkpoints': {'en': 'Reused checkpoints', 'id': 'Titik pemeriksaan digunakan kembali', 'fr': 'Points de reprise réutilisés', 'ru': 'Повторно использованные контрольные точки', 'pt-BR': 'Pontos de controle reutilizados', 'uk': 'Повторно використані контрольні точки'},
    'blocked_inputs': {'en': 'Blocked extraction inputs', 'id': 'Masukan ekstraksi terhalang', 'fr': 'Entrées d’extraction bloquées', 'ru': 'Заблокированные входы извлечения', 'pt-BR': 'Entradas de extração bloqueadas', 'uk': 'Заблоковані входи видобування'},
    'missing_owners': {'en': 'Groups without extraction', 'id': 'Kelompok tanpa ekstraksi', 'fr': 'Groupes sans extraction', 'ru': 'Группы без извлечения', 'pt-BR': 'Grupos sem extração', 'uk': 'Групи без видобування'},
    'scope_expansion': {'en': 'Scope expansion', 'id': 'Perluasan cakupan', 'fr': 'Extension du périmètre', 'ru': 'Расширение области', 'pt-BR': 'Ampliação do escopo', 'uk': 'Розширення області'},
    'requested_scope': {'en': 'Requested scope', 'id': 'Cakupan diminta', 'fr': 'Périmètre demandé', 'ru': 'Запрошенная область', 'pt-BR': 'Escopo solicitado', 'uk': 'Запитана область'},
    'row_evidence': {'en': 'Exact row evidence', 'id': 'Bukti baris persis', 'fr': 'Éléments exacts de la ligne', 'ru': 'Точные данные строки', 'pt-BR': 'Evidências exatas da linha', 'uk': 'Точні дані рядка'},
    'indexed_coordinates': {'en': 'Indexed coordinates', 'id': 'Koordinat terindeks', 'fr': 'Coordonnées indexées', 'ru': 'Индексированные координаты', 'pt-BR': 'Coordenadas indexadas', 'uk': 'Індексовані координати'},
    'unindexed_coordinates': {'en': 'Unindexed coordinates', 'id': 'Koordinat tidak terindeks', 'fr': 'Coordonnées non indexées', 'ru': 'Неиндексированные координаты', 'pt-BR': 'Coordenadas não indexadas', 'uk': 'Неіндексовані координати'},
    'extraction_complete': {'en': 'Complete extractions', 'id': 'Ekstraksi lengkap', 'fr': 'Extractions complètes', 'ru': 'Полные извлечения', 'pt-BR': 'Extrações completas', 'uk': 'Повні видобування'},
    'extraction_partial': {'en': 'Partial extractions', 'id': 'Ekstraksi sebagian', 'fr': 'Extractions partielles', 'ru': 'Частичные извлечения', 'pt-BR': 'Extrações parciais', 'uk': 'Часткові видобування'},
    'extraction_unsupported': {'en': 'Unsupported extractions', 'id': 'Ekstraksi tidak didukung', 'fr': 'Extractions non prises en charge', 'ru': 'Неподдерживаемые извлечения', 'pt-BR': 'Extrações não suportadas', 'uk': 'Непідтримувані видобування'},
    'unresolved_targets': {'en': 'Unresolved target expressions', 'id': 'Ungkapan sasaran belum terselesaikan', 'fr': 'Expressions cibles non résolues', 'ru': 'Неразрешённые целевые выражения', 'pt-BR': 'Expressões do texto-alvo não resolvidas', 'uk': 'Нерозв’язані цільові вирази'},
    'uncertain_navigation': {'en': 'Row ownership is unresolved; use the protected WIP range.', 'id': 'Kepemilikan baris belum jelas; gunakan rentang WIP terlindungi.', 'fr': 'L’attribution des lignes est incertaine ; utiliser la plage WIP protégée.', 'ru': 'Принадлежность строк не установлена; используйте защищённый диапазон WIP.', 'pt-BR': 'A atribuição das linhas está indefinida; use o intervalo WIP protegido.', 'uk': 'Належність рядків не встановлена; використовуйте захищений діапазон WIP.'},
    'unlocated_navigation': {'en': 'No proven WIP location; Western coordinates below identify coverage only.', 'id': 'Lokasi WIP belum terbukti; koordinat Barat berikut hanya menunjukkan cakupan.', 'fr': 'Aucun emplacement WIP établi ; les coordonnées occidentales ci-dessous indiquent seulement la couverture.', 'ru': 'Расположение WIP не установлено; западные координаты ниже обозначают только покрытие.', 'pt-BR': 'Sem localização WIP comprovada; as coordenadas ocidentais abaixo indicam apenas cobertura.', 'uk': 'Розташування WIP не встановлено; західні координати нижче позначають лише покриття.'},
    'planning_notice': {'en': 'Planning estimates describe initial extraction batches, not measured time, cost or model qualification. Semantic calls and retries depend on evidence.', 'id': 'Perkiraan rencana menjelaskan batch ekstraksi awal, bukan waktu, biaya terukur atau kualifikasi model. Panggilan semantik dan percobaan ulang bergantung pada bukti.', 'fr': 'Les estimations décrivent les lots d’extraction initiaux, sans mesurer durée, coût ou qualification du modèle. Les appels sémantiques et les nouvelles tentatives dépendent des éléments disponibles.', 'ru': 'Плановые оценки описывают исходные пакеты извлечения, а не измеренное время, стоимость или квалификацию модели. Семантические вызовы и повторы зависят от данных.', 'pt-BR': 'As estimativas descrevem lotes iniciais de extração, sem medir tempo, custo ou qualificação do modelo. Chamadas semânticas e novas tentativas dependem das evidências.', 'uk': 'Планові оцінки описують початкові пакети видобування, а не виміряний час, вартість чи кваліфікацію моделі. Семантичні виклики й повтори залежать від даних.'},
    'preflight': {'en': 'Scoped preflight', 'id': 'Pemeriksaan awal cakupan', 'fr': 'Vérification préalable du périmètre', 'ru': 'Предварительная проверка области', 'pt-BR': 'Verificação prévia do escopo', 'uk': 'Попередня перевірка області'},
    'reference_expectations': {'en': 'Reference expectations (Western)', 'id': 'Ekspektasi rujukan (Barat)', 'fr': 'Attentes de référence (occidentales)', 'ru': 'Ожидания по эталону (западные)', 'pt-BR': 'Expectativas de referência (ocidentais)', 'uk': 'Очікування за еталоном (західні)'},
    'protected_groups': {'en': 'Protected groups', 'id': 'Kelompok terlindungi', 'fr': 'Groupes protégés', 'ru': 'Защищённые группы', 'pt-BR': 'Grupos protegidos', 'uk': 'Захищені групи'},
    'mandatory_profile': {'en': 'Number Style Profile (required even with presentation OFF)', 'id': 'Profil Gaya Angka (wajib meski penyajian OFF)', 'fr': 'Profil de style numérique (requis même avec présentation OFF)', 'ru': 'Профиль оформления чисел (обязателен даже при оформлении OFF)', 'pt-BR': 'Perfil de estilo numérico (obrigatório mesmo com apresentação OFF)', 'uk': 'Профіль оформлення чисел (обов’язковий навіть за оформлення OFF)'},
    'input_language': {'en': 'Input language and script', 'id': 'Bahasa dan aksara masukan', 'fr': 'Langue et écriture d’entrée', 'ru': 'Язык и письменность входа', 'pt-BR': 'Idioma e escrita de entrada', 'uk': 'Мова й писемність входу'},
    'failed_calls': {'en': 'Failed calls', 'id': 'Panggilan gagal', 'fr': 'Appels échoués', 'ru': 'Неудачные вызовы', 'pt-BR': 'Chamadas com falha', 'uk': 'Невдалі виклики'},
    'exact_evidence': {'en': 'Exact evidence', 'id': 'Bukti persis', 'fr': 'Éléments exacts', 'ru': 'Точные данные', 'pt-BR': 'Evidências exatas', 'uk': 'Точні дані'},
}
for _nca_key, _nca_translations in _NCA_OPTIMIZED_LABELS.items():
    for _nca_locale in ("en", "en-US", "en-GB", "id", "fr", "ru", "pt-BR", "uk"):
        _CATALOGUE.setdefault(_nca_locale, {})["report.nca." + _nca_key] = _nca_translations[
            "en" if _nca_locale in {"en-US", "en-GB"} else _nca_locale]
...
def message_for_languages(
    ...
    *,
    ...
    """Render model-supplied localized challenge text without inventing missing wording."""
    values: list[str] = []
...
_CONSOLE_EXACT_KEYS = {
    ...
    "SAGE INITIALIZATION RESULT": "report.initialisation_result",
...
_CONSOLE_PREFIX_KEYS = {
    ...
    "Largest estimated routed-SFM tokens": "label.largest_estimated_tokens",
    "Largest routed-SFM bytes": "label.largest_serialized_bytes",
...
def paired_label_value_text(
    ...
    *,
    ...
    """Render one labeled value in every configured language without altering the value."""
    languages = resolved_languages(
        channel,
        operator_language=operator_language,
        source_language=source_language,
...
class LocalizedConsoleStream:
    """Localize recognized report headings and labels while preserving canonical values."""
    ...
    def write(self, text: str) -> int:
        """Write text after localizing complete human-facing report lines."""
        self._buffer += text
        while "\n" in self._buffer:
    ...
    def _render_line(self, line: str) -> str:
        """Localize one recognized line and leave commands, JSON, IDs, and diagnostics intact."""
...
class OperationalLogger:
    """Append canonical JSONL events and render concise localized console lines."""
    ...
    def __init__(
        ...
        *,
        ...
        self.target_language = target_language
        self.reference_language = reference_language
        self.path = storage_layout(root).logs_root / "operational.jsonl"
        self.human_path = storage_layout(root).logs_root / "operational.log"
    ...
    def emit(
        ...
        *,
        ...
        """Write one canonical event and optionally print one readable localized line."""
