...
USJ_VERSION = "3.1"
USJ_COMPILER = "SAGE-USJ-COMPILER-2"
...
NOTE_FIELD_MARKERS = {
    "fr", "ft", "fk", "fq", "fqa", "fl", "fw", "fp", "fv", "fdc", "fm",
            ...
            nodes.append(
                {
                    "type": "note",
                    "marker": marker,
                    "content": _parse_note_content(inner, errors),
                }
            )
            continue
...
def _parse_note_content(text: str, errors: list[str]) -> list[Any]:
    # Note fields are implicit siblings; explicitly paired nested character styles
    # remain children of the active field. This prevents punctuation in a note from
    # being mistaken for punctuation in the surrounding verse stream.
    """Parse note fields and nested character styles without leaking them into body text."""
    nodes: list[Any] = []
    active: list[Any] = nodes
    ...
    while cursor < len(text):
        ...
        if not match:
            if cursor < len(text):
                active.append(text[cursor:])
            break
        if match.start() > cursor:
            active.append(text[cursor:match.start()])
        raw_marker = match.group(1)
        marker = _clean_marker(raw_marker)
        closing = bool(match.group(2))
        content_start = _consume_delimiter(text, match.end())
        if closing:
            # USFM 3.1 permits both implicit and explicit closure for structural
            # footnote/cross-reference fields (for example \fr ... \fr*).
            if marker in NOTE_FIELD_MARKERS and active_field == marker:
                active = nodes
                active_field = None
            else:
                errors.append(f"UNEXPECTED_NOTE_CLOSING_MARKER:{marker}")
            cursor = match.end()
            continue
        if not raw_marker.startswith("+") and marker in NOTE_FIELD_MARKERS:
            field = {"type": "char", "marker": marker, "content": []}
            nodes.append(field)
        ...
        if marker in PAIRED_CHAR_MARKERS:
            close = _find_close(text, content_start, marker)
            if not close:
                errors.append(f"UNCLOSED_NOTE_CHARACTER_MARKER:{marker}")
                inner = text[content_start:]
                cursor = len(text)
            else:
                inner = text[content_start:close[0]]
                cursor = close[1]
            active.append(
                {"type": "char", "marker": marker, "content": _parse_inline(inner, errors)}
            )
            continue
        active.append({"type": "marker", "marker": marker, "closing": False})
        cursor = content_start
    return _merge_adjacent_text(nodes)
...
def compile_usfm_text(
    ...
    *,
    ...
    # Parse document structure separately from verse records. Inline parser errors inside
    # verses are re-emitted below with exact Scripture coordinates; retaining the broad
    # duplicate would make every bounded section appear defective.
    top_level_errors: list[str] = []
    content = _build_top_level_usj(text, book_code, top_level_errors)
    ...
    coordinated_tails = {
        item.split(":", 2)[2]
        for item in errors
        if item.startswith(f"{book_code} ") and item.count(":") >= 2
    }
    errors.extend(item for item in top_level_errors if item not in coordinated_tails)
