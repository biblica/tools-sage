"""Deterministic Paratext/PTLite short-name parsing for SAGE."""
...
def project_code_is_path_safe(project_code: str) -> bool:
    """Return whether a Paratext short name is safe as a path component on all supported hosts."""
    code = str(project_code).strip()
    if not _PROJECT_CODE_RE.fullmatch(code) or code.endswith((" ", ".")):
        return False
    return code.split(".", 1)[0].upper() not in _WINDOWS_RESERVED_BASENAMES
...
class ProjectCodeParts:
    ...
    def to_dict(self) -> dict[str, object]:
        """Return stable serializable structural metadata for Project Inventory persistence."""
        return {
...
def parse_project_code(
    ...
    *,
    ...
    """
    code = str(project_code).strip()
    if not project_code_is_path_safe(code):
