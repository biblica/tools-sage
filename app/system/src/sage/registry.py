...
from .storage import StorageError, storage_layout, resolve_declared_path
...
def _resolve_projects_root(root: Path, value: str) -> Path:
    """Resolve the governed internal Projects root through the storage contract."""
    try:
        return resolve_declared_path(root, value, "paths.internal_scripture_root")
    ...
    for raw_id, raw_value in data.items():
        ...
        for index, raw_entry in enumerate(entries_raw):
            ...
            if not contemporary_roles.intersection(source.scope.roles):
                raise ConfigurationError(
                    f"Evaluation source {contemporary_source} lacks an authorized contemporary role"
                )
...
def load_ecosystem(settings_path: Path) -> EcosystemConfig:
    ...
    cache_root = resolve_workspace_path(
        root,
        require_string(paths.get("cache_root", "@system/cache"), "paths.cache_root"),
        "paths.cache_root",
    )
    runtime_state_root = resolve_workspace_path(
        root,
        require_string(paths.get("runtime_state_root", "@system"), "paths.runtime_state_root"),
    ...
    default_file = require_string(
        versification.get("default_file", "eng.vrs"),
        "versification.default_file",
    )
    if Path(default_file).suffix.lower() != ".vrs" or len(Path(default_file).parts) != 1:
        raise ConfigurationError("versification.default_file must be one .vrs filename")
    if default_file.casefold() not in base_vrs_files:
        raise ConfigurationError("versification.default_file must be listed in versification.base_files")
    ...
    return EcosystemConfig(
        ...
        system_root=layout.system_root,
        ...
        default_versification=default_file,
