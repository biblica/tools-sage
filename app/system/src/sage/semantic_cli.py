...
from .semantic.authority_registry import authority_source_for_type
...
def command_rwc_authority_semdom(args: argparse.Namespace) -> int:
    ...
    config = _config(args)
    source_id = args.source_id or authority_source_for_type(config, "semdom").default_source_id
    result = import_semdom_authority_json(config, Path(args.file), source_id=source_id)
...
def command_rwc_authority_folders(args: argparse.Namespace) -> int:
    ...
    config = _config(args)
    source_id = args.source_id or authority_source_for_type(config, "folders").default_source_id
    result = import_specific_first_docx(config, Path(args.file), source_id=source_id)
...
def command_rwc_initialise(args: argparse.Namespace) -> int:
    ...
    result["rule"] = "Initialization binds explicit project IDs to semantic namespaces and builds only from active local inputs."
    return _emit(args, "SAGE RWC INITIALIZATION", result)
...
def register_rwc_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    initialise = actions.add_parser("initialize", help="Bind project/resource IDs and build current local indexes")
    ...
    semdom.add_argument("--source-id", help="Override the governed default source ID")
    ...
    folders.add_argument("--source-id", help="Override the governed default source ID")
    ...
    imports = actions.add_parser("import", help="Import immutable governed project-index snapshots")
