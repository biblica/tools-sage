...
def mounts_path(root: Path) -> Path:
    ...
    return storage_layout(root).state_root / "resource-mounts.json"


def normalize_operator_path(value: str) -> str:
    """Normalize a path pasted from a shell without requiring shell quoting rules."""
...
def interpret_operator_project_location(project_id: str, value: str) -> tuple[Path, Path | None, str | None]:
    ...
    Surrounding shell quotes and escaped Unix spaces are normalized first.
    """
...
def _absolute_directory(value: str, label: str, *, require_exists: bool = False) -> str:
    """Normalize one configured external directory and optionally require it to exist."""
    path = Path(normalize_operator_path(value)).expanduser()
    if not path.is_absolute():
...
def discover_project_folders(project_root: Path) -> tuple[str, ...]:
    """Return immediate subfolders containing the Paratext ``settings.xml`` marker.

    Discovery is intentionally tree-only. File content validation belongs to Project
    inspection or a Full Scan, not to root discovery.
    """
...
def set_project_root(
    ...
    *,
    ...
    """Persist the one absolute Paratext/PTLite Projects root and refresh its catalog."""
    ...
    # Selecting a Projects root immediately builds only the lightweight discovery catalog.
    from .paratext_catalog import scan_paratext_projects
    scan_paratext_projects(root, value, full=False, progress=progress)
    return destination
...
def invalidate_runtime_settings(root: Path) -> int:
    """Remove derived Job controller settings so the next controller call rebuilds them.

    Job manifests and operator-facing outputs live under ``@jobs``. Runtime settings and
    the derived workflow profile are controller-owned machine state under
    ``@system/jobs`` and may contain host-specific resource paths. Any resource-mount or
    local-setting change therefore invalidates both derived files without touching the
    durable Job itself.
    """
    count = 0
    job_state_root = storage_layout(root).system_root / "jobs"
    for pattern in ("*/*/runtime.yml", "*/*/profile.yml"):
