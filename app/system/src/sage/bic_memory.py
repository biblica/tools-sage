...
def validate_inspect_submission(
    ...
    *,
    ...
    # Keep proposal validation separate from human approval; model output cannot authorize memory use.
...
def _materialized_approved_memory(memory_root: Path) -> list[dict[str, Any]]:
    """Build approved memory only from verified BIC INSPECT provenance."""
    approved: list[dict[str, Any]] = []
    seen: set[str] = set()
    for source_label, path in _memory_sources(memory_root):
        if source_label != "INSPECT":
            continue
        for record in eligible_memory_records(_read_list(path)):
            provenance = record.get("provenance") or {}
            if str(provenance.get("submission_source", "")).upper() != "BIC_INSPECT":
                continue
...
def transition_memory_record_transactionally(
    *,
    ...
    # Maintenance invariant: governance state may review imports, but only INSPECT provenance can gain content authority.
    ...
    with WorkspaceLock(lock_path, "BIC_MEMORY_STATE_TRANSITION"):
        ...
        if normalized_target == "APPROVED_FOR_USE":
            if source_label != "INSPECT":
                raise MemoryGovernanceError(
                    "Generic lexicon imports cannot be promoted into BIC content evidence",
                    code="LEXICON_IMPORT_CONTENT_AUTHORITY_PROHIBITED",
                )
            provenance = record.get("provenance") or {}
            resource_fingerprints = provenance.get("resource_fingerprints") or {}
            if (
                str(provenance.get("submission_source", "")).upper() != "BIC_INSPECT"
                or not str(record.get("bic_job_id", "")).strip()
                or not isinstance(resource_fingerprints, dict)
                or not resource_fingerprints
            ):
                raise MemoryGovernanceError(
                    "BIC memory requires same-Job INSPECT provenance and resource fingerprints before approval",
                    code="MEMORY_CONTENT_PROVENANCE_REQUIRED",
                )
        ...
        normalized_entries.append(
            {
                ...
                "provenance": {
                    ...
                    "content_authority": "PROHIBITED",
                    "governance_use": "REVIEW_ONLY",
                    "import_id": import_id,
                    "source": source,
