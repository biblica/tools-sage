...
Feature maturity is separate from the product release phase. An Alpha, Beta, RC, or approved release may
...
- A product-version change must preserve recognized `localdata` Operator, Project, Job, Run, report,
  ...
  `localdata/.system/runtime/python` and `venv` are fingerprinted and repaired/rebuilt when runtime or dependency contracts change.
