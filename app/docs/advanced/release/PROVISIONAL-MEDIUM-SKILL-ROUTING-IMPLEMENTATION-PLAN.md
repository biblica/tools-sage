...
**Goal:** Make an Alpha build execute governed Skills through a truthful Medium provisional route when, and only when, no current qualification evidence exists, while preserving qualified-route precedence and adverse/stale evidence blocking.

**Architecture:** Extend the deterministic Python route resolver with an explicit no-data state and an Alpha-only policy branch. Keep exact qualification and manual override logic separate from provisional routing. Project the selected basis into CLI/menu displays, execution receipts, reports, and a pre-`Working` SAW preflight so `CONTINUE RUN` can retry the same sealed task.
...
- Work on `alpha/0.02alpha1`; do not merge to `main`.
...
- Provisional routing is enabled only for the governed Alpha release state.
...
## Task 1: Add the Alpha no-data policy contract
...
1. Add failing tests for the Alpha provisional policy and implicit Codex Medium default.
2. Run the focused tests and confirm the new cases fail for missing behavior.
3. Add `provisional_routing` to the shipped policy and schema, including Alpha state, status label, provider default, prohibited native settings, and block effects.
...
1. Add failing resolver tests for:
   ...
   - non-Alpha builds block provisional execution;
...
6. Make `resolve_skill_route()` apply the approved precedence and construct a deterministic provisional route only in the Alpha no-data state.
...
- Modify: relevant Operator model-routing and Alpha release documents discovered by reference search
...
1. Document Medium as an Alpha no-data fallback, not a qualified recommendation.
...
6. Commit the verified implementation on `alpha/0.02alpha1`. Do not push or merge unless the Operator requests it.
