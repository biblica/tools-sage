...
| `GOVERNED_SKILL` | A semantic or linguistic decision that affects a finding, rewrite, adjudication, or other governed result | Requires an available exact route; every release state uses the truthful no-data Medium fallback |
...
ready. Stale, failed, unreliable, unsupported, or unavailable states fail closed; they are not
...
remain `QUALIFIED`. In a true no-data state only, the universal fallback is
...
3. `AUTOMATIC / NO DATA` always uses the policy's provider-native Medium default, independent of release state.
...
5. blocks adverse/stale/unavailable states, or always selects the Medium fallback for true no-data;
...
through the universal no-data Medium fallback. Release state does not gate this fallback.
...
|---|---|---|
| `NO_PROVISIONAL_SKILL_ROUTE` | A true no-data Skill has no available provider route supporting the Medium fallback | Connect/refresh the provider or qualify a route |
