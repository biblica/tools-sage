...
### Follow-up (2026-09-17): fold Number Style into Language Profile; derive setup samples from CLDR

Two further questions raised in the same thread:

1. **Should Number Style Profile fold into Language Profile?** Today Number Style is a fully separate, independently-selected profile (`system/config/profiles/numbers/`), chosen per NCA Job via `choose_style()`/`style_profile_candidates()` in `nca_menu.py`/`numbers/style.py`, filtered by language+script but not actually part of the Language Profile object. Grammar Profiles already solve the equivalent problem via `LanguageProfileVariantSpec`'s `role` field (a Language Profile owns multiple role-specific variants). Recommendation: fold Number Style in as another variant role under that same mechanism instead of a parallel profile type — auto-derive a sensible default from language+region with no extra operator step, while still allowing an explicit per-project override variant when a house style deviates from the regional default (matching how grammar variants already work).

2. **Minimize setup input by deriving sample alphabet/numbers/punctuation/number formats from the selected language+region.** This is the same bundled-snapshot architecture as the ISO 639-3/3166-1 discussion above, but needs a richer source: the bundled ISO files only carry names, not script glyphs or number-formatting patterns. The correct official source for exactly this content is **CLDR** (Unicode's Common Locale Data Repository) — the standard repo for per-locale digit sets, decimal/grouping separators, and native script samples. Recommendation: bundle a periodically-refreshed slice of CLDR data (only the locales actually in use) rather than a live fetch, so setup can show a derived preview ("here's what pt-BR numbers/punctuation typically look like") for the operator to confirm or override, instead of asking them to type it in.
