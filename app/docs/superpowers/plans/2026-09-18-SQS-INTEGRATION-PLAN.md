...
- [ ] Add `services/sqs/contracts/execution-channel-descriptor.schema.json`: `{execution_channel, provider_family, display_name, provisioning_model, capability_classes[], reasoning_tier_taxonomy}`. `execution_channel` is an open string (`codex_workspace` today, `api_key` for a future direct-API channel to the same `provider_family`) — not an enum of two, since new channels will appear, possibly to the same model vendor. `provisioning_model` distinguishes org-managed seat provisioning (`workspace`, e.g. every staff member draws on the org's OpenAI Codex workspace entitlement — the confirmed first binding) from per-integration credentials (`api_key`) — this affects how a channel's capability/tier assumptions can be trusted to hold uniformly across all staff versus needing per-credential verification.
- [ ] Add `services/sqs/seed/execution-channels/codex-workspace.yml` as the first descriptor: `execution_channel: codex_workspace`, `provider_family: openai`, `provisioning_model: workspace`. Leave `seed/openai-provider.yml`'s `provider_family: openai` model catalog as-is (it's correct as the model vendor's catalog); just wire the previously-unused `execution_provider_aliases` field into something real, or replace it with a reference to the new descriptor file.
- [ ] Add `execution_channel` as a required field on `qualifications[]` in `contracts/sqs-bundle.schema.json` (1.1) alongside the existing `provider_family` — a qualification is now keyed on (provider_family, model_id, execution_channel, profile_id, capability), not just the first three. Confirm via `git grep` whether anything on the SAGE side already assumes a 4-tuple vs the current 3-tuple key before changing it.
- [ ] `contracts/sqs-bundle-1.0.schema.json` stays `provider_family: const "openai"` (still correct under the two-field model) — no change needed there; only 1.1's qualification key changes.
- [ ] Write a cross-catalog consistency test: any `execution_channel` appearing in a published bundle must exist in the execution-channel descriptor catalog, and (once Task 6 exists) in SAGE's governed-provider allowlist. Fail closed — an unregistered channel in a bundle is a hard error, not a warning.
- [ ] Update `evaluation/planner.py`/`domain.py`/`qualification.py`'s fingerprint and evidence canonicalization to include `execution_channel` wherever it currently only hashes/records `provider_family` + `model_id` (grep first; report findings before changing, since this changes `capability_fingerprint`/`evidence_sha256` shape).
