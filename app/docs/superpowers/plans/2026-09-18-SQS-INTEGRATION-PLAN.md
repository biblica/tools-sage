...
- [ ] Add `services/sqs/contracts/provider-descriptor.schema.json`: `{provider_family, display_name, access_mode, capability_classes[], reasoning_tier_taxonomy}`. `access_mode` is an open string (e.g. `chatgpt_auth_cli` for `codex`, `api_key` for a future direct-API provider) — not an enum of two, since new access modes will appear.
- [ ] Add `services/sqs/seed/providers/codex.yml` as the first descriptor, replacing the assumption baked into `seed/openai-provider.yml`. Keep `openai-provider.yml` only if a real direct-OpenAI-API provider is ever separately onboarded; otherwise remove it rather than leave a stale, unused seed.
