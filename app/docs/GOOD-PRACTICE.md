...
- Treat Ollama as optional local-admin infrastructure only; it remains disabled
  for governed BIC/RTC/STC work.
- Map Paratext/PTLite resources directly when appropriate; external reads are `.SFM`/`.VRS` only.
- Use `READ_ONLY_SCRIPTURE` for every SOURCE, DONOR, RTC/STC WIP, RTC REFERENCE, and OL mapping.
...
- Keep BIC and RTC/STC project configuration independent; do not create handoff or role-conversion conventions between them.
...
- Keep RTC/STC partition units separate and aggregate only after finalization.
