...
| Cue / code | Action |
|---|---|
| `macOS quarantine is attached to the SAGE launch/runtime boundary` | SAGE stopped before installing or importing native Python wheels. Verify the release ZIP with its adjacent `.sha256` receipt, then use the checksum-first macOS authorization steps in `RECOVERY.md`. Do not disable Gatekeeper globally. |
| `xattr: Permission denied: .../.git/objects/...` | A broad quarantine-removal command reached Git's intentionally read-only object database. Do not change those permissions. Use the Git-checkout-scoped command in `RECOVERY.md`; remaining `.git` quarantine does not affect SAGE execution. |
