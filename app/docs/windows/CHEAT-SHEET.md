...
Frozen TUI preview (`0.02b1`):

```bat
.\sage.cmd tui
```

The TUI uses the supplemental `system\requirements-tui.txt` profile. Its current navigation/status baseline remains available, but additional workflow functionality is deferred to a later release. If the supplemental profile cannot be installed, the classic menu and scriptable CLI remain available.
...
The classic menu remains authoritative for `0.02b1`. Further TUI action parity is paused until a later release.
