...
By default, SAGEdata is created as a sibling of Core: `<parent>/SAGE/` and `<parent>/SAGEdata/`. Use `sage data-home show`, `sage data-home set PATH`, or `SAGE_DATA_HOME` to use another location. Core contains only version-controlled product material and approved built-in resources; all operator/machine-local data is external. See `docs/advanced/architecture/STORAGE-AND-CORE-BOUNDARY.md`.

Open a terminal in the SAGE Core root and run the matching launcher. The root files are thin forwarding entry points; application launchers and bootstrap logic remain under `system/bin/`. Do **not** start Codex first. SAGE remains the parent process, resolves or creates the external `SAGEdata` root, creates/validates the managed Python environment at `SAGEdata/.system/runtime/venv`, checks prerequisites, resumes incomplete setup when needed, remembers the last active Job/Run, and invokes Codex only for sign-in or bounded AI work. First use does not require a separate setup command.
...
The source ZIP contains Core only. It does **not** contain a Python virtual environment, Jobs, reports, imported Projects, local resources, caches, or machine state. The machine/OS-specific environment is created at `SAGEdata/.system/runtime/venv` on first approved launch. Startup prints the exact SAGE root, SAGEdata root, managed-environment path, Python version, and pre-release state-policy status before application setup begins.

For a Git-based deployment, normal use is `git clone` followed by `./sage` or `.\sage.cmd`; the launcher bootstraps SAGEdata automatically. The optional [clone helper](system/tools/CLONE-AND-INSTALL.md) automates the same flow and can bind an existing Paratext Projects root on a new host. It never deletes an existing recognized SAGEdata directory.

Pre-release version changes do **not** delete operator, Project, Job, Run, report, resource, or settings data. `SAGE/` is replaceable Git-controlled Core; `SAGEdata/` is persistent local state. A clean first installation begins with zero SAGE Projects, while a Git update or re-clone can reuse an existing recognized SAGEdata root. There is no legacy-layout migration feature in this Beta; the sibling SAGE/SAGEdata contract is the canonical 0.01beta layout.
