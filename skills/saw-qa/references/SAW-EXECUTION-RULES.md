...
- Normal translation/meaning QA keeps deterministic discourse units intact, then coalesces adjacent units toward the governed token target. A natural unit is one prose paragraph, one `li1` major list unit with subordinate `li2+`, or one operational poetry stanza. An operational poetry stanza is the maximal uninterrupted run of `q/q#`, `qm/qm#`, `qr`, `qc`, or `qd`; `v` never breaks it. Break at `b`, `qa`, section/major-section boundaries, Psalm chapter/associated `cl`, `d`, or transition to non-poetry. Do not infer additional literary stanza boundaries. Explicitly labelled context-only coordinates may inform interpretation but must not appear in coverage, review receipts, or ordinary findings.
